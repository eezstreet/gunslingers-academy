...
{//RACC - update position for all the movers on this "team".
	...
	for ( part = ent ; part ; part = part->teamchain ) {
		...
		//eezstreet fixed: call the reached function for angular moves too
		if( part->s.apos.trType == TR_LINEAR_STOP ||
			part->s.apos.trType == TR_NONLINEAR_STOP )
		{
...
//	gentity_t	*other = ent->enemy;
	...
	ent->activator = activator; //FIXME: ...what the hell is this nonsense?
...
void SP_func_static( gentity_t *ent ) 
{
	...
	G_SpawnInt("gametype", "-1", &gametype);
	if(gametype == g_gametype.integer){
		//spawn me
	}
	else if(gametype != g_gametype.integer && gametype > GT_MAX_GAME_TYPE){
		//might belong to a gametype group
		switch(gametype){
			case GT_MAX_GAME_TYPE+1:
				IFNGAME((GT_FFA|GT_HOLOCRON|GT_JEDIMASTER|GT_DUEL|GT_POWERDUEL|GT_SINGLE_PLAYER)){
					G_FreeEntity(ent);
					return;
				}
				break;
			case GT_MAX_GAME_TYPE+2:
				IFNGAME((GT_FFA|GT_HOLOCRON|GT_JEDIMASTER|GT_DUEL|GT_POWERDUEL)){
					G_FreeEntity(ent);
					return;
				}
				break;
			case GT_MAX_GAME_TYPE+3:
				IFNGAME((GT_FFA|GT_HOLOCRON|GT_JEDIMASTER)){
					G_FreeEntity(ent);
					return;
				}
				break;
			case GT_MAX_GAME_TYPE+4:
				IFNGAME((GT_DUEL|GT_POWERDUEL)){
					G_FreeEntity(ent);
					return;
				}
				break;
			case GT_MAX_GAME_TYPE+5:
				IFNGAME((GT_TEAM|GT_SIEGE|GT_CTF|GT_CTY|GT_DOMINATION|GT_VIP_ESCORT|GT_COMMANDER)){
					G_FreeEntity(ent);
					return;
				}
				break;
			case GT_MAX_GAME_TYPE+6:
				IFNGAME((GT_CTF|GT_CTY)){
					G_FreeEntity(ent);
					return;
				}
				break;
			case GT_MAX_GAME_TYPE+7:
				IFNGAME((GT_VIP_ESCORT|GT_COMMANDER)){
					G_FreeEntity(ent);
					return;
				}
				break;
		...
		//all gametypes are accepted
	}
	else{ //your gametypes didn't match up
		G_FreeEntity(ent);
		return;
	}
