...
void DetPackNoBlow(gentity_t *self)
{
...
void charge_stick (gentity_t *self, gentity_t *other, trace_t *trace)
{
	...
	{//stick to it?
		...
		!Q_stricmp(other->classname, "func_door_new")/* && other->doorState == 0*/)
	{
...
void DetPackBlow(gentity_t *self)
{
	...
	level.explodeTime = level.time + 7500;
...
void drop_charge (gentity_t *self, vec3_t start, vec3_t dir) 
{
	...
	VectorScale(dir, weaponData[self->s.weapon].bulletSpeed, bolt->s.pos.trDelta );
	...
	VectorSet(bolt->s.apos.trDelta, weaponData[self->s.weapon].bulletSpeed, 0, 0 );
	bolt->s.apos.trTime = level.time;
	...
	if ( ent->client->ps.hasDetPackPlanted )
	{
		...
		{//loop through all ents and blow the crap out of them!
			if ( found->parent == ent )
			{
				...
				//[/CoOp]
				// change the dynamic music to action
