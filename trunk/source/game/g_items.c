...
#define MAX_SENTRY_DISTANCE			756
...
void ItemUse_Binoculars(gentity_t *ent)
{
	...
	*/
	...
	//Fuck binoculars. Let's maek radar!

}

void ItemUse_Radar(gentity_t *ent)
{
	if (!ent || !ent->client)
	{
		return;
	...
	level.radarTimes[ent->client->ps.clientNum] = level.time + 30000;
}
...
//----------------------------------------------------------------
{
	...
	WP_FireTurretMissile(&g_entities[ent->genericValue3], myOrg, fwd, qfalse, 10, 7000, MOD_SENTRY, ent );
...
{
	qboolean	moved;
	float		diffYaw, diffPitch, diffRoll;
	vec3_t		enemyDir, org;
...
//	G_RunObject(ent);
	...
	moved = qfalse;
	diffYaw = 0.0f; diffPitch = 0.0f; diffRoll = 0.0f;
	...
	if ( ent->enemy )
	{
		...
		diffRoll = AngleSubtract( ent->random, desiredAngles[ROLL] );
	}
	else
	{
	...
	if ( ent->enemy && ent->attackDebounceTime < level.time )
	{
		...
		if ( ent->count )
		{
			pas_fire( ent );
			ent->s.fireflag = 1;
			ent->attackDebounceTime = level.time + 70;
		}
		else
		{
			//ent->nextthink = 0;
...
void EWebFire(gentity_t *owner, gentity_t *eweb)
{
	...
	//create the missile
	missile = CreateMissile( bPoint, d, 7200.0f, 10000, owner, qfalse );
			...
			{ //make sure the anim timer is done
				...
				if (owner->client->pers.cmd.buttons & BUTTON_ATTACK)
				{
					...
					{ //we can fire another shot off
						...
						self->genericValue5 = level.time + 50;
					}
				}
...
int Pickup_Ammo (gentity_t *ent, gentity_t *other)
{
	...
	if ( ent->count ) {
		...
		quantity = ammoData[ent->item->giTag].max / 4;
	}
...
//[/VisualWeapons]
int Pickup_AmmoForWeapon (gentity_t *ent, gentity_t *other)
{
...
int Pickup_Weapon (gentity_t *ent, gentity_t *other) {
	int		quantity;
	weaponData_t	weapon;
	qboolean hadWeaponBefore = qfalse;

	weapon = weaponData[ent->item->giTag];
...
int Pickup_Health (gentity_t *ent, gentity_t *other) {
	...
	// return speed upon health pickup or more than maximum health
	/*other->client->ps.speed += quantity;
	if (other->client->ps.speed > g_speed.integer) {
		other->client->ps.speed = g_speed.integer;
	}
...
void Touch_Item (gentity_t *ent, gentity_t *other, trace_t *trace) {
	...
	qboolean	predict;
	weaponData_t *weapon;
	weaponData_t *yourGun;
	qboolean	swap;
#ifdef AKIMBO
	qboolean	equipAsAkimbo = qfalse;
#endif
	qboolean switchGuns = qfalse;
	qboolean tossLeftGun = qtrue;


	weapon = &weaponData[ent->item->giTag];
	if(!other->client)
		return;
	yourGun = &weaponData[other->client->ps.weapon];
	...
	if(g_gametype.integer != GT_SINGLE_PLAYER || ent->item->giType != IT_WEAPON)
	{
	...
	if(!(other->client->pers.cmd.buttons & BUTTON_USE) &&
		...
		g_gametype.integer != GT_JEDIMASTER){
			...
			{ //CO-OP override
				...
				if(numGuns < 2 && 
					(weaponData[ent->item->giTag].weaponType == WT_PRIMARY ||
					weaponData[ent->item->giTag].weaponType == WT_SIDEARM))
				{
					goto pickMeUpQuickly;
				}
				else if(numExplosives < 1 && weaponData[ent->item->giTag].weaponType == WT_EXPLOSIVE)
				{
pickMeUpQuickly:
					G_LogPrintf( "Item: %i %s\n", other->s.number, ent->item->classname );
					predict = other->client->pers.predictItemPickup;
					other->client->ps.ammo[ent->item->giTag] += (ammoData[weapon->ammoIndex].max / 4); //Give us some initial ammo.
					other->client->lastWeaponSwap = level.time;
					if(other->client->ps.ammo[weapon->ammoIndex] > ammoData[weapon->ammoIndex].max)
							other->client->ps.ammo[weapon->ammoIndex] = ammoData[weapon->ammoIndex].max;
			...
			if(!BG_CanItemBeGrabbed( g_gametype.integer, &ent->s, &other->client->ps ))
				return;
			if(ent->s.eFlags & EF_DROPPEDWEAPON && !(other->client->ps.stats[STAT_WEAPONS] & (1 << ent->item->giTag))){
				//trap_SendServerCommand(other->s.number, va("weps %i", ent->item->giTag));
				return;
			} else if(!(ent->s.eFlags & EF_DROPPEDWEAPON)){
				//trap_SendServerCommand(other->s.number, va("weps %i", ent->item->giTag));
				return;
			} else if(ent->s.eFlags & EF_DROPPEDWEAPON && other->client->ps.stats[STAT_WEAPONS] & (1 << ent->item->giTag)){
#ifdef AKIMBO
				if(!weaponData[ent->item->giTag].akimbo) {
#endif
					swap = qfalse;
#ifdef AKIMBO
				} else
...
#ifdef AKIMBO
		if(other->client->pers.cmd.buttons & BUTTON_SIGHTS)
		{ //Have sights down - sure fire sign that we want to akimbo this!
			if(weaponData[ent->item->giTag].akimbo && weaponData[other->s.weapon].akimbo)
			{
				equipAsAkimbo = qtrue;
			}
		}
	...
	switch( ent->item->giType ) {
		case IT_WEAPON:
			if((level.time - other->client->lastWeaponSwap) < 350)
				return;
			if(!ent->ammoOnItem){
				//Dropped weapons store the ammo in the current weapon. Old School weapon pickups restore ammo.
				//If we have the dropped weapon, it discards the dropped weapon and adds the ammo off the weapon.
				ent->ammoOnItem = (ammoData[weapon->ammoIndex].max / 4);
			}
#ifdef AKIMBO
			if(equipAsAkimbo)
			{
				if(other->s.weapon2)
				{
					if(other->s.weapon2 == ent->item->giTag)
					{
						return;
					}
					TossClientItems(other, qfalse, qtrue);
					other->client->akimboCounterPart[other->client->ps.weapon] = ent->item->giTag;
					other->client->ps.weapon2 = ent->item->giTag;
					other->client->lastWeaponSwap = level.time + 1000;
				}
				respawn = adjustRespawnTime(g_weaponRespawn.integer, ent->item->giType, ent->item->giTag);
				//if(ent->s.weapon2)
				WP_SetAkimboWeapon(other, ent->item->giTag, other->s.weapon);
				other->client->pers.cmd.buttons &= ~BUTTON_USE; //Okay, you can let go now...
			}
			else
#endif
			if(swap || g_gametype.integer == GT_JEDIMASTER){
				if(other->client->ps.stats[STAT_WEAPONS] & (1 << ent->item->giTag ) && g_gametype.integer != GT_JEDIMASTER)
				{
					other->client->ps.ammo[ent->item->giTag] += ent->ammoOnItem;
					respawn = Pickup_AmmoForWeapon(ent, other);
					switchGuns = qfalse;
					if(other->client->akimboCounterPart[ent->item->giTag] == ent->item->giTag)
					{
						return;
					}
				}
				else
				{
					...
					{
						other->client->ps.ammo[yourGun->ammoIndex] = 0;
#ifdef AKIMBO
						if(other->client->ps.weapon2 && weaponData[other->client->ps.weapon].akimbo && weaponData[ent->item->giTag].akimbo && ent->item->giTag != other->client->ps.weapon)
						{
							int oldweapon = other->client->ps.weapon;
							other->client->ps.stats[STAT_WEAPONS] &= ~(1 << other->client->ps.weapon);
							other->client->akimboCounterPart[ent->item->giTag] = other->client->ps.weapon2;
							other->client->akimboCounterPart[other->client->ps.weapon] = 0;
							other->client->ps.weapon = ent->item->giTag;
							trap_SendServerCommand(other->s.number, va("brokenakimbo %i %i %i", other->client->ps.weapon, oldweapon, other->client->ps.weapon2));
							switchGuns = qtrue;
							tossLeftGun = qfalse;
						}
						else
				...
				other->client->pers.cmd.buttons &= ~BUTTON_USE; //Okay, you can let go now...
				if(ent->s.eFlags & EF_DROPPEDWEAPON)
#ifdef AKIMBO
				{
					if(!tossLeftGun)
					{
						TossClientItems(other, qfalse, 2);
					}
					else if(0)
					{ //Have an akimbo gun and we're switching our right one
						//Toss our right hand weapon
						TossClientItems(other, qfalse, 2);
						if(weaponData[ent->item->giTag].akimbo)
						{ //Our desired weapon can be akimbo'd...let's switch the right hand weapon
							WP_SetAkimboWeapon(other, other->s.weapon2, ent->item->giTag);
						}
						WP_SetAkimboWeapon(other, WP_NONE, ent->item->giTag);
					}
					else
					{
						if(tossLeftGun)
							TossClientItems(other, qfalse, qfalse);
						else
							TossClientItems(other, qfalse, 2);
					}
				}
#else
					TossClientItems(other, qfalse);
#endif
				if(OJP_AllPlayersHaveClientPlugin() && switchGuns)
					G_AddEvent(other, EV_WEAPINVCHANGE, other->client->ps.stats[STAT_WEAPONS]);
				other->client->lastWeaponSwap = level.time;
			}
			else
			...
			ent->item->ready = qtrue;
			other->client->ps.ammo[weapon->ammoIndex] += ent->ammoOnItem;
			if(other->client->ps.ammo[weapon->ammoIndex] > ammoData[weapon->ammoIndex].max)
				other->client->ps.ammo[weapon->ammoIndex] = ammoData[weapon->ammoIndex].max;
		//		predict = qfalse;
			predict = qtrue;
...
keepOnRolling:
	...
	if(ent->item->giType == IT_WEAPON && !switchGuns)
	{
		if (predict) {
			if (other->client)
			{
				BG_AddPredictableEventToPlayerstate( EV_AMMO_PICKUP, ent->s.number, &other->client->ps);
			}
			else
			{
				G_AddPredictableEvent( other, EV_AMMO_PICKUP, ent->s.number );
			}
		} else {
			G_AddEvent( other, EV_AMMO_PICKUP, ent->s.number );
		}
	} else {
		if (predict) {
			if (other->client)
...
gentity_t *LaunchItem( gitem_t *item, vec3_t origin, vec3_t velocity ) {
	...
	if ((g_gametype.integer == GT_CTF || g_gametype.integer == GT_CTY) && item->giType == IT_TEAM) { // Special case for CTF flags
		...
		dropped->nextthink = level.time + 30000/((level.numDroppedGuns > 16) ? (level.numDroppedGuns/8) : 1);
	}
	...
	}*/
	{
		...
		VectorAdd(dropped->s.angles, weaponData[item->giTag].lerpAngles, output);
		VectorCopy(output, dropped->s.angles);
	}
...
//	VectorSet( ent->r.maxs, ITEM_RADIUS, ITEM_RADIUS, ITEM_RADIUS );
	...
	if (g_gametype.integer == GT_DUEL || g_gametype.integer == GT_POWERDUEL)
	{
		if ( ent->item->giType == IT_ARMOR ||
			...
			(ent->item->giType == IT_HOLDABLE && (ent->item->giTag == HI_MEDPAC || ent->item->giTag == HI_MEDPAC_BIG || ent->item->giTag == HI_AIRSTRIKE)) )
		{
			G_FreeEntity(ent);
			return;
		}
...
void G_SpawnItem (gentity_t *ent, gitem_t *item) {
	...
	RegisterItem( item );
	if ( !wDisable && g_gametype.integer != GT_JEDIMASTER ){
		G_FreeEntity( ent );
		return;
	}
...
Does stuff when the item gets used
==========================
*/
extern gentity_t *KS_FireMissileBomb( gentity_t *ent );
void G_KillstreakOptions(gentity_t *ent, unsigned int itemNum)
{
	int i = 0;
	if(!ent->client)
		return;
	switch(itemNum)
	{
		case HI_BINOCULARS:
			...
			else
			{
				...
				if(g_gametype.integer >= GT_TEAM)
				{
					for( ; i < MAX_CLIENTS; i++ )
					{
						if(g_entities[i].client)
						{
							...
							{
								trap_SendServerCommand(i, va("radarKSReward %i %i", rewardVariationForReward(HI_BINOCULARS), ent->client->ps.clientNum));
							}
						}
					}
				}
				...
				{
					trap_SendServerCommand(ent->client->ps.clientNum, va("radarKSReward %i %i", rewardVariationForReward(HI_BINOCULARS), ent->client->ps.clientNum));
				}
				ent->client->ps.stats[STAT_HOLDABLE_ITEMS] &= ~(1 << HI_BINOCULARS);
			}
			break;
		case HI_AIRSTRIKE:
			KS_FireMissileBomb(ent);
			break;
	}
}