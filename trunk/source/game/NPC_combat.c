...
{//racc - sets initial attack delay and roamTime for new enemy  
	...
	{//delay their attack based on how far away they're facing from enemy
		...
		{
			case WP_BLASTER:
				attDelay = (4-g_spskill.integer)*10;
				break;
			...
			case WP_BOWCASTER:
				...
				break;
			case WP_DISRUPTOR:
				attDelay = (4-g_spskill.integer)*1000;
		}
		...
		//if( self->client->playerTeam != NPCTEAM_PLAYER )
		{
			if(self->s.weapon != WP_MELEE && self->s.weapon != WP_SABER)
			{
				if(weaponData[self->s.weapon].hcIdx == HC_M14)
				{ //M14 is a weird case because it doesn't fire quite as quickly as the firetime would suggest
					attDelay = (4-g_spskill.integer)*300;
				}
				else
				{
					attDelay = weaponData[self->s.weapon].fireTime[0];
				}
			}
		}
		if(self->s.weapon != WP_BLASTER && self->s.weapon != WP_BOWCASTER)
			attDelay += floor( (DotProduct( fwd, dir )+1.0f) * 5.0f );//add up to 500ms delay if they're facing away
		...
		*/
		...
		case WP_BLASTER:
			...
			{//rapid-fire blasters
				attDelay += Q_irand( 0, 20 );
			}
			else
			{//regular blaster
				attDelay -= Q_irand( 0, 70 );
			}
			break;
		case WP_BOWCASTER:
			attDelay += Q_irand( 0, 40 );
			break;
		case WP_REPEATER:
			if ( !(self->NPC->scriptFlags&SCF_ALT_FIRE) )
			{//rapid-fire blasters
				attDelay += Q_irand( 0, 10 );
			}
			break;
		case WP_FLECHETTE:
			attDelay += Q_irand( 500, 1500 );
...
{//racc - set this enemy for this NPC
	...
	if(enemy->client)
	{
		if(enemy->client->ps.pm_type == PM_DEAD)
		{
			return; //Can't target dead enemies
		}
	}
	...
	*/
	...
	//	ent->NPC->burstSpacing = 1000;//attackdebounce
		/*if ( g_spskill.integer == 0 )
			...
			ent->NPC->burstSpacing = 500;//attack debounce*/
		ent->NPC->burstMin = 3;
		ent->NPC->burstMean = 24;
		ent->NPC->burstMax = 30;
		ent->NPC->burstSpacing = weaponData[WP_BOWCASTER].fireTime[0];
		break;
	...
	case WP_REPEATER:
		...
		else
		{
			...
			/*if ( g_spskill.integer == 0 )
				...
				ent->NPC->burstSpacing = 500;//attack debounce*/
			ent->NPC->burstSpacing = weaponData[WP_REPEATER].fireTime[0];
	...
	*/
	...
	case WP_BLASTER:
		...
		else
		{
			...
			ent->NPC->burstMax = 30;
			ent->NPC->burstSpacing = weaponData[WP_BLASTER].fireTime[0];
			/*if ( g_spskill.integer == 0 )
				ent->NPC->burstSpacing = 1000;//attack debounce
			else if ( g_spskill.integer == 1 )
				ent->NPC->burstSpacing = 750;//attack debounce
		...
		//ent->NPC->burstSpacing = weaponData[WP_BLASTER].fireTime;
		break;
...
*/
	...
	else 
	{
		...
		//delay = NPCInfo->burstSpacing + Q_irand(-150, 150);
		//delay = NPCInfo->burstSpacing;
		delay = weaponData[client->ps.weapon].reloadTime;
		if(!delay)
		{
			delay = 1000;
		}
...
void WeaponThink( qboolean inCombat ) 
{
	if ( client->ps.weaponstate == WEAPON_RAISING || client->ps.weaponstate == WEAPON_DROPPING ) 
	{
		if(ucmd.buttons & BUTTON_SPRINT){
			return; //just sprint, don't fuck around
		}
		else{
...
//	qboolean	duck_ok = qfalse;
	...
	{//He's in our FOV
		...
		CalcEntitySpot( NPC->enemy, SPOT_CHEST, enemy_head);
...
//	NPCInfo->tempGoal->waypoint = foundWeap->r.currentOrigin;
