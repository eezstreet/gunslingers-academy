...
void AddScore( gentity_t *ent, vec3_t origin, int score )
{
	...
	*/
	...
	IFGAME(GT_HERO){
		if(ent->s.number == level.redHero ||
			ent->s.number == level.blueHero){
				score *= 2;
		}
	}
	if ( !ent->client ) {
	...
	//
	if ( (g_gametype.integer == GT_FFA || g_gametype.integer == GT_JEDIMASTER) && level.numPlayingClients > 1 && score > 0)
	{
		for(i = 0; i < level.numConnectedClients; i++)
		{
			if(level.clients[i].ps.persistant[PERS_SCORE] > ent->client->ps.persistant[PERS_SCORE])
				break;
			else if(level.clients[i].ps.clientNum == ent->s.number)
				continue;
			else if(level.clients[i].ps.persistant[PERS_SCORE] == ent->client->ps.persistant[PERS_SCORE])
			{
				equalScores[numEqualScores++] = level.clients[i].ps.clientNum;
			}
		}
		if(numEqualScores > 0)
		{
			for(i = 0; i < numEqualScores; i++)
			{
				trap_SendServerCommand(level.clients[equalScores[i]].ps.clientNum, "forceiswithyou 0");
			}
			if(ent->client->ps.persistant[PERS_RANK] & RANK_TIED_FLAG && (ent->client->ps.persistant[PERS_RANK] & ~RANK_TIED_FLAG)==0)
			{
				trap_SendServerCommand(ent->s.number, "forceiswithyou 1");
			}
		}
	}
	else if ( g_gametype.integer == GT_TEAM || g_gametype.integer == GT_HERO && !g_dontPenalizeTeam ){
		...
		if ( team == TEAM_RED ) {
			if ( level.teamScores[ TEAM_RED ] + score == level.teamScores[ TEAM_BLUE ] ) {
				...
				// red scored sound
				//ent->s.eventParm = GTS_REDTEAM_SCORED;
			}
		}
		else {
			...
			if ( level.teamScores[ TEAM_BLUE ] <= level.teamScores[ TEAM_RED ] &&
				...
				// blue scored sound
				//ent->s.eventParm = GTS_BLUETEAM_SCORED;
			}
		}
	}
...
//[/CoOp]
#ifdef AKIMBO
void TossClientItems( gentity_t *self, qboolean death, int onlyLeftWeapon ) {
#else
void TossClientItems( gentity_t *self, qboolean death ) {
#endif
	gitem_t		*item;
	int			weapon;
#ifdef AKIMBO
	...
	{
#ifdef AKIMBO
		self->s.weapon2 = WP_NONE;
#endif
		self->s.weapon = WP_NONE;
		item = NULL;

#ifdef AKIMBO
		if(weapon2 != WP_NONE && weapon2 != WP_SABER && weapon2 != WP_MELEE)
		{// find the item type for this weapon
			item = BG_FindItemForWeapon( (weapon_t) weapon2 );
		}
		if ( item && !dropped2 && onlyLeftWeapon != 2)
		{//we have a weapon and we haven't already dropped it.
			gentity_t *te; //temp entity used to tell the clients to remove the body's
			
			// spawn the item
			dropped2 = Drop_Item( self, item, 45.0f );
			dropped2->nextthink = level.time + (g_gunRemoveTime.integer*1000);
			dropped2->think = G_FreeEntity;
			dropped2->item->ready = qfalse;
			// Now, drop your weapon!
			{
				weaponData_t *ourGun;
				ourGun = &weaponData[weapon];
				dropped2->ammoOnItem = self->client->ps.ammo[ourGun->ammoIndex];
				dropped2->s.weapon = weapon2;
				dropped2->s.eFlags2 |= EF2_SWAPPABLE_WEAPON;
			}
			//MP version
...
#endif
		if ( weapon == WP_THERMAL /*|| weapon == WP_TRIP_MINE*/ && death )
		...
		else if(weapon != WP_NONE && weapon != WP_SABER && weapon != WP_MELEE)
		{// find the item type for this weapon
			item = BG_FindItemForWeapon( (weapon_t) weapon );
		}
		if ( item && !dropped && onlyLeftWeapon != 1)
		{//we have a weapon and we haven't already dropped it.
			gentity_t *te; //temp entity used to tell the clients to remove the body's
			
			// spawn the item
			dropped = Drop_Item( self, item, 0 );
			dropped->nextthink = level.time + (g_gunRemoveTime.integer*1000);
			dropped->think = G_FreeEntity;
			dropped->item->ready = qfalse;
			// Now, drop your weapon!
			{
				weaponData_t *ourGun;
				ourGun = &weaponData[weapon];
				dropped->ammoOnItem = self->client->ps.ammo[ourGun->ammoIndex];
				dropped->s.weapon = weapon;
				dropped->s.eFlags2 |= EF2_SWAPPABLE_WEAPON;
			}
			//MP version
			// tell all clients to remove the weapon model on this guy until he respawns
			if(death){
...
void player_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath ) {
	...
	if(g_gametype.integer == GT_VIP_ESCORT || g_gametype.integer == GT_COMMANDER){
		if(g_gametype.integer == GT_VIP_ESCORT){
			if(self->s.clientNum == level.vipPlayer){
				if(meansOfDeath != MOD_UNKNOWN && meansOfDeath != MOD_STUN_BATON && meansOfDeath != MOD_SUICIDE && meansOfDeath != MOD_FALLING){
					if(attacker->client->sess.sessionTeam == TEAM_BLUE){
						AddTeamScore(attacker->client->ps.origin, TEAM_RED, 1);
						trap_SendServerCommand(-1, "cp \"Blue team! You killed the VIP! Red wins.\"");
						gEscaping = qtrue;
						LogExit("Blue team! You killed the VIP!");
					}
					else{
						AddTeamScore(attacker->client->ps.origin, TEAM_BLUE, 1);
						trap_SendServerCommand(-1, "cp \"Red team failed to dispose of the VIP properly.\nBlue wins.\"");
						gEscaping = qtrue;
						LogExit("Red team failed to dispose of the VIP properly.\nBlue wins.");
					}
				}
				else if(meansOfDeath == MOD_STUN_BATON){
					if(attacker->client->sess.sessionTeam == TEAM_BLUE){
						AddTeamScore(attacker->client->ps.origin, TEAM_RED, 1);
						trap_SendServerCommand(-1, "cp \"Blue team! What are you thinking?\"");
						gEscaping = qtrue;
						LogExit("Blue team, what are you thinking?");
					}
					else{
						AddTeamScore(attacker->client->ps.origin, TEAM_BLUE, 1);
						trap_SendServerCommand(-1, "cp \"Red team has captured the VIP.\"");
						gEscaping = qtrue;
						LogExit("Red team has captured the VIP. Red wins.");
					}
				}
				else if(meansOfDeath == MOD_SUICIDE){
					level.anotherVIP = qtrue;
					trap_SendServerCommand(-1, "print \"The VIP commited suicide.\nAuto-picking a new one...\n\"");
					trap_SendServerCommand(-1, "cp \"The VIP commited suicide.\nAuto-picking a new one...\n\"");
				}
				else if(meansOfDeath == MOD_FALLING){
					level.anotherVIP = qtrue;
					trap_SendServerCommand(-1, "print \"The VIP fell. Picking a new one...\n\"");
					trap_SendServerCommand(-1, "cp \"The VIP fell. Picking a new one...\n\"");
				}
			}
		}
		else if(g_gametype.integer == GT_COMMANDER){
			if(self->s.clientNum == level.blueTeamCommander){
				if(meansOfDeath != MOD_UNKNOWN && meansOfDeath != MOD_SUICIDE && meansOfDeath != MOD_FALLING){
					if(attacker->client->sess.sessionTeam == TEAM_BLUE){
						AddTeamScore(attacker->client->ps.origin, TEAM_RED, 1);
						LogExit("Blue team! You killed your own Commander!");
					}
					else{
						AddTeamScore(attacker->client->ps.origin, TEAM_RED, 1);
						LogExit("Excellent work, Red. The Commander is dead.\nRed wins.");
					}
				}
				else if(meansOfDeath == MOD_SUICIDE){
					level.anotherBCommander = qtrue;
					trap_SendServerCommand(-1, "print \"BLUE's Commander committed suicide.\nAuto-picking a new one...\n\"");
					trap_SendServerCommand(-1, "cp \"BLUE's Commander committed suicide.\nAuto-picking a new one...\n\"");
				}
				else if(meansOfDeath == MOD_FALLING){
					level.anotherBCommander = qtrue;
					trap_SendServerCommand(-1, "print \"BLUE's Commander fell. Picking a new one...\n\"");
					trap_SendServerCommand(-1, "cp \"BLUE's Commander fell. Picking a new one...\n\"");
				}
			}
			else if(self->s.clientNum == level.redTeamCommander){
				if(meansOfDeath != MOD_UNKNOWN && meansOfDeath != MOD_SUICIDE && meansOfDeath != MOD_FALLING){
					if(attacker->client->sess.sessionTeam == TEAM_RED){
						AddTeamScore(attacker->client->ps.origin, TEAM_BLUE, 1);
						LogExit("Red team! You killed your own Commander!");
					}
					else{
						AddTeamScore(attacker->client->ps.origin, TEAM_BLUE, 1);
						LogExit("Excellent work, Blue. The Commander is dead.\nBlue wins.");
					}
				}
				else if(meansOfDeath == MOD_SUICIDE){
					level.anotherRCommander = qtrue;
					trap_SendServerCommand(-1, "print \"RED's Commander committed suicide.\nAuto-picking a new one...\n\"");
					trap_SendServerCommand(-1, "cp \"RED's Commander committed suicide.\nAuto-picking a new one...\n\"");
				}
				else if(meansOfDeath == MOD_FALLING){
					level.anotherRCommander = qtrue;
					trap_SendServerCommand(-1, "print \"RED's Commander fell. Picking a new one...\n\"");
					trap_SendServerCommand(-1, "cp \"RED's Commander fell. Picking a new one...\n\"");
				}
			}
		}
	}
	if(attacker->s.clientNum != self->s.clientNum &&
		attacker->client)
		attacker->client->ps.persistant[PERS_KILLS]++;
tryThisAgain:
	if(self->lastAttacker >= 0 && self->lastAttacker <= MAX_CLIENTS && self->s.clientNum != attacker->s.clientNum && self->lastAttacker != attacker->s.number){
		if(self->lastAttacker == attacker->s.clientNum && self->lastAttacker != self->previousAttacker)
		{
			self->lastAttacker = self->previousAttacker;
			goto tryThisAgain;
		}
		g_entities[self->lastAttacker].client->ps.persistant[PERS_ASSIST_COUNT]++;
		g_entities[self->lastAttacker].client->pers.teamState.assists++;
		trap_SendServerCommand(self->lastAttacker, va("send_event\"^5Assist Kill: %s\"", self->client->pers.netname));
		trap_SendServerCommand(self->lastAttacker, "aprogmulti 1 18 19 20");
	}
	self->previousAttacker = -1;
	self->lastAttacker = -1;

	//Sentry gun?
	if(meansOfDeath == MOD_SENTRY)
	{
#ifdef ACHIEVEMENTS
		trap_SendServerCommand(attacker->s.number, "aprog 11 1");
#endif
	}

	//Holiday Feats of Valor
	{
		int month = T_month();
		int day = T_monthday(); //note that this doesn't start at 0

		if(month == 1 && day == 14) { //Valentine's Day
			trap_SendServerCommand(attacker->s.number, "aprog 48 1");
		}
		else if(month == 2 && day == 15) { //Ides of March
			trap_SendServerCommand(attacker->s.number, "aprog 49 1");
		}
		else if(month == 4 && day == 28) { //Memorial Day
			trap_SendServerCommand(attacker->s.number, "aprog 50 1");
		}
		else if(month == 6 && day == 4) { //July 4th
			trap_SendServerCommand(attacker->s.number, "aprog 51 1");
		}
		else if(month == 9 && day == 31) { //Halloween
			trap_SendServerCommand(attacker->s.number, "aprog 52 1");
		}
		else if(month == 10 && day == 11) { //Veteran's Day
			trap_SendServerCommand(attacker->s.number, "aprog 53 1");
		}
		else if(month == 11 && day == 7) { //Pearl Harbor Remembrance Day
			trap_SendServerCommand(attacker->s.number, "aprog 54 1");
		}
	}

	self->lastKilledBy = attacker->s.number;

	//Nemesis system
	//These are 'just for fun'. They really don't have much of a point.
	//End Game Data
	if(attacker->client && attacker->s.number != self->s.number){
		qboolean grenades = qfalse;
		switch(attacker->client->ps.weapon){
			case WP_STUN_BATON:
			case WP_MELEE:
			case WP_SABER:
				attacker->client->endGameStats.killMakeUp[KILLMU_MELEE]++;
				break;
			case WP_BRYAR_PISTOL:
				attacker->client->endGameStats.killMakeUp[KILLMU_PISTOLS]++;
				break;
			case WP_DEMP2:
				attacker->client->endGameStats.killMakeUp[KILLMU_PISTOLS]++;
				break;
			case WP_BLASTER:
			case WP_BOWCASTER:
				attacker->client->endGameStats.killMakeUp[KILLMU_SMGS]++;
				break;
			case WP_REPEATER:
			case WP_BRYAR_OLD:
				attacker->client->endGameStats.killMakeUp[KILLMU_ASSAULT]++;
				break;
			case WP_CONCUSSION:
				attacker->client->endGameStats.killMakeUp[KILLMU_LMGS]++;
				break;
			case WP_FLECHETTE:
				attacker->client->endGameStats.killMakeUp[KILLMU_SHOTGUNS]++;
				break;
			case WP_DISRUPTOR:
				attacker->client->endGameStats.killMakeUp[KILLMU_SNIPERS]++;
				break;
			case WP_ROCKET_LAUNCHER:
			case WP_THERMAL:
			case WP_TRIP_MINE:
			case WP_DET_PACK:
				attacker->client->endGameStats.killMakeUp[KILLMU_EXPLOSIVES]++;
				break;
		}
		if(meansOfDeath == MOD_THERMAL || meansOfDeath == MOD_THERMAL_SPLASH)
		{
			grenades = qtrue;
			trap_SendServerCommand(attacker->s.number, "aprogmulti 1 34 35 36 37 38");
		}
		if(!grenades && meansOfDeath != MOD_UNKNOWN && meansOfDeath != MOD_SENTRY && meansOfDeath != MOD_TELEFRAG)
		{
			if(modData[meansOfDeath].headShot > 0)
			{ //HEADSHOTS go here
				switch(weaponData[attacker->client->ps.weapon].hcIdx)
				{
					case HC_M14:
						trap_SendServerCommand(attacker->s.number, "aprogmulti 1 42 43 44");
						break;
					case HC_MAGNUM:
						trap_SendServerCommand(attacker->s.number, "aprogmulti 1 39 40 41 4 5 6");
						break;
					case HC_M98C:
						trap_SendServerCommand(attacker->s.number, "aprogmulti 1 45 46 47");
						break;
				}
			}
			else if(modData[meansOfDeath].index == weaponData[attacker->client->ps.weapon].methodOfDeath || modData[meansOfDeath].index == weaponData[attacker->client->ps.weapon].altMethodOfDeath)
			{
				switch(weaponData[attacker->client->ps.weapon].hcIdx)
				{
		#ifdef ACHIEVEMENTS
					case HC_TASER:
						trap_SendServerCommand(attacker->s.number, "aprogmulti 1 25 26 27");
						break;
					case HC_GLOCK18C:
						trap_SendServerCommand(attacker->s.number, "aprogmulti 1 28 29 30");
						break;
					case HC_MELEE:
						trap_SendServerCommand(attacker->s.number, "aprogmulti 1 21 22 23");
						break;
					case HC_MANDP:
						trap_SendServerCommand(attacker->s.number, "aprogmulti 1 0 1 2");
						break;
					case HC_M16:
						trap_SendServerCommand(attacker->s.number, "aprogmulti 1 7 8 9");
						break;
					case HC_MAGNUM:
						trap_SendServerCommand(attacker->s.number, "aprogmulti 1 4 5 6");
						break;	
					case HC_GAS_GRENADE:
						trap_SendServerCommand(attacker->s.number, "aprogmulti 1 55 56 57");
						break;
		#endif
				}
			}
	...
	EvaluateMiniRewards(self, attacker, meansOfDeath);

	attacker->cumulativeDebounce = level.time;
	attacker->last_kill_time = level.time;
	self->cumulativeKills = 0;
	self->last_kill_time = 0;

	//Copy our old killstreak rewards to prevent them from being lost
	if(self->client)
		killstreakRewards[self->client->ps.clientNum] = self->client->ps.stats[STAT_HOLDABLE_ITEMS];
...
extern void RunEmplacedWeapon( gentity_t *ent, usercmd_t **ucmd );
	...
	*/
	...
	if(self->client->killStreak > 2 && attacker->client && killer != ENTITYNUM_WORLD && g_entities[self->s.number].s.eType == ET_PLAYER)
	{
		trap_SendServerCommand(-1, va("send_event\"%s's ^1%i ^7kill streak was ended by %s\"", self->client->pers.netname, self->client->killStreak, attacker->client->pers.netname));
	}
	self->client->killStreak = 0;
	if( killer != ENTITYNUM_WORLD && g_entities[killer].s.eType == ET_PLAYER && ((g_gametype.integer >= GT_TEAM && g_entities[killer].client->sess.sessionTeam != self->client->sess.sessionTeam) || g_gametype.integer < GT_TEAM) && !g_barebones.integer){ //clearly the world shouldn't be getting killstreaks - eezstreet
		attacker->client->killStreak++;
		if(attacker->client->killStreak >= 2 && attacker->client->killStreak <= 250){
			/*switch(attacker->client->killStreak){
				case 3:
//					G_Event(g_entities, EVENT_STREAK, va("%s has a 3 kill streak!", attacker->client->pers.netname));
					attacker->client->ps.stats[STAT_HOLDABLE_ITEMS] |= HI_EWEB;
					trap_SendServerCommand(killer, "ksReward 3");
					trap_SendServerCommand(-1, va("send_event\"%s ^2has a 3 kill streak!\"", attacker->client->pers.netname));
					break;
				case 5:
//					G_Event(g_entities, EVENT_STREAK, va("%s has a 5 kill streak!", attacker->client->pers.netname));
					attacker->client->ps.stats[STAT_HOLDABLE_ITEMS] |= HI_SENTRY_GUN;
					trap_SendServerCommand(killer, "ksReward 5");
					trap_SendServerCommand(-1, va("send_event\"%s ^2has a 5 kill streak!\"", attacker->client->pers.netname));
					break;
				case 10:
//					G_Event(g_entities, EVENT_STREAK, va("%s has a 10 kill streak!", attacker->client->pers.netname));
					trap_SendServerCommand(-1, va("send_event\"%s ^2has a 10 kill streak!\"", attacker->client->pers.netname));
					break;
				case 20:
//					G_Event(g_entities, EVENT_STREAK, va("%s has a 20 kill streak!", attacker->client->pers.netname));
					trap_SendServerCommand(-1, va("send_event\"%s ^2has a 20 kill streak!\"", attacker->client->pers.netname));
					break;
			}*/
			int i = 0;
			for( ; i < numKS; i++ )
			{
				if(attacker->client->killStreak == killstreakData[i].kill)
				{
					trap_SendServerCommand(killer, va("ksReward %i", attacker->client->killStreak));
					attacker->client->ps.stats[STAT_HOLDABLE_ITEMS] |= ( 1 << killstreakData[i].reward );
					attacker->client->ps.stats[STAT_HOLDABLE_ITEM] = killstreakData[i].reward;
					trap_SendServerCommand(-1, va("send_event\"%s ^2has a %i kill streak!\"", attacker->client->pers.netname, attacker->client->killStreak));
				}
			}
		}
	}
	if(self->client)
	{
		self->client->savedkillstreaks = self->client->ps.stats[STAT_HOLDABLE_ITEMS];
	}
	...
	*/
	...
	if ( !( contents & CONTENTS_NODROP ) && !self->client->ps.fallingToDeath) {
		...
		{
#ifdef AKIMBO
			TossClientItems( self, qtrue, qfalse );
#else
	...
	*/
	...
	self->client->ps.stats[STAT_HOLDABLE_ITEMS] &= ~(1 << HI_EWEB);
	if(rewardVariationForReward(HI_BINOCULARS) == 5)
	{
		self->client->ps.stats[STAT_HOLDABLE_ITEMS] &= ~(1 << HI_BINOCULARS);
	}
...
void G_ApplyKnockback( gentity_t *targ, vec3_t newDir, float knockback )
{
	...
	if ( g_gravity.value > 0 )
	{
		if(g_instagib.integer == 1){
			VectorScale( newDir, g_knockback.value * (float)knockback / mass * 0.01, kvel );
			kvel[2] = newDir[2] * g_knockback.value * (float)knockback / mass * 0.03;
		}
		else{
		VectorScale( newDir, g_knockback.value * (float)knockback / mass * 0.1, kvel );
		kvel[2] = newDir[2] * g_knockback.value * (float)knockback / mass * 0.3;
		}
	}
	else
	{
...
void LimbThink( gentity_t *ent )
{
	...
	if (ent->speed < level.time || (level.time - ent->timestamp) > g_corpseRemovalTime.integer*1000 + 2500)
	{
...
void G_Dismember( gentity_t *ent, gentity_t *enemy, vec3_t point, int limbType, float limbRollBase, float limbPitchBase, int deathAnim, qboolean postDeath )
{
	...
	*/
	...
	limb->speed = level.time + Q_irand(280000, 300000);
	...
	VectorMA( vel, 5, dir, limb->epVelocity );
	...
	if (enemy && enemy->client && ent && ent != enemy && ent->s.number != enemy->s.number &&
		...
		(level.time - enemy->client->lastSaberStorageTime) < 200/* 1*/)
	{ //The enemy has valid saber positions between this and last frame. Use them to factor in direction of the limb.
		...
		{ //if he's done with his death anim we don't actually want the limbs going far
			...
			{
				float mAmt = 20.0f/totalDistance;
...
extern void knockdownFixme( int num );
//[/CoOp]
void G_Knockdown( gentity_t *victim, gentity_t *attacker, const vec3_t pushDir, float strength )
{
	//[CoOp]
...
int G_LocationDamage(vec3_t point, gentity_t* targ, gentity_t* attacker, int take) {
	...
	//[WeaponBasedLDMG]
	weaponData_t	weapon;
	float HeadMod, FaceMod, ShoulderMod, ChestMod, StomachMod, GroinMod, LegMod, FootMod;
	float DefaultMods[8];
	int i;
	//[/""]
	...
	if(attacker->client){
		weapon = weaponData[attacker->client->ps.weapon];
		if(attacker->previousAttack == 1){
			HeadMod = weapon.Mods[0];
			FaceMod = weapon.Mods[1];
			ShoulderMod = weapon.Mods[2];
			ChestMod = weapon.Mods[3];
			StomachMod = weapon.Mods[4];
			GroinMod = weapon.Mods[5];
			LegMod = weapon.Mods[6];
			FootMod = weapon.Mods[7];
		} else if(attacker->previousAttack == 2){
			HeadMod = weapon.AltMods[0];
			FaceMod = weapon.AltMods[1];
			ShoulderMod = weapon.AltMods[2];
			ChestMod = weapon.AltMods[3];
			StomachMod = weapon.AltMods[4];
			GroinMod = weapon.AltMods[5];
			LegMod = weapon.AltMods[6];
			FootMod = weapon.AltMods[7];
		}
		else{
			HeadMod = DefaultMods[0];
			FaceMod = DefaultMods[1];
			ShoulderMod = DefaultMods[2];
			ChestMod = DefaultMods[3];
			StomachMod = DefaultMods[4];
			GroinMod = DefaultMods[5];
			LegMod = DefaultMods[6];
			FootMod = DefaultMods[7];
		}
	} else{
			HeadMod = DefaultMods[0];
			FaceMod = DefaultMods[1];
	...
	// The upper body never changes height, just distance from the feet
		...
		{
			case LOCATION_HEAD:
				takeModifier += HeadMod;
				break;
			case LOCATION_FACE: //head shots kill on first shot always, eezstreet
				takeModifier += FaceMod; // Faceshots REALLY suck
				break;
			case LOCATION_SHOULDER:
				if (targ->client->lasthurt_location & (LOCATION_FRONT | LOCATION_BACK))
					takeModifier += ShoulderMod; // Throat or nape of neck
				else
					takeModifier += ShoulderMod; // Shoulders
				break;
			case LOCATION_CHEST:
				if (targ->client->lasthurt_location & (LOCATION_FRONT | LOCATION_BACK))
					takeModifier += ChestMod; // Belly or back
				else
					takeModifier += 0.8; // Arms
				break;
			case LOCATION_STOMACH:
				takeModifier += StomachMod;
				break;
			case LOCATION_GROIN:
				if (targ->client->lasthurt_location & LOCATION_FRONT)
					takeModifier += GroinMod; // Groin shot
				break;
			case LOCATION_LEG:
				takeModifier += LegMod;
...
void G_Damage( gentity_t *targ, gentity_t *inflictor, gentity_t *attacker,
	...
	G_CheckForDoorDamage( targ, attacker, damage, mod );
	if(attacker != NULL && attacker->s.number != ENTITYNUM_WORLD && inflictor != NULL && inflictor->s.number != ENTITYNUM_WORLD){
		if(mod == MOD_FALLING || mod == MOD_SUICIDE || mod == MOD_TRIGGER_HURT){
			if(attacker->s.number == targ->s.number ||
				inflictor->s.number == targ->s.number){
					//We can't poof if we're suiciding or falling on our own accord
			}
			else if(!attacker->alreadyPoofed){
				//Only poof one time if we die from falling.
				trap_SendServerCommand(attacker->s.clientNum, "poof");
				attacker->alreadyPoofed = qtrue;
			}
			else{
	...
	//eezstreet fixme: If the person in question dies via falling and we've been injured within the past 7 (yeah really arbitrary I know) seconds, last damager was at fault
	if(mod == MOD_FALLING)
	{
		if(targ->client)
		{
			if((level.time - targ->client->lasthurt_time) <= 7000)
			{
				//Hurt within the last five seconds...blame our last attacker!
				if(targ->client->lasthurt_client < MAX_CLIENTS)
				{
					attacker = &g_entities[targ->client->lasthurt_client];
				}
			}
		}
	}
	...
	*/
	...
	//Inflict knockdown, if necessary
	if( mod == MOD_STUN)
	{
		G_Knockdown( targ, attacker, attacker->s.origin, (damage*20));
	}
	...
	if(targ->client && modData[mod].bloodSpray)
	{
		//Sprays blood all over nearby clients' screens
		int size = MAX_CLIENTS;
		int i;
		int flashedTargets[MAX_CLIENTS];
		int direction = 0;
		fb_targetsInRadius(&size, flashedTargets, targ->s.origin, modData[mod].bloodSplashRadius /*512*/);
		for(i = 0; i < size; i++)
		{
			if(flashedTargets[i] != targ->s.number) //Don't want ourselves to get splashed on - the directional damage code should handle that
			{
				/*vec3_t difference, angles;
				float yawAngle;
				VectorSubtract(targ->s.origin, g_entities[flashedTargets[i]].client->ps.origin, difference);
				VectorNormalize(difference);
				vectoangles(difference, angles);
				difference[YAW] = AngleSubtract(g_entities[flashedTargets[i]].client->ps.viewangles[YAW], angles[YAW]);

				yawAngle = angles[YAW];

				if(yawAngle < -1.0f && yawAngle > -2.0f){
					Com_Printf("Blood Angles: FRONT\n");
					direction = 0;
				}

				Com_Printf("Blood Angles: %f\n", yawAngle);*/
				trap_SendServerCommand(flashedTargets[i], va("bldspy %i", mod));
			}
		}
	}
	...
	if (targ->client) {
		if(attacker->client && attacker->s.number != targ->s.number && attacker - g_entities != ENTITYNUM_WORLD)
			targ->client->lasthurt_time = level.time;
		if(ERNG(mod, MOD_STUN_BATON, MOD_FORCE_DARK) || mod >= MOD_GLOCK){
			// set the last client who damaged the target
			targ->client->lasthurt_client = attacker->s.number; //eezstreet headshot code:
				...
				case LOCATION_FACE:
					/*switch(mod){
						...
						case MOD_BRYAR_PISTOL_ALT:
							mod = MOD_BRYAR_PISTOL_HEAD;
							break;
						case MOD_BLASTER:
							mod = MOD_BLASTER_HEAD;
							break;
						case MOD_DISRUPTOR:
						case MOD_DISRUPTOR_SPLASH:
						case MOD_DISRUPTOR_SNIPER:
							mod = MOD_DISRUPTOR_HEAD;
							break;
						case MOD_BOWCASTER:
							mod = MOD_BOWCASTER_HEAD;
							break;
						case MOD_REPEATER:
							mod = MOD_REPEATER_HEAD;
							break;
						case MOD_DEMP2:
						case MOD_DEMP2_ALT:
							mod = MOD_DEMP2_HEAD;
							break;
						case MOD_FLECHETTE:
							mod = MOD_FLECHETTE_HEAD;
							break;
					}*/
					if(modData[mod].headShot < 0)
					{
						unsigned int oldMod = mod;
						mod = (int)modData[oldMod].headShotLine;
					}
					break;
			}
		}
	...
	*/
	...
	if(attacker->s.eType == ET_PLAYER && targ->client){ //Calculate endgame data, wolololo
		short damages[8];
		short percentages[8];
		int totalDamages = 0, i;
		switch(targ->client->lasthurt_location & ~(LOCATION_BACK | LOCATION_LEFT | LOCATION_RIGHT | LOCATION_FRONT))
		{
			case LOCATION_HEAD:
				attacker->client->endGameStats.headHits[HITINFO_HITS]++;
				attacker->client->endGameStats.headHits[HITINFO_DAMAGE] += (short)take;
				break;
			case LOCATION_FACE:
				attacker->client->endGameStats.faceHits[HITINFO_HITS]++;
				attacker->client->endGameStats.faceHits[HITINFO_DAMAGE] += (short)take;
				break;
			case LOCATION_SHOULDER:
				attacker->client->endGameStats.shoulderHits[HITINFO_HITS]++;
				attacker->client->endGameStats.shoulderHits[HITINFO_DAMAGE] += (short)take;
				break;
			case LOCATION_CHEST:
				attacker->client->endGameStats.chestHits[HITINFO_HITS]++;
				attacker->client->endGameStats.chestHits[HITINFO_DAMAGE] += (short)take;
				break;
			case LOCATION_STOMACH:
				attacker->client->endGameStats.stomachHits[HITINFO_HITS]++;
				attacker->client->endGameStats.stomachHits[HITINFO_DAMAGE] += (short)take;
				break;
			case LOCATION_GROIN:
				attacker->client->endGameStats.groinHits[HITINFO_HITS]++;
				attacker->client->endGameStats.groinHits[HITINFO_DAMAGE] += (short)take;
				break;
			case LOCATION_LEG:
				attacker->client->endGameStats.legHits[HITINFO_HITS]++;
				attacker->client->endGameStats.legHits[HITINFO_DAMAGE] += (short)take;
				break;
			case LOCATION_FOOT:
				attacker->client->endGameStats.footHits[HITINFO_HITS]++;
				attacker->client->endGameStats.footHits[HITINFO_DAMAGE] += (short)take;
				break;
		}
		damages[0] = attacker->client->endGameStats.headHits[HITINFO_DAMAGE];
		damages[1] = attacker->client->endGameStats.faceHits[HITINFO_DAMAGE];
		damages[2] = attacker->client->endGameStats.shoulderHits[HITINFO_DAMAGE];
		damages[3] = attacker->client->endGameStats.chestHits[HITINFO_DAMAGE];
		damages[4] = attacker->client->endGameStats.stomachHits[HITINFO_DAMAGE];
		damages[5] = attacker->client->endGameStats.groinHits[HITINFO_DAMAGE];
		damages[6] = attacker->client->endGameStats.legHits[HITINFO_DAMAGE];
		damages[7] = attacker->client->endGameStats.footHits[HITINFO_DAMAGE];

		for(i = 0; i < 8; i++)
			totalDamages += damages[i];
		for(i = 0; i < 8; i++){
			percentages[i] = (((float)damages[i] / (float)totalDamages) * 100);
		}

		attacker->client->endGameStats.headHits[HITINFO_PERCENT] = percentages[0];
		attacker->client->endGameStats.faceHits[HITINFO_PERCENT] = percentages[1];
		attacker->client->endGameStats.shoulderHits[HITINFO_PERCENT] = percentages[2];
		attacker->client->endGameStats.chestHits[HITINFO_PERCENT] = percentages[3];
		attacker->client->endGameStats.stomachHits[HITINFO_PERCENT] = percentages[4];
		attacker->client->endGameStats.groinHits[HITINFO_PERCENT] = percentages[5];
		attacker->client->endGameStats.legHits[HITINFO_PERCENT] = percentages[6];
		attacker->client->endGameStats.footHits[HITINFO_PERCENT] = percentages[7];
	}
	if(targ->s.eType == ET_PLAYER && targ->s.number != attacker->s.number){
		if(targ->lastAttacker == attacker->s.number){
			return; //Don't double stack assists.
		}
		if(targ->lastAttacker == targ->s.number){
			return; //Don't assist in our own killing.
		}
		if(targ->lastAttacker >= 0 && targ->lastAttacker <= MAX_CLIENTS){
			targ->previousAttacker = targ->lastAttacker;
		}
		targ->lastAttacker = attacker->s.number;
	}
...
qboolean G_RadiusDamage ( vec3_t origin, gentity_t *attacker, float damage, float radius,
	...
	*/
	...
	for ( e = 0 ; e < numListedEntities ; e++ ) {
		...
		if( CanDamage (ent, origin) && damage > 0.0f) {
