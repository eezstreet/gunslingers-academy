...
//eezstreet add - Rolling
qboolean TAB_BotCanRoll(bot_state_t *bs)
{
	if(weaponData[bs->virtualWeapon].canRoll && level.clients[bs->client].ps.stats[STAT_STAMINA] > 200)
	{
		return qtrue;
	}
	return qfalse;
...
void TAB_BotRoll(bot_state_t *bs, int rollType)
{
	if(!TAB_BotCanRoll(bs))
	{ //Can't roll so don't bother
		return;
	}
	if(rollType == -1)
	{
		//No roll direction specified...let's assume forward.
		rollType = 0;
	}
	...
	{//found a pressable/hackable button
		...
		//if(tr.entityNum == test->s.number || tr.fraction == 1.0)
		{
			...
			{//inside trigger zone, press use.
				bs->useTime = level.time + test->genericValue7 + 500;
				bs->DontSpamButton = bs->useTime + 15000;
			}
			else
			{//need to move closer
...
int FindWeaponforRange(bot_state_t *bs, float range)
{
	...
	for(i=0;i < WP_NUM_WEAPONS; i++)
	{
		if ((bs->cur_ps.ammo[weaponData[i].ammoIndex] >= weaponData[i].energyPerShot ||
			bs->cur_ps.ammo[weaponData[i].ammoIndex] >= weaponData[i].altEnergyPerShot)
...
void TAB_ScanforEnemies(bot_state_t *bs)
{
	...
	for(i = 0; i <= level.num_entities; i++)
	{
		if (OJP_PassStandardEnemyChecks(bs, &g_entities[i]))
		{
			...
			vectoangles(a, a);
			if(g_entities[i].client && g_entities[i].client->sess.sessionTeam != TEAM_SPECTATOR){
				if (g_entities[i].client->ps.isJediMaster)
				{ //make us think the Jedi Master is close so we'll attack him above all
					distcheck = 1;
				}
				//eezstreet add
				//this does a vchat cmd that says "sniper" if the enemy is wielding a sniper rifle
				if(g_entities[bs->client].health < 75)
				{ //I need a doctor!
					RequestSiegeAssistance(bs, REQUEST_MEDIC);
				}
				else if (g_entities[i].client->ps.weapon == WP_DISRUPTOR && g_instagib.integer != 1 && g_entities[i].client->sess.sessionTeam != g_entities[bs->client].client->sess.sessionTeam && g_gametype.integer != GT_TEAM)
				{ //Sniper!
					RequestSiegeAssistance(bs, SPOTTED_SNIPER);
				}
				else if((g_entities[i].client->ps.weapon == WP_REPEATER || g_entities[i].client->ps.weapon == WP_BOWCASTER || g_entities[i].client->ps.weapon == WP_FLECHETTE || g_entities[i].client->ps.weapon == WP_THERMAL) && g_instagib.integer != 1 && (g_entities[bs->client].client->ps.weapon == WP_REPEATER || g_entities[bs->client].client->ps.weapon == WP_BOWCASTER || g_entities[bs->client].client->ps.weapon == WP_FLECHETTE)
					&& ( bs->botOrder == BOTORDER_DEFEND || bs->objectiveType == OT_DEFEND || bs->objectiveType == OT_DEFENDCAPTURE) && g_gametype.integer != GT_TEAM)
				{ //Troops, incoming
					RequestSiegeAssistance(bs, SPOTTED_TROOPS);
				}
				else if((g_entities[i].client->ps.weapon == WP_ROCKET_LAUNCHER || g_entities[i].client->ps.weapon == WP_CONCUSSION) && (g_entities[i].client->sess.sessionTeam != g_entities[bs->client].client->sess.sessionTeam))
				{ //Crap, he has a bigger gun than us.
					RequestSiegeAssistance(bs, REQUEST_ASSISTANCE);
				}
				else if((g_entities[bs->client].client->ps.weapon == WP_BLASTER || g_entities[bs->client].client->ps.weapon == WP_BRYAR_PISTOL || g_entities[bs->client].client->ps.weapon == WP_STUN_BATON || g_entities[bs->client].client->ps.weapon == WP_DEMP2) || bs->objectiveType == OT_CAPTURE)
				...
				else if ((g_entities[i].client->ps.weapon == WP_BLASTER || g_entities[i].client->ps.weapon == WP_BRYAR_PISTOL || g_entities[i].client->ps.weapon == WP_STUN_BATON || g_entities[i].client->ps.weapon == WP_DEMP2)
					&& (bs->botOrder == BOTORDER_SEARCHANDDESTROY || bs->objectiveType == OT_ATTACK))
				...
				else if (g_entities[i].client->ps.weapon == g_entities[bs->client].client->ps.weapon)
...
void RequestSiegeAssistance(bot_state_t *bs, int BaseClass)
{
	...
	{
		//we aren't in a team game
		return;
	}
		
	//	voice_cmd
	// close jamp, delete those dlls, and then try debuggin again
	// as long as you don't edit then save these files while it's running :P
	//ok
	switch(BaseClass)
	{
		...
		case SPC_SUPPORT:
			...
			break;
		case SPOTTED_SNIPER:
			trap_EA_Command(bs->client, "voice_cmd spot_sniper");
			bs->vchatTime = level.time + Q_irand(25000, 45000);
			break;
		case SPOTTED_TROOPS:
			trap_EA_Command(bs->client, "voice_cmd spot_troops");
			bs->vchatTime = level.time + Q_irand(12000, 30000);
			break;
		case REQUEST_SUPPLIES:
			trap_EA_Command(bs->client, "voice_cmd req_sup");
			bs->vchatTime = level.time + Q_irand(60000, 75000);
			break;
		case REQUEST_ASSISTANCE:
			trap_EA_Command(bs->client, "voice_cmd req_assist");
			bs->vchatTime = level.time + Q_irand(15000, 30000);
			break;
		case REQUEST_MEDIC:
			trap_EA_Command(bs->client, "voice_cmd req_medic");
			bs->vchatTime = level.time + Q_irand(5000, 10000);
			break;
		case TACTICAL_COVERME:
			trap_EA_Command(bs->client, "voice_cmd tac_cover");
			bs->vchatTime = level.time + Q_irand(10000, 25000);
			break;
		case TACTICAL_HOLDPOSITION:
			trap_EA_Command(bs->client, "voice_cmd tac_hold");
			bs->vchatTime = level.time + Q_irand(5000, 12000);
			break;
		case TACTICAL_FOLLOW:
			trap_EA_Command(bs->client, "voice_cmd tac_follow");
			bs->vchatTime = level.time + Q_irand(5000, 10000);
			break;
		case REPLY_YES:
			trap_EA_Command(bs->client, "voice_cmd reply_yes");
			bs->vchatTime = level.time + Q_irand(2000, 4000);
			break;
		case REPLY_COMING:
			trap_EA_Command(bs->client, "voice_cmd reply_coming");
			bs->vchatTime = level.time + Q_irand(2000, 4000);
			break;
	};		
}
...
{//has the bot decide what role it should take in a CTF fight
	...
	int NumAwaitingOffense = 0;//eezstreet: waves
	...
	for(i=0; i < MAX_CLIENTS; i++)
	{
		...
		{//this bot is going for/defending the flag
			...
			{//it's on defense
				if(tempbot->secondaryObjectiveType == SOT_WAITFORATTACKWAVE)
					NumAwaitingOffense++;
				NumDefense++;
			}
		}
	...
	{//we have less defenders than attackers.  Go on the defense.
		if(NumAwaitingOffense >= (NumDefense - (NumDefense / 2)))
		{
			bs->objectiveType = OT_CAPTURE;
		}
		else
	...
	{//go on the attack
		...
		{
			bs->secondaryObjectiveType = SOT_WAITFORATTACKWAVE;
		}
	}
}
...
{//This function checks to see if the bot wishes to go for ammo for a particular weapon.
	...
	int favWeapon;
	int ammoMax;
	//We need supplies if this returns true, so ask for supplies...nicely.
	...
	{//keep checking until we run out of fav weapons we want to check.
		...
		if(weaponData[favWeapon].ammoIndex == AMMO_RPG && g_gametype.integer == GT_SIEGE)
		...
		{//we have less ammo than we'd like, check to see if there's ammo for this weapon on
			...
			if(item)
			{
				...
				{//we want to switch our current tactic.
					bs->currentTactic = BOTORDER_RESUPPLY;
					bs->tacticEntity = item;
					RequestSiegeAssistance(bs, REQUEST_SUPPLIES);
				}
				return item;
			}
		}
...
void TAB_StandardBotAI(bot_state_t *bs, float thinktime)
{
	...
	{//disable ai during bot editting or while waiting for respawn in siege.
		...
		bs->objectiveType = 0;
		bs->MiscBotFlags = 0;
		bs->secondaryObjectiveType = SOT_NONE;
		return;
