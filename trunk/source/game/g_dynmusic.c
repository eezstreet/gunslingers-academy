...
void LoadDMSSongData(char *buffer, char *song, DynamicMusicSet_t *songData, char *mapname)
{
	...
	{//still have a transition file to add
		...
		while((numExits > 9) ? BG_SiegeGetPairedValue(transitionGroup, va("time%i", numExits), Value) : BG_SiegeGetPairedValue(transitionGroup, va("time0%i", numExits), Value))
		{
...
void TransitionBetweenState(void)
{
	...
	if((DMSData.olddmState != DM_ACTION) 
		|| !DMSData.dmStartTime)
...
void G_DynamicMusicUpdate( void )
{
	...
	DMSData.dmBeatTime = level.time + 20;//1 second beats

	if(level.time < level.explodeTime)
	{
		goto actionState;
	}
	for(i = 0; i < MAX_CLIENTS; i++)
	{
		...
		//hurt-based
		if(player->client->lasthurt_time + 10000 > level.time && level.time > 5000)
		{
			goto actionState;
		}
		...
		{//no active enemies, but look for missiles, shot impacts, etc...
			...
			{//FIXME: maybe tripwires and other FIXED things need their own sound, some kind of danger/caution theme
				if ( G_CheckForDanger( player, alert ) || grenadeHasGoneOff)
