...
//Deferred Server Commands
//Use these to send a reliable command after X seconds
typedef struct
{
	char messageText[1024];
	int clientsInvolved[MAX_CLIENTS];
	int timeToSend; //level.time + this time = send
} deferredReliableCommand_t;

deferredReliableCommand_t deferredCommands[128];
int numDeferredCommands;

typedef enum
{
	ROCKETFIRE_VEER,		//hard turn
	ROCKETFIRE_DRUNK,		//fly all drunkenlike. Who knows if it'll veer off target?
	ROCKETFIRE_PERFECT		//baseJKA fly type
} rocketTrajectory_t;
...
struct gentity_s {
	...
	int				delayScriptTime;
//	char			*fullName;
	char			*fullName; //hijacked MAX_QPATH, but hey, they're the same! ~eezstreet
	...
	int				firingMode[WP_NUM_WEAPONS];
	int				firingModePos[WP_NUM_WEAPONS]; //position in firing mode array

	//[Assists]
	int			lastAttacker;
	int			previousAttacker;
	//[/Assists]

	//[WeaponBasedLDMG]
	int			previousAttack; //Determine whether our last attack was alt fire or primary fire
	//[/""]

	int			ammoOnItem;		//items -- amount of ammo on a dropped weapon
...
//Think Functions
	...
	int			last_move_time;
	int			doorState;					//Used for new doors -> whether we're closed, opened forward or opened behind
	qboolean	isTrigger1;
	vec3_t		doorAngles;
	int			lastUsedBy;
	int			lockType;
	qboolean	doorBroken;
	qboolean	noDeathSound;

//Double Kills
	int			last_kill_time;
	int			cumulativeKills;
	int			cumulativeDebounce;
...
//Health and damage fields
	...
	qboolean	alreadyPoofed;	//fixes a glitch with poofing
	...
	//NoSee trigger stuff
	int			nosee_contents[MAX_CLIENTS];
	vec3_t		nosee_origins[MAX_CLIENTS];
	int			nosee_num;

	//Stuff for TASER
	qboolean	taser_attach;
	qboolean	taser_signal;
};
...
typedef struct {
	...
	//[LastManStanding]
	int			livesLeft;			// # of lives left
	int			livesUsed;			// # of lives used
	//[/LastManStanding]
	parm_t		primaryWeapon;
	parm_t		nextPrimaryWeapon;
	sarm_t		sideArm;
	sarm_t		nextSideArm;
	earm_t		explosives;
	earm_t		nextExplosives;
...
typedef struct {
	short headHits[HITINFO_MAX];
	short faceHits[HITINFO_MAX];
	short shoulderHits[HITINFO_MAX];
	short chestHits[HITINFO_MAX];
	short stomachHits[HITINFO_MAX];
	short groinHits[HITINFO_MAX];
	short legHits[HITINFO_MAX];
	short footHits[HITINFO_MAX];

	int killMakeUp[KILLMU_MAX];
} ggameStat_t;
...
struct gclient_s {
	...
	int			savedkillstreaks;

	//[Ninja]
	//Tier 1 core ninja abilities
	//Wall walk,
	//Wall bounce,
	//Lunge.
	qboolean	isTier1;
	//Tier 2 core ninja abilities
	//Roll stab
	//Sprint Slice
	//Duck swipe
	qboolean	isTier2;
	//Tier 3 core ninja abilities
	//Knockback Slice
	//Katana Takedown
	//Kata?? (placeholder)
	qboolean	isTier3;
	//Tier 4 core ninja abilities

	//[Reloading]
	int			weaponClips[WP_NUM_WEAPONS];
	//[/Reloading]

	ggameStat_t	endGameStats;
	...
	//[AdminSys]
	int			adminRank;
	qboolean	lockdownProof;
	qboolean	superficialProof;
	qboolean	muted;
	//[/AdminSys]
	//[VIP Escort]
	//[Commander Escort]
//	qboolean				isVIP;
	...
	//Autoreload switch - 2.4.4
	qboolean	noAutoReload;

	//[Timers]
	int			timers[NUM_TIMERS];

	//Ninja mode: restore our old model
	...
	int			damage_lastMOD;		// last Means of Death applied to this client
	...
	//Raz0r start
	int			clipammo[WP_NUM_WEAPONS];
#ifdef AKIMBO
	int			clipammo2[WP_NUM_WEAPONS];
	qboolean	isAkimbo[WP_NUM_WEAPONS];
	int			akimboCounterPart[WP_NUM_WEAPONS];
#endif
	//raz end
...
typedef struct {
	...
	//Control number of dropped weapons
	int			numDroppedGuns;
	...
	int			vipPlayer;				//GT_VIP_ESCORT: VIP
	int			redTeamCommander;		//GT_COMMANDER: Red Team, Commander
	int			blueTeamCommander;		//GT_COMMANDER: Blue Team, Commander

	//int			blueTeamHero;
	//int			redTeamHero;
	int			lastPickTime;

	int			heroPickTime;			//Time accumulated since last hero pick
	int			redHero;
	int			blueHero;

	//Integers handling killstreak rewards
	int			radarTimes[MAX_CLIENTS];

	qboolean	anotherVIP;
	qboolean	anotherRCommander;
	qboolean	anotherBCommander;
...
{
	qboolean gameActive;
	byte players[MAX_CLIENTS];
	int currentChamber;
	int numChambers;
	byte gameOwner;
} russianRoulette_t;

russianRoulette_t rRGame;
...
static void G_EventTo( gentity_t *target, int color, const char *message );
...
extern vmCvar_t g_gunRemoveTime;
...
//typedef enum dynamicMusic_t;
extern void SetDMSState ( int DMSState );
qboolean grenadeHasGoneOff;
...
extern vmCvar_t		g_instagib_restrict;
//[/Instagib]

//[AnnouncementsSys]
extern vmCvar_t	g_amessageDelay;
extern vmCvar_t	g_amessage1;
extern vmCvar_t	g_amessage2;
extern vmCvar_t	g_amessage3;
//[/AnnouncementsSys]

//[RP]
extern vmCvar_t g_barebones;
//[/RP]

//[Lobby]
extern vmCvar_t	g_pregameLobby;
extern vmCvar_t	g_pregameLobbyTime;
extern vmCvar_t	g_pregameLobbyForceWait;
//[/Lobby]
...
//[Respawn Timer]
extern vmCvar_t		g_ffaRespawnTimer;
//[/Respawn Timer]

//[AdminSys]
extern vmCvar_t		g_level1Password;
extern vmCvar_t		g_level2Password;
extern vmCvar_t		g_level3Password;
extern vmCvar_t		g_level4Password;
extern vmCvar_t		g_level5Password;

extern vmCvar_t		g_superficial;
...
//[LastManStanding]
extern vmCvar_t		g_LMS;
extern vmCvar_t		g_LMSlives;
//[/LastManStanding]

//[ServerRules]
extern vmCvar_t		g_serverRules;
//[/ServerRules]
...
extern	vmCvar_t	g_specInactive;
...
//[Stamina]
extern	vmCvar_t	g_staminaMax;
extern	vmCvar_t	g_staminaRegenRate;
...
#ifdef AKIMBO
extern void WP_SetAkimboWeapon( gentity_t *ent, int newWeapon, int weaponIndex);
#endif
...
unsigned int killstreakRewards[MAX_CLIENTS];

void EvaluateMiniRewards(gentity_t *self, gentity_t *attacker, int mod);

unsigned int primaryWeaponList[MAX_WEAPONS];
unsigned int sidearmList[MAX_WEAPONS];
unsigned int explosiveList[MAX_WEAPONS];
unsigned int numPrimaryWeapons;
unsigned int numSidearms;
unsigned int numExplosives;
