...
void WP_SaberStartMissileBlockCheck( gentity_t *self, usercmd_t *ucmd  )
{
	...
	/*for(i = 0; i < FP_MAX; i++){
		if( self->client->ps.fd.pd[i]->buttonPressType == HOLD_HOLD && (self->client->ps.fd.forcePowersActive & (1 << i))){
			doFullRoutine = qfalse;
		}
		else if(i == FP_REFLECT_BULLETS && self->client->ps.fd.pd[i]->startingRank < 1){
			doFullRoutine = qfalse;
		}
	}*/
		...
		{ //within range
			...
			{ //Slice them
				...
				{ //a good hit
					...
					{ //8x damage for the Jedi Master
						G_Damage(ent, saberOwner, saberOwner, dir, tr.endpos, saberent->damage*8, dflags, MOD_SABER);
					}
					else
					{
	...
	if ( trace.fraction < 1.0f && !trace.startsolid && !trace.allsolid )
	{
		...
		{//we hit an entity
			...
			{//hurt it
				...
				G_Damage( hitEnt, ent, ent, kickDir, trace.endpos, weaponData[WP_MELEE].altDamage, DAMAGE_NO_KNOCKBACK, MOD_MELEE );
			}
			if ( hitEnt->client 
