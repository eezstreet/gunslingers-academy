...
qboolean WP_SaberParseParms( const char *SaberName, saberInfo_t *saber ) 
{
	...
	//Establish defaults.
	strcpy(saber->shortName, "Melee Weapon");
	strcpy(saber->iconPath, "gfx/hud/w_icon_lightsaber");
...
//===BLADE-SPECIFIC FIELDS=================================================================================
		...
		if( !Q_stricmp( token, "shortName" ) )
		{
			...
			SkipRestOfLine( &p );
#elif defined CGAME
			strcpy(saber->shortName, value);
			SkipRestOfLine( &p );
#endif
			continue;

		}
		if( !Q_stricmp( token, "iconPath" ) )
		{
			if( COM_ParseString( &p, &value ) )
				continue;
			...
			SkipRestOfLine( &p );
#elif defined CGAME
			if(!trap_R_RegisterShader(value))
				strcpy(saber->iconPath, "gfx/hud/w_icon_lightsaber");
			else
				strcpy(saber->iconPath, value);
			SkipRestOfLine( &p );
#endif
			continue;
		}
