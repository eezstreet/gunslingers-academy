...
qboolean Asset_Parse(int handle) {
	...
	while ( 1 ) {
		...
		if (Q_stricmp(token.string, "digitalFont") == 0) {
			int pointSize;
			if(!trap_PC_ReadToken(handle, &token) || !PC_Int_Parse(handle, &pointSize)) {
				return qfalse;
			}
			uiInfo.uiDC.Assets.qhDigitalFont = trap_R_RegisterFont(token.string);
			continue;
		}
...
static const char* UI_GetGameTypeName(int gtEnum)
{
	...
	case GT_HERO:
		return UI_GetStringEdString("MENUS", "HERO_TFFA");//"Team FFA";
	case GT_SIEGE:
	...
	case GT_VIP_ESCORT:
		return UI_GetStringEdString("MENUS", "VIP_ESCORT");
	}
	return UI_GetStringEdString("MENUS", "SAGA");//"Team FFA";
}
...
extern void GSA_BuildWeaponsArrays();
static void UI_RunMenuScript(char **args) 
{
	...
	if (String_Parse(args, &name)) 
	{
		...
		//	trap_Cvar_Set("ui_singlePlayerActive", "0");
			...
			//[OLDGAMETYPES]
			trap_Cvar_SetValue( "g_gametype", Com_Clamp( GT_FFA, GT_HERO, uiInfo.gameTypes[ui_netGameType.integer].gtEnum ) );
			...
			UI_ParseGameInfo("ui/jamp/gameinfo.txt");
			UI_LoadBestScores(uiInfo.mapList[ui_currentMap.integer].mapLoadName, uiInfo.gameTypes[ui_gameType.integer].gtEnum);
			GSA_LoadWeaponTXT("ext_data/weapons", Info_ValueForKey( info, "mapname" ), weaponData);
		...
		} else if (Q_stricmp(name, "RefreshLoadoutIcons") == 0){
			return;
...
#ifdef ACHIEVEMENTS
		else if (Q_stricmp(name, "loadachievefeeders") == 0)
		{
			GSA_UI_LoadAchievementFile("acx", "sav_data");
			GSA_UI_SortIntoFeeders();
		}
#endif
		else if (Q_stricmp(name, "loadoutcrap") == 0)
		{
			GSA_UI_UpdateLoadout();
		}
		else if (Q_stricmp(name, "loadoutinitcrap") == 0)
		{
			GSA_UI_SetInitialLoadout();
		}
//[Asteroids]
#ifdef _XBOX
...
static int UI_MapCountByGameType(qboolean singlePlayer) {
	...
	*/
	...
	//Likewise, Hero TFFA uses the same maps as TFFA
	else if(game == GT_HERO){
		game = GT_TEAM;
	}
...
static int UI_FeederCount(float feederID) 
{
	...
	switch ( (int)feederID )
	{
		...
		case FEEDER_PRIMARY:
		case FEEDER_SIDEARM:
		case FEEDER_EQUIPMENT:
		case FEEDER_OPERATIONS:
		case FEEDER_FEATEARNED:
			return GSA_UI_NumberInAchieveFeeder((int)feederID);
#endif
		case FEEDER_LO_PRIMARY:
			return numPrimary;
		case FEEDER_LO_SIDEARM:
			return numSidearms;
		case FEEDER_LO_EXPLOSIVE:
...
#ifdef ACHIEVEMENTS
	else if (feederID >= FEEDER_FEATS && feederID <= FEEDER_FEATEARNED)
	{
		int feederINT = (int)feederID;
		return achievementFeeders[feederINT-FEEDER_FEATS][index];
	}
#endif
	else if(feederID >= FEEDER_LO_PRIMARY && feederID <= FEEDER_LO_EXPLOSIVE)
	{
		int feederINT = (int)feederID;
		switch(feederINT)
		{
			case FEEDER_LO_PRIMARY:
				return primaryGuns[index];
				break;
			case FEEDER_LO_SIDEARM:
				return sidearmGuns[index];
				break;
			case FEEDER_LO_EXPLOSIVE:
				return explosiveGuns[index];
				break;
		}
	}
#ifdef _XBOX
	...
	while (i < WP_NUM_WEAPONS)
	{
		...
		if (scl->weapons & (1<<i))
		{
			...
			else
			{
				...
				GSA_BuildWeaponsArrays();
				//[SIEGECVARFIX]
				siege_Cvar_Set(va("ui_class_weapon%i", count), weaponData[i].iconPath);
...
#ifdef ACHIEVEMENTS
extern int retrieveAchieveIndexByName(const char *comparison);
void GSA_UI_AchieveFeederAction(int index, int feederID);
#endif
extern void GSA_UI_LoadoutFeederAction(int index, int feederID);
...
//		int validCnt = 0;
	...
	{
		GSA_UI_AchieveFeederAction(index, feederID);
	}
#endif
	else if(feederID >= FEEDER_LO_PRIMARY && feederID <= FEEDER_LO_EXPLOSIVE)
	{
		GSA_UI_LoadoutFeederAction(index, feederID);
	}
#ifdef _XBOX
	else if (feederID == FEEDER_XBL_ACCOUNTS)
	{
...
static qboolean GameType_Parse(char **p, qboolean join) {
	...
	while ( 1 ) {
		...
		if (token[0] == '{') {
			...
			//uiInfo.numGameTypes = GT_MAX_GAME_TYPE-1;
			//uiInfo.numJoinGameTypes = GT_MAX_GAME_TYPE;
...
void UI_DrawConnectScreen( qboolean overlay ) {
	...
	if (!Q_stricmp(cstate.servername,"localhost")) {
		...
		menu = Menus_FindByName("SoloConnect");
	} else {
	...
	case CA_CONNECTING:
		{
			...
			menu = Menus_FindByName("Connect0");
		}
		break;
	case CA_CHALLENGING:
		{
			...
			menu = Menus_FindByName("Connect1");
		}
		break;
	case CA_CONNECTED: {
		char downloadName[MAX_INFO_VALUE];
			...
			if (*downloadName) {
				menu = Menus_FindByName("Connect3");
		...
		menu = Menus_FindByName("Connect2");
		break;
	case CA_LOADING:
		return;
	case CA_PRIMED:
...
//eezstreet: New loadout menu
vmCvar_t	ui_primaryGun;
vmCvar_t	ui_sidearm;
vmCvar_t	ui_explosives;
//eezstreet end
...
//[ColorLoadout]
vmCvar_t	ui_loadout1;
vmCvar_t	ui_loadout2;
vmCvar_t	ui_loadout3;
//[/ColorLoadout]
...
//OVERLORD UI CVARS
vmCvar_t	ui_breakingNews;
//OVERLORD UI CVARS

//Achievements
#ifdef ACHIEVEMENTS
vmCvar_t	ui_currentAchieve;
vmCvar_t	ui_currentAchieveDesc;
vmCvar_t	ui_currentAchieveProgress;
#endif
//EndAchieves
...
static cvarTable_t		cvarTable[] = {
	...
	//eezstreet add
	{ &ui_primaryGun,			"ui_primaryGun",		"0",CVAR_ROM|CVAR_INTERNAL},
	{ &ui_sidearm,				"ui_sidearm",			"0",CVAR_ROM|CVAR_INTERNAL},
	{ &ui_explosives,			"ui_explosives",		"0",CVAR_ROM|CVAR_INTERNAL},
	//eezstreet end
	...
	//[ColorLoadout]
	{ &ui_loadout1, "ui_loadout1", "1", CVAR_ARCHIVE|CVAR_INTERNAL },
	{ &ui_loadout2, "ui_loadout2", "1", CVAR_ARCHIVE|CVAR_INTERNAL },
	{ &ui_loadout3, "ui_loadout3", "1", CVAR_ARCHIVE|CVAR_INTERNAL },
	//[/ColorLoadout]
...
//[/RGBSabers]
	...
	{ &ui_breakingNews,		"ui_breakingNews", "-1", CVAR_INTERNAL },

	//Achieves
#ifdef ACHIEVEMENTS
	{ &ui_currentAchieve,	"ui_currentAchieve", "", CVAR_INTERNAL},
	{ &ui_currentAchieveDesc,"ui_currentAchieveDesc", "", CVAR_INTERNAL},
	{ &ui_currentAchieveProgress, "ui_currentAchieveProgress", "", CVAR_INTERNAL},
#endif
	//End Achieves
};
