	...
	for( n = 0; n < ui_numArenas; n++ ) {
		...
		if( *type ) {
			...
			if( strstr( type, "team" ) ) {
				...
				uiInfo.mapList[uiInfo.mapCount].typeBits |= (1 << GT_HERO);
			}
			//[/Asteroids]
			//[CoOp]
			if( strstr( type, "domination" ) ) {
				uiInfo.mapList[uiInfo.mapCount].typeBits |= (1 << GT_DOMINATION);
			}
			if( strstr( type, "vip" ) ) {
				uiInfo.mapList[uiInfo.mapCount].typeBits |= (1 << GT_VIP_ESCORT);
			}
			if( strstr( type, "commander" ) ) {
				uiInfo.mapList[uiInfo.mapCount].typeBits |= (1 << GT_COMMANDER);
			}
			if( strstr( type, "coop" ) ) {
