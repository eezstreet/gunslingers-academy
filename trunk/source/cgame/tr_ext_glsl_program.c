...
static void R_EXT_GLSL_OutputInfoLog ( int objectID )
{
    ...
    if ( logLength > 0 )
    {
        trap_TrueMalloc (&logText, logLength);
...
static glslShader_t* R_EXT_GLSL_CreateShader ( const char* shaderPath, int shaderType )
{
    ...
    trap_TrueMalloc( &shaderCode, fileLength + 1 );
    if ( shaderCode == NULL )
    {
    ...
    if ( !shader->id )
    {
        ...
        trap_TrueFree( &shaderCode );
        
        return NULL;
    }
    
    glShaderSourceARB (shader->id, 1, &shaderCode, NULL);
    if ( glGetError() == GL_INVALID_OPERATION )
    {
        CG_Printf ("Invalid source code in shader file %s.\n", shaderPath);
    ...
    {
        CG_Printf ("Failed to compile shader source for shader file %s\n", shaderPath);
        R_EXT_GLSL_OutputInfoLog (shader->id);
...
static glslProgramVariable_t* R_EXT_GLSL_GetUniformLocation ( glslProgram_t* program, const char* name )
{
	...
	if ( variable == NULL )
	{
		if ( prev == NULL )
		{
			trap_TrueMalloc (&program->uniforms, sizeof (glslProgramVariable_t));
			variable = program->uniforms;
		}
		else
		{
			trap_TrueMalloc (&prev->next, sizeof (glslProgramVariable_t));
			variable = prev->next;
		}
...
static glslProgramVariable_t* R_EXT_GLSL_GetAttributeLocation ( glslProgram_t* program, const char* name )
{
    ...
    if ( variable == NULL )
    {
        trap_TrueMalloc (&variable, sizeof (glslProgramVariable_t));
...
void R_EXT_GLSL_SetUniformMatrix ( glslProgram_t *program, const char *name, float *matrix, int size )
{
	glslProgramVariable_t *variable = R_EXT_GLSL_GetUniformLocation (program, name);
	glUniformMatrix2fvARB(variable->location, size, 0, matrix);
}