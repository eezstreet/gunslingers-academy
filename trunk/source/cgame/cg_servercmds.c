...
/*
=================
CG_UseRadarKillstreak

Use radar item
=================
*/
void CG_UseRadarKillstreak(void)
{
	int variationID = atoi(CG_Argv(1));
	int clientNum = atoi(CG_Argv(2));

	switch(variationID)
	{
		case RADAR_RECON:
			CG_EVBox_AddString(va("^7Recon Team called in by %s for 30 seconds", cgs.clientinfo[clientNum].name));
			cg.reconTime = cg.time + 30000;
			CG_AddBufferedSound(trap_S_RegisterSound(va("sound/chars/killstreak/recon%i.wav", Q_irand(1,4))));
#ifdef ACHIEVEMENTS
			GSA_IncrementAchievement(&cgs.achievements[A_RECONNAISSANCE], 1);
#endif
			break;
		case RADAR_RADAR:
			CG_EVBox_AddString(va("^7UAV Radar called in by %s for 30 seconds", cgs.clientinfo[clientNum].name));
			cg.uavTime = cg.time + 30000;
			break;
		case RADAR_COMM_TAP:
			CG_EVBox_AddString(va("^7Comm Tap in place by %s for 1 minute", cgs.clientinfo[clientNum].name));
			cg.commTapTime = cg.time + 60000;
			break;
		case RADAR_COUNTER_UAV:
			cg.counterUAVTime = cg.time + 60000;
		case RADAR_JAMMER:
			cg.radarJammerTime = cg.time + 30000;
			break;
	}
}
/*
...
Plays effect upon awarding of a killstreak.
=================
*/
void CG_KillstreakReward( void )
{
	int killNumber = atoi(CG_Argv(1));
	int i = 0;

	CG_ChatBox_AddString(va("^5%i kill streak!", killNumber));
	for( ; i < numKS; i++ )
	{
		if(killNumber == killstreakData[i].kill)
		{
			CG_ChatBox_AddString(va("^5Reward: %s", killstreakData[i].displayText));
			CG_EVBox_AddString(va("^5Reward: %s", killstreakData[i].displayText));
			CG_AddBufferedSound(trap_S_RegisterSound(killstreakData[i].soundPath));
		}
	}
...
Uses the origin of a flashbang as reference
and draws the alpha of the flashbang effect
accordingly.
=================
*/
void CG_Flashbang(vec3_t origin){
	if(cg.snap->ps.origin[0] <= (origin[0] - 4096) || cg.snap->ps.origin[0] >= (origin[0] + 4096) ||
		cg.snap->ps.origin[1] <= (origin[1] - 4096) || cg.snap->ps.origin[1] >= (origin[1] + 4096) ||
		cg.snap->ps.origin[2] <= (origin[2] - 4096) || cg.snap->ps.origin[2] >= (origin[2] + 4096))
		return;
	else{
		if(cg.snap->ps.origin[0] <= (origin[0] - 2048) || cg.snap->ps.origin[0] >= (origin[0] + 2048) ||
		cg.snap->ps.origin[1] <= (origin[1] - 2048) || cg.snap->ps.origin[1] >= (origin[1] + 2048) ||
		cg.snap->ps.origin[2] <= (origin[2] - 2048) || cg.snap->ps.origin[2] >= (origin[2] + 2048)){
			cg.flashTime = (cg.time + 5000);
		}
		else{
			if(cg.snap->ps.origin[0] <= (origin[0] - 1024) || cg.snap->ps.origin[0] >= (origin[0] + 1024) ||
				cg.snap->ps.origin[1] <= (origin[1] - 1024) || cg.snap->ps.origin[1] >= (origin[1] + 1024) ||
				cg.snap->ps.origin[2] <= (origin[2] - 1024) || cg.snap->ps.origin[2] >= (origin[2] + 1024)){
					cg.flashTime = (cg.time + 7000);
			}
			else
			{
				if(cg.snap->ps.origin[0] <= (origin[0] - 512) || cg.snap->ps.origin[0] >= (origin[0] + 512) ||
					cg.snap->ps.origin[1] <= (origin[1] - 512) || cg.snap->ps.origin[1] >= (origin[1] + 512) ||
					cg.snap->ps.origin[2] <= (origin[2] - 2048) || cg.snap->ps.origin[2] >= (origin[2] + 512)){
						cg.flashTime = (cg.time + 8500);
				}
				else
				{
					if(cg.snap->ps.origin[0] <= (origin[0] - 256) || cg.snap->ps.origin[0] >= (origin[0] + 256) ||
						cg.snap->ps.origin[1] <= (origin[1] - 256) || cg.snap->ps.origin[1] >= (origin[1] + 256) ||
						cg.snap->ps.origin[2] <= (origin[2] - 256) || cg.snap->ps.origin[2] >= (origin[2] + 256)){
							cg.flashTime = (cg.time + 12500);
					}
					else
					{
						cg.flashTime = (cg.time + 14000);
					}
				}
			}
		}
...
When called, plays the "Reinforcements have arrived" effect.
=================
*/
void CG_Reinforcements (void){
	CG_EVBox_AddString("^6Reinforcements have arrived.");
	trap_S_StartLocalSound(cgs.media.updateSound, CHAN_ANNOUNCER);
}
...
Plays the "The Force..is with you" voice
=================
*/
void CG_ForceIsWithYou(void)
{
	if(atoi(CG_Argv(1)))
	{
		CG_AddBufferedSound(trap_S_RegisterSound("sound/chars/protocol/misc/40MOM051.wav"));
	}
	else
	{
		CG_AddBufferedSound(trap_S_RegisterSound("sound/chars/protocol/misc/40MOM052.wav"));
	}
}
...
=================
*/
static void CG_ParseLobby( void ) {
	int i, readScores;

	cg.pregameLobbyNum =	atoi( CG_Argv( 1 ) );
	cg.pregameLobbyStart =	atoi( CG_Argv( 2 ) );
	cg.pregameLobbyEnd =	atoi( CG_Argv( 3 ) );

	CAP(cg.pregameLobbyNum, MAX_CLIENTS);
	readScores = cg.pregameLobbyNum;
	CAP(readScores, MAX_CLIENT_SCORE_SEND);

	memset(cg.lobby, 0, sizeof(cg.lobby));

	for(i = 0; i < readScores; i++)
	{
		cg.lobby[i].client =	atoi( CG_Argv( i * 2 + 4 ) );
		cg.lobby[i].ping =		atoi( CG_Argv( i * 2 + 5 ) );
	}
}
...
static void CG_ParseScores( void ) {
	...
	for ( i = 0 ; i < readScores ; i++ ) {
		...
		cg.scores[i].kills = atoi(CG_Argv(i * 14 + 11));
		cg.scores[i].deaths = atoi(CG_Argv(i * 14 + 12));
		cg.scores[i].assists = atoi(CG_Argv(i * 14 + 13));
...
//[/SIEGECVARFIX]
//Char array for the achievements
...
//[/CoOp]
//[CO-OP: New]
extern void GSA_CG_SetFriendlyNPCName(const char *string, int num);
extern void GSA_CG_SetFriendlyNPC(int slot, int value);
//[/CO-OP: New]
...
// Ghoul2 Insert end
	...
	//eezstreet add
	else if (num >= CS_NPCS && num < CS_NPCS+MAX_FRIENDLY_NPCS)
	{
		GSA_CG_SetFriendlyNPCName(str, num);
	}
	//eezstreet end
	//[/CoOp]
...
extern void CG_EVBox_AddString(char *chatStr);
...
//	char buf[64];
	...
	if(!strcmp(cmd, "gid"))
	{ //delete instance of cg.grenadeIndicators[x]
		CG_DeleteGrenadeIndicatorEnt((unsigned short)atoi(CG_Argv(1)));
		return;
	}
	...
	//eezstreet edit
	if (!strcmp(cmd, "fnpc"))
	{ //sets the ent num of a friendly NPC slot
		GSA_CG_SetFriendlyNPC(atoi(CG_Argv(1)), atoi(CG_Argv(2)));
		return;
	}
	//eezstreet end
	//[/CO-OP: New]
	...
	if(!strcmp(cmd, "vippb"))
	{
		cgs.vipName = atoi(CG_Argv(1));
		return;
	}
	if(!strcmp(cmd, "ppb"))
	{
		cgs.blueCName = atoi(CG_Argv(1));
		return;
	}
	if(!strcmp(cmd, "ppr"))
	{
		cgs.redCName = atoi(CG_Argv(1));
		return;
	}
	//[/VIP Escort Fix]
	...
	if( !strcmp( cmd, "weps" )){
		CG_WeaponSwapInit(atoi(CG_Argv(1)));
		return;
	}
	...
	if(!strcmp(cmd, "flashbang"))
	{
		vec3_t parameter;
		parameter[0] = atof(CG_Argv(1));
		parameter[1] = atof(CG_Argv(2));
		parameter[2] = atof(CG_Argv(3));
		CG_Flashbang(parameter);
		return;
	}

	if(!strcmp(cmd, "clrdraw"))
	{
		cg.flashTime = 0;
	...
	{
#ifdef ACHIEVEMENTS
		GSA_IncrementAchievement(&cgs.achievements[atoi(CG_Argv(1))], atoi(CG_Argv(2)));
#endif
		return;
	}
	if(!strcmp(cmd, "aprogmulti"))
	{
#ifdef ACHIEVEMENTS
		int i;
		int amount = atoi(CG_Argv(1));
		for(i = 2; i < trap_Argc(); i++)
		{
			GSA_IncrementAchievement(&cgs.achievements[atoi(CG_Argv(i))], amount);
		}
#endif
		return;
	}
	...
	if(!strcmp(cmd, "cheats")){
		cg.cheatsActive = qtrue;
		return;
	}
	...
	if( !strcmp( cmd, "scp" ) )
	{
		...
		CG_SubCenterPrint( strEd, 260, 12, 0.5f );
		return;
	}
	...
	if( !strcmp( cmd, "send_event" )){ //Another send_ command, brought to you by your reliable command network (tm)
		//possibly have a sound for this?
		Q_strncpyz( text, CG_Argv(1), MAX_SAY_TEXT );
		CG_RemoveChatEscapeChar( text );
		CG_EVBox_AddString(text);
		CG_Printf( "*%s\n", text );
		return;
	}
	if( !strcmp( cmd, "progress_message")){
		if(!cg_progress.integer){ //If we don't do this, the game will asplode -- you can't divide by zero
			return;
		}
		else if(atoi(CG_Argv(2))%cg_progress.integer){
			Q_strncpyz( text, CG_Argv(1), MAX_SAY_TEXT );
			CG_RemoveChatEscapeChar( text );
			CG_EVBox_AddString(text);
			CG_Printf("*%s\n", text);
	...
	if(!strcmp(cmd, "ksReward"))
	{
		CG_KillstreakReward();
		return;
	}
	if(!strcmp(cmd, "multikill"))
	{
		int playerNum = atoi(CG_Argv(1));
		CG_EVBox_AddString(va("^2%s performed a Multi-Kill!", cgs.clientinfo[playerNum].name));
		trap_S_StartSound(NULL, 0, CHAN_LOCAL, trap_S_RegisterSound("sound/weapons/force/protect.mp3"));
		return;
	}
	if(!strcmp( cmd, "sbd" )){ //send base data
		int i;
		cgs.dom.numBases = atoi(CG_Argv(1));
		for(i = 0; i < cgs.dom.numBases; i++)
			cgs.dom.baseControl[i] = atoi(CG_Argv(i + 2));
		return;
	}
	if(!strcmp( cmd, "sbn" )){ //send base names
		int i;
		for(i = 0; i<atoi(CG_Argv(1)); i++){
			strcpy(cgs.dom.baseNames[i], CG_Argv(2+i));
		}
		return;
	}
	if(!strcmp( cmd, "dom_cbp")){ //change base possession
		int i;
		int base = atoi(CG_Argv(1));
		int team = atoi(CG_Argv(2));
		int ourPossession = 0, enemyPossession = 0;
		//Broadcast the event.
		if(team == cg.snap->ps.persistant[PERS_TEAM])
			CG_EVBox_AddString(va("We took %s!", cgs.dom.baseNames[base]));
		else
			CG_EVBox_AddString(va("We've lost %s!", cgs.dom.baseNames[base]));

		//Let's find out how many bases we possess
		for(i = 0; i < cgs.dom.numBases; i++){
			if(cgs.dom.baseControl[i] == cg.snap->ps.persistant[PERS_TEAM])
				ourPossession++;
			else if(cgs.dom.baseControl[i] != TEAM_FREE && cgs.dom.baseControl[i] != TEAM_SPECTATOR)
				enemyPossession++;
		}
		if(cg.snap->ps.persistant[PERS_TEAM] == team && ourPossession == cgs.dom.numBases-1){
			//The enemy is being dominated!
			CG_AddBufferedSound( cgs.media.redLeadsSound );
			CG_EVBox_AddString("The enemy is being dominated!");
			return;
		}
		else if(cg.snap->ps.persistant[PERS_TEAM] == team && ourPossession > enemyPossession) //We've taken the lead on number of bases
			CG_AddBufferedSound( cgs.media.redLeadsSound );
		else if(cg.snap->ps.persistant[PERS_TEAM] == team) //We took this base, but we didn't necessarily take the lead
			CG_AddBufferedSound( cgs.media.redScoredSound );
		else if(cg.snap->ps.persistant[PERS_TEAM] != team && enemyPossession == cgs.dom.numBases-1){
			//We're being dominated!
			//TODO: Countdown timer
			CG_AddBufferedSound( cgs.media.blueLeadsSound );
			CG_EVBox_AddString("We're being dominated!");
			return;
		}
		else if(cg.snap->ps.persistant[PERS_TEAM] != team && ourPossession < enemyPossession) //Enemy has the lead
			CG_AddBufferedSound( cgs.media.blueLeadsSound );
		else if(cg.snap->ps.persistant[PERS_TEAM] != team) //Enemy took base, but is not in the lead.
	...
	if( !strcmp( cmd, "lobbyEnd" ) ){
		cg.pregameLobbyEnd = 0;
		return;
	}
	if( !strcmp( cmd, "bldspy" ) ) {
		int mod = atoi(CG_Argv(1));
		cg.bloodSprayTime = cg.time + modData[mod].bloodSplashTime;
		cg.bloodSprayStartTime = cg.time;
		return;
	}
	if( !strcmp( cmd, "poof")){
		trap_S_StartLocalSound(cgs.media.poofSound, CHAN_LOCAL_SOUND);
		cg.poofTime = (cg.time + 2000);
		return;
	}
	if( !strcmp( cmd, "radarKSReward" ))
	{
		CG_UseRadarKillstreak();
		return;
	}
#ifdef AKIMBO
	if( !Q_stricmp(cmd, "brokenakimbo") )
	{
		int arg1 = atoi(CG_Argv(1));	//New weapon
		int arg2 = atoi(CG_Argv(2));	//Old weapon
		int arg3 = atoi(CG_Argv(3));	//Akimbo gun

		cg.akimboGuns[arg2] = WP_NONE;
		cg.akimboGuns[arg1] = arg3;
		return;
	}
#endif
	if( !strcmp( cmd, "arise")){
		...
		const char *info;
		info = CG_ConfigString( CS_SERVERINFO );
		if(!strcmp(CG_Argv(1), "r")){
			if(atoi(CG_Argv(2)) == cg.clientNum){
				s = Info_ValueForKey( info, "g_redteam" );
				trap_S_StartBackgroundTrack( va("music/hero/%s.mp3", s), va("music/hero/%s.mp3", s), qfalse );
			}
			cgs.redHero = atoi(CG_Argv(2));
		}
		else if(!strcmp(CG_Argv(1), "b")){
			s = Info_ValueForKey( info, "g_blueteam" );
			trap_S_StartBackgroundTrack( va("music/hero/%s.mp3", s), va("music/hero/%s.mp3", s), qfalse );
			cgs.blueHero = atoi(CG_Argv(2));
		}
		return;
	}
	if (!strcmp( cmd, "fixmusic")){
		CG_StartMusic(qtrue);
		return;
	}
	...
	//[QuoteSys]
	if ( !strcmp(cmd, "quote" ) ) {
		cg.warQuote = atoi(CG_Argv(1));
		return;
	}

	if ( !strcmp( cmd, "reinforcements" ) ){
		CG_Reinforcements();
		return;
	}
	if(!strcmp(cmd, "forceiswithyou" ) ) {
		CG_ForceIsWithYou();
		return;
	}
