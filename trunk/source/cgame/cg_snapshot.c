...
#ifdef AKIMBO
		CG_CopyG2WeaponInstance(&cg_entities[snap->ps.clientNum], FIRST_WEAPON, cg_entities[snap->ps.clientNum].ghoul2, qfalse);
		CG_CopyG2WeaponInstance(&cg_entities[snap->ps.clientNum], WP_NONE, cg_entities[snap->ps.clientNum].ghoul2, qtrue);
#else
...
static void CG_TransitionSnapshot( void ) {
	...
	for ( i = 0 ; i < cg.snap->numEntities ; i++ ) {
		if(cg.snap->entities[ i ].number > 1024)
			continue;
