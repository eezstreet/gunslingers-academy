...
#define SB_KILLS_X			(SB_SCORELINE_X + .55 * SB_SCORELINE_WIDTH)
#define SB_ASSISTS_X			(SB_SCORELINE_X + .65 * SB_SCORELINE_WIDTH)
#define SB_DEATHS_X			(SB_SCORELINE_X + .80 * SB_SCORELINE_WIDTH)
#define SB_PING_X			(SB_SCORELINE_X + 1.0 * SB_SCORELINE_WIDTH)
#define SB_TIME_X			(SB_SCORELINE_X + 0.95 * SB_SCORELINE_WIDTH)
...
static int scoreSize;		//size of the scoreboard
...
static void CG_DrawClientScore( int y, score_t *score, float *color, float fade, qboolean largeFormat ) 
{
	...
	if ( score->ping != -1 )
	{
		...
		CG_Text_Paint (SB_KILLS_X, y, 1.0f * scale, colorWhite, va("%i", score->kills),0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL );
		CG_Text_Paint (SB_ASSISTS_X, y, 1.0f * scale, colorWhite, va("%i", score->assists),0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL );
		CG_Text_Paint (SB_DEATHS_X, y, 1.0f * scale, colorWhite, va("%i", score->deaths),0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL );
		if(score->ping <= 100){
			CG_DrawPic (SB_PING_X, y+4, 16 * scale, 16 * scale, cgs.media.pingIcon1);
		}
		else if(score->ping > 100 && score->ping <= 270){
			CG_DrawPic (SB_PING_X, y+4, 16 * scale, 16 * scale, cgs.media.pingIcon2);
		}
		else if(score->ping > 270 && score->ping <= 700){
			CG_DrawPic (SB_PING_X, y+4, 16 * scale, 16 * scale, cgs.media.pingIcon3);
		}
		else if(score->ping > 700){
			CG_DrawPic (SB_PING_X, y+4, 16 * scale, 16 * scale, cgs.media.pingIcon4);
		}
		else{
			CG_DrawPic (SB_PING_X, y+4, 16 * scale, 16 * scale, cgs.media.pingIcon4);
		}
//		CG_Text_Paint (SB_PING_X, y, 1.0f * scale, colorWhite, va("%i", score->ping),0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL );	
	...
	else
	{
		...
		CG_Text_Paint (SB_KILLS_X, y, 1.0f * scale, colorWhite, "-",0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL );
		CG_Text_Paint (SB_ASSISTS_X, y, 1.0f * scale, colorWhite, "-",0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL );
		CG_Text_Paint (SB_DEATHS_X, y, 1.0f * scale, colorWhite, "-",0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL );
...
qboolean CG_DrawOldScoreboard( void ) {
	...
	x = ( SCREEN_WIDTH ) / 2;
	y = SCREEN_HEIGHT - 40;
	CG_Text_Paint ( x - CG_Text_Width ( CG_GetStringEdString("MP_QUOTES", va("QUOTE_%i", cg.warQuote)), 0.6f, FONT_SMALL ) / 2, y, 0.6f, colorWhite, CG_GetStringEdString("MP_QUOTES", va("QUOTE_%i", cg.warQuote)), 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL );
	//[/QuoteSys]
	...
	//Ping and time are not drawn. Instead, a small green/yellow/red/black icon is displayed
	//Green - less than 100
	//Yellow - Between 100 and 270
	//Red - Between 270 and 700
	//Black - 700 and up
	//CG_Text_Paint ( SB_PING_X, y, 1.0f, colorWhite, CG_GetStringEdString("MP_INGAME", "PING"), 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM );
	CG_Text_Paint ( SB_KILLS_X, y, 1.0f, colorWhite, CG_GetStringEdString("MP_INGAME", "KILLS"), 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM );
	CG_Text_Paint ( SB_ASSISTS_X, y, 1.0f, colorWhite, CG_GetStringEdString("MP_INGAME", "ASSISTS"), 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM );
	CG_Text_Paint ( SB_DEATHS_X, y, 1.0f, colorWhite, CG_GetStringEdString("MP_INGAME", "DEATHS"), 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM );
