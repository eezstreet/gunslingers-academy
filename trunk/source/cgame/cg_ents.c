...
static void CG_EntityEffects( centity_t *cent ) {
	...
	//laser sights
	if(cent->currentState.eFlags2 & EF2_LASER_SIGHT)
	{
		trace_t tr;
		vec3_t muzzle, pitchConstraint, end;
		vec3_t d_f, d_rt, d_up;
		vec3_t WHITE = {1.0f,1.0f,1.0f};
		int entNum = cent - cg_entities;

		if(cg.renderingThirdPerson || entNum != cg.snap->ps.clientNum)
		{
			...
			{
				VectorCopy(cent->lerpAngles, pitchConstraint);
			}
		}
		else
		{
		...
		//CG_CalcMuzzlePoint( entNum, muzzle ); //Calculate muzzle point
		VectorCopy( cent->tag_laser, muzzle );
		VectorMA( muzzle, cg.distanceCull, d_f, end ); 

		CG_Trace(&tr, muzzle, NULL, NULL, end, -1, CONTENTS_SOLID);
		trap_FX_AddLine( muzzle, tr.endpos, 0.1f, 1.0f, 0.0f, 0.5f, 0.0f, 0.0f, WHITE, WHITE, 0.0f, 175, trap_R_RegisterShader( "gfx/effects/redLine" ), 0x00000100 | 0x00000001 );
	}
...
void CG_Disintegration(centity_t *cent, refEntity_t *ent)
{
	...
	if ( cg.time - ent->endTime < 1000 && (cg_timescale.value * cg_timescale.value * random()) > 0.05f )
	{
		...
		//if ( random() > 0.5f )
		//{
		if(!cent->dAlreadyDone){
			trap_FX_PlayEffectID( cgs.effects.mDisruptorDeathSmoke, fxOrg, fxDir, -1, -1 );
			cent->dAlreadyDone = qtrue;
		}
		//}
	}
}
...
{
	if(cg.leaderLineCount >= sizeof(cg.leaderLines)/sizeof(cg.leaderLines[0]))
	{
		return;
	}
	cg.leaderLines[cg.leaderLineCount++].entNum = cent->currentState.number;
	cg.leaderLines[cg.leaderLineCount].type = type;
	cg.leaderLines[cg.leaderLineCount].visibilityMask = mask;
	cent->leaderLine = qtrue;
	cent->leaderLineNum = cg.leaderLineCount;
}

void CG_DeleteLeaderLineEnt(unsigned short number)
{
	int i;
	int deleteCount = 0;
	centity_t *cent = &cg_entities[number];
	for(i = 0; i < cg.leaderLineCount-1; i++)
	{
		if(cg.leaderLines[i].entNum == number)
		{
			deleteCount++;
			cg.leaderLines[i].entNum = -1;
		}
	}
	if(cg.leaderLineCount > 0)
	{
		cg.leaderLineCount--;
	}
	cent->leaderLine = qfalse;
}

void CG_AddGrenadeIndicatorEnt(centity_t *cent)
{
	if(cg.grenadeIndicatorCount >= sizeof(cg.grenadeIndicators)/sizeof(cg.grenadeIndicators[0]))
	{
		return;
	}
	cg.grenadeIndicators[cg.grenadeIndicatorCount++] = cent->currentState.number;
	//cg.grenadeIndicatorCount++;
	cent->grenadeIndicator = qtrue;
	cent->grenadeIndicatorNum = cg.grenadeIndicatorCount;
}

void CG_DeleteGrenadeIndicatorEnt(unsigned short number)
{
	int i;
	int deleteCount = 0;
	centity_t *cent = &cg_entities[number];
	...
	{
		if(cg.grenadeIndicators[i] == number)
		{
			deleteCount++;
			cg.grenadeIndicators[i] = -1;
		}
	}

	if(cg.grenadeIndicatorCount > 0)
		cg.grenadeIndicatorCount--;
	cent->grenadeIndicator = qfalse;
}
...
static void CG_General( centity_t *cent ) {
	...
	if (cent->currentState.eFlags & EF_RADAROBJECT && !(cent->currentState.eFlags2 & EF_RADAROBJECT) && !(cent->currentState.eFlags & EF_RADAROBJECT))
	{
		CG_AddRadarEnt(cent);
	}
	else if(cent->currentState.eType == ET_MISSILE && cent->currentState.weapon == WP_THERMAL)
	{
		CG_AddGrenadeIndicatorEnt(cent);
	}
...
*/
	...
	//eezstreet - Tint domination bases
	if(cent->currentState.weapon == WP_NUM_WEAPONS){
		if(cent->currentState.teamowner == TEAM_BLUE){
			ent.shaderRGBA[2] = 0xff;
		}
		else if(cent->currentState.teamowner == TEAM_RED){
	...
	{ //special case for client limbs
		...
		if (!cent->ghoul2)
		{
			...
			//Check blood bolting
			if(!cent->bleedBoltInit)
			{
				cent->bleedBoltTime = cg.time + 5000;
				cent->bleedBoltInit = qtrue;
				cent->bleedBolts[0] = 0;
			}
			...
			if ( newBolt != -1 )
			{
				...
				trap_FX_PlayEffectID(cgs.effects.mLimbBolton, boltOrg, boltAng, -1, -1);
				//eez: Ok, the above doesn't make any sense. Let's tie it to the CENT, and then when we run CG_General again, we play the EFX
				cent->bleedBolts[0] = newBolt;
				cent->bleedBolts[1] = 3;
			}
		...
		if(cent->ghoul2 && cent->bleedBoltInit && cent->bleedBoltTime > cg.time && cent->bleedBolts[1] < 1)
		{
			trap_FX_PlayBoltedEffectID(cgs.effects.mLimbBolton, cent->currentState.origin, cent->ghoul2, cent->bleedBolts[0], cent->currentState.number, 0, 100, qtrue);
			cent->bleedBolts[1] = 50;
		}
		cent->bleedBolts[1]--;
...
/*		switch(powerType){
			case FP_CLOAK:
			case FP_SNEAK_ATTACK:
				ent.shaderRGBA[0] = wv*156;
				ent.shaderRGBA[1] = wv*128;
			...
			case FP_RESTORE:
			case FP_MEDITATE:
				ent.shaderRGBA[0] = wv*128;
				ent.shaderRGBA[1] = wv*128;
				ent.shaderRGBA[2] = wv*255;
				break;
			case FP_LUNGE:
			case FP_ROLLSTAB:
			case FP_SPRINT_SLICE:
			case FP_DUCK_SWIPE:
			case FP_KNOCKBACK_SLICE:
			case FP_KATANA:
			case FP_KATA:
			case FP_REFLECT_BULLETS:
			case FP_POISON:
				ent.shaderRGBA[0] = 0;
				ent.shaderRGBA[1] = wv*255;
				ent.shaderRGBA[2] = 0;
				break;
			case FP_FIRE_BOLT:
			case FP_FLAMETHROWER:
			case FP_MELT:
			case FP_VOLCANO:
				ent.shaderRGBA[0] = wv*255;
				ent.shaderRGBA[1] = wv*196;
				ent.shaderRGBA[2] = wv*128;
				break;
			case FP_ICE_BOLT:
			case FP_FROSTNOVA:
			case FP_METEOR:
				ent.shaderRGBA[0] = wv*196;
				ent.shaderRGBA[1] = wv*196;
				ent.shaderRGBA[2] = wv*255;
				break;
			case FP_SPARK:
			case FP_THUNDERSTORM:
				ent.shaderRGBA[0] = wv*255;
				ent.shaderRGBA[1] = wv*255;
				ent.shaderRGBA[2] = 0;
				break;
...
/*				switch(powerType){
			...
			case FP_VOLCANO:
				fxSArgs.sAlpha *= 1.5;
				fxSArgs.eAlpha *= 1.5;
				fxSArgs.shader = cgs.media.orangeSaberGlowShader;
				trap_FX_AddSprite(&fxSArgs);
				break;
			case FP_ICE_BOLT:
			case FP_FROSTNOVA:
			...
			case FP_LEVITATE:
				fxSArgs.sAlpha *= 1.5;
				fxSArgs.eAlpha *= 1.5;
				fxSArgs.shader = cgs.media.purpleSaberGlowShader;
				trap_FX_AddSprite(&fxSArgs);
				break;
		}*/
		if (cent->currentState.trickedentindex3 == 1)
...
/*		if (CG_GreyItem(item->giType, item->giTag, cg.snap->ps.fd.forceSide))
	...
	else
	{
		...
		VectorAdd(cent->lerpOrigin, weaponData[item->giTag].lerpOrigin, output);
		VectorCopy(output, cent->lerpOrigin);
	}

	if(item->giType == IT_WEAPON && VectorDistance(cg.snap->ps.origin, cent->lerpOrigin) < 36.0f && !(cg.snap->ps.stats[STAT_WEAPONS] & (1 << item->giTag)) &&
		( (weaponData[item->giTag].weaponType == WT_EXPLOSIVE && weaponData[cg.snap->ps.weapon].weaponType == WT_EXPLOSIVE) ||
		weaponData[item->giTag].weaponType != WT_EXPLOSIVE && weaponData[cg.snap->ps.weapon].weaponType != WT_EXPLOSIVE) )
	{
		CG_WeaponSwapInit(item->giTag);
	}
	else if(item->giType == IT_WEAPON && VectorDistance(cg.snap->ps.origin, cent->lerpOrigin) < 36.0f && cg.snap->ps.stats[STAT_WEAPONS] & (1 << item->giTag) &&
		!(cent->currentState.eFlags & EF_DROPPEDWEAPON))
	{
		CG_WeaponAddAmmoInit(item->giTag);
	}
...
//	int	col;
	...
	if (s1->weapon == WP_THERMAL && s1->eFlags & EF_RADAROBJECT && !cent->grenadeIndicator)
	{
		CG_AddGrenadeIndicatorEnt(cent);
	}
	...
	{
		// add trails
		//hackhackhack for marker nades
		if(s1->weapon == WP_TRIP_MINE && s1->activeForcePass)
		{
			cg_weapons[WP_ROCKET_LAUNCHER].missileTrailFunc( cent, &cg_weapons[WP_ROCKET_LAUNCHER] );
		}
...
*/
	...
	{ //in jedimaster always make the saber glow when on the ground
		...
		cg.leaderLines[0].entNum = cent - cg_entities;
		cg.leaderLines[0].type = LL_KATANA;
...

		{
			...
			{
				CG_PlayerFloatSprite( cent, CG_LeaderLineGraphicForType(LL_KATANA), qfalse, 50 );
			}
		}
...
{
	char buffer[64];
	if(cent->currentState.soundSetIndex < 0)
		return;

	BG_GetDoorSoundName(cent->currentState.soundSetIndex, buffer);

	//NOTE: I used rand() instead of Q_irand() because Q_irand() was always giving me the same result
	switch(type)
	{
		case NDS_SQUEAK:
			trap_S_StartSound(NULL, cent->currentState.number, CHAN_AUTO, trap_S_RegisterSound(va("sound/movers/doors_new/%s_squeak%i", buffer, Q_irand(1, 3))));
			break;
		case NDS_OPEN:
			trap_S_StartSound(NULL, cent->currentState.number, CHAN_AUTO, trap_S_RegisterSound(va("sound/movers/doors_new/%s_open%i", buffer, Q_irand(1, 2))));
			break;
		case NDS_CLOSE:
			trap_S_StartSound(NULL, cent->currentState.number, CHAN_AUTO, trap_S_RegisterSound(va("sound/movers/doors_new/%s_close%i", buffer, Q_irand(1, 2))));
			break;
		case NDS_OPENBLOCK:
			trap_S_StartSound(NULL, cent->currentState.number, CHAN_AUTO, trap_S_RegisterSound(va("sound/movers/doors_new/%s_openblocked", buffer)));
			break;
		case NDS_CLOSEBLOCK:
			trap_S_StartSound(NULL, cent->currentState.number, CHAN_AUTO, trap_S_RegisterSound(va("sound/movers/doors_new/%s_closeblocked", buffer)));
			break;
	}
}
...
void CG_AddPacketEntities( qboolean isPortal ) {
	...
	if (isPortal)
	{
		...
		{
			if(cg.snap->entities[ num ].number > 1024)
				continue;
