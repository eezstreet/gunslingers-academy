...
/*	fileHandle_t	f;
	...
	{//keep scanning thru the weapon define groups until we're out of define groups.
		...
		{//set firetime
			weaponData[WeaponNum].fireTime[pm->ps->stats[STAT_FPOS]] = atoi(text);
		}
		...
		{//set altfiretime
			weaponData[WeaponNum].altFireTime[pm->ps->stats[STAT_FPOS]] = atoi(text);
		}
	...
	return; //STFUUUUUUUUUUUUUUUUUUUUUUUU
}
//[/WEAPONSDAT]
