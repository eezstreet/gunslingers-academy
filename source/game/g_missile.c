...
void G_BounceMissile( gentity_t *ent, trace_t *trace ) {
	...
	if (ent->s.weapon == WP_THERMAL || ent->s.weapon == WP_TRIP_MINE || ent->s.weapon == WP_REPEATER || ent->s.weapon == WP_AUXILIARY6 || ent->s.weapon == WP_AUXILIARY7 ||
		ent->s.weapon == WP_AUXILIARY8)
...
void G_ExplodeMissile( gentity_t *ent ) {
	...
	// grenade indicators
	if(ent->s.eFlags & EF_RADAROBJECT && ent->s.eFlags2 & EF_RADAROBJECT)
	{
...
void G_MissileImpact( gentity_t *ent, trace_t *trace ) {
	...
	else if( (ent->s.weapon == WP_THERMAL || ent->s.weapon == WP_TRIP_MINE || ent->s.weapon == WP_AUXILIARY6 || ent->s.weapon == WP_AUXILIARY7 || ent->s.weapon == WP_AUXILIARY8) 
		&& other->takedamage ){
		if(ent->fleshBounces > 0)
			return;
		G_Damage(other, ent->parent, ent->parent, ent->r.currentAngles, ent->r.currentOrigin, TD_SMACK_DAMAGE, 0, ent->methodOfDeath);
		ent->fleshBounces++;
		G_BounceMissile( ent, trace );
		G_AddEvent( ent, EV_GRENADE_BOUNCE, 0 );
		return;
	}
	else if( ent->s.weapon == WP_REPEATER && ent->s.userInt1 < level.time && other->takedamage && ent->alt_fire )
	{
		if(ent->fleshBounces > 0)
			return;
		G_Damage(other, ent->parent, ent->parent, ent->r.currentAngles, ent->r.currentOrigin, TD_SMACK_DAMAGE, 0, ent->methodOfDeath);
	...
	if (ent->s.weapon == WP_ROCKET_LAUNCHER || ent->s.weapon == WP_THERMAL || ent->s.weapon == WP_DET_PACK || ent->s.weapon == WP_TRIP_MINE){

		SetDMSState( 3 );
	}
	
	// I would glom onto the FL_BOUNCE code section above, but don't feel like risking breaking something else
	if ( ((ent->bounceCount > 0 || ent->bounceCount == -5) && ( ent->flags&(FL_BOUNCE_SHRAPNEL) ) ) || ((trace->surfaceFlags&SURF_FORCEFIELD)&&!ent->splashDamage&&!ent->splashRadius&&(ent->bounceCount > 0 || ent->bounceCount == -5)) ) 
	{
		if(other->takedamage && ent->s.weapon == WP_STUN_BATON)
		{
...
killProj:
	...
	if ( other->takedamage && other->client && !isKnockedSaber ) {
		...
		gentity_t *te = G_TempEntity( ent->r.currentOrigin, EV_MISSILE_MISS );
		te->s.eventParm = DirToByte( trace->plane.normal );
		te->s.generic1 = (trace->surfaceFlags & MATERIAL_MASK);
