...
extern void Jedi_Decloak( gentity_t *self );
extern void BG_SetAnimFinal(playerState_t *ps, animation_t *animations,
					 int setAnimParts,int anim,int setAnimFlags,
					 int blendTime);		// default blendTime=350
void ForceGrip( gentity_t *self )
{
	//ANIMS:
	//Not moving: Use BOTH_V1_B__S1
	//Moving: Use BOTH_FORCE_DRAIN_START
	...
	{
		Jedi_Cloak(self);
		if(abs(self->client->pers.cmd.forwardmove) > 0 || abs(self->client->pers.cmd.rightmove) > 0)
		{
			BG_SetAnimFinal(&self->client->ps, bgHumanoidAnimations, SETANIM_TORSO, BOTH_FORCE_DRAIN_START, SETANIM_FLAG_HOLD, 150);
		}
		else
		{
			BG_SetAnimFinal(&self->client->ps, bgHumanoidAnimations, SETANIM_TORSO, BOTH_FORCE_DRAIN_START, SETANIM_FLAG_HOLD, 250);
		}
		self->client->ps.forceHandExtend = HANDEXTEND_FORCEPUSH;
		self->client->ps.forceHandExtendTime = level.time + 250;
	}
	else
	{
		Jedi_Decloak(self);
...
{ //keep jedi master status updated for JM gametype
	...
	while (i < NUM_FORCE_POWERS)
	{
		if (self->client->ps.isJediMaster)
		{
			...
			if( Q_stricmpn(Info_ValueForKey ( userinfo, "model" ), "ninja", 5) )
			{
				ClientUserinfoChanged( clientNum );
				strcpy(self->client->oldModel, Info_ValueForKey ( userinfo, "model" ));
				Info_SetValueForKey(userinfo, "model", "ninja");
				trap_SetUserinfo( clientNum, userinfo );
				ClientUserinfoChanged( clientNum );
			}
			if( Q_stricmpn(Info_ValueForKey (userinfo, "saber" ), self->client->saber[0].name, 6) )
			{
				ClientUserinfoChanged( clientNum );
				Info_SetValueForKey(userinfo, "saber", "katana");
				trap_SetUserinfo( clientNum, userinfo );
				ClientUserinfoChanged( clientNum );
				if(self->client->ps.fd.saberAnimLevel == SS_DUAL)
				{
					if( Q_stricmpn(Info_ValueForKey (userinfo, "saber2" ), self->client->saber[1].name, 6) )
					{
						ClientUserinfoChanged( clientNum );
						Info_SetValueForKey(userinfo, "saber2", "katana");
						trap_SetUserinfo( clientNum, userinfo );
						ClientUserinfoChanged( clientNum );
					}
				}
		...
		else
		{
			...
			{
				ClientUserinfoChanged( clientNum );
				if(self->client->oldModel && Q_stricmpn(self->client->oldModel, "ninja", 5))
					Info_SetValueForKey(userinfo, "model", self->client->oldModel);
				else
