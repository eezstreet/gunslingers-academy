...
/*
===============
Deferred Reliable Command code
===============
*/

void G_InitDeferredReliableCommands( void )
{
	numDeferredCommands = 0;
	memset(deferredCommands, 0, sizeof(deferredCommands));
}

void G_AddDeferredReliableCommand( int *clientNums, int time, char *text )
{
	deferredReliableCommand_t *command = &deferredCommands[numDeferredCommands];

	if(clientNums[0] < 0)
	{
		//Probably sending this to all clients if this is the case
		command->clientsInvolved[0] = -1;
	}
	else
	{
		//command->clientsInvolved = clientNums;
	}
	command->timeToSend = time;
	strcpy(command->messageText, text);

	numDeferredCommands++;
}

void G_ExecuteReliableCommand( int number )
{
	int i;
	deferredReliableCommand_t command = deferredCommands[number];

	if(command.clientsInvolved[0] < 0)
	{
		//Send to all clients
		trap_SendServerCommand(-1, command.messageText);
	}
	else
	{
		//Little more complicated but still not too difficult - loop through wanted clients
		for( i = 0;; i++ )
		{
			if(i > 0 && command.clientsInvolved[i] <= 0)
			{
				break;
			}
			else
			{
				trap_SendServerCommand(command.clientsInvolved[i], command.messageText);
			}
		}
	}
	//Rearrange the deferred command stack
	if(number < numDeferredCommands)
	{
		for(i = number; i < numDeferredCommands; i++)
		{
			deferredCommands[i] = deferredCommands[i+1];
		}
	}
	numDeferredCommands--;
}

//Check the level.time against the reliable command set
void G_CheckReliableCommands( void )
{
	int i;
	for(i = 0; i < numDeferredCommands; i++)
	{
		if(deferredCommands[i].timeToSend <= level.time)
		{
			G_ExecuteReliableCommand(i);
		}
	}
}
...
void P_DamageFeedback( gentity_t *player ) {
	...
	if ( (level.time > player->pain_debounce_time) && !(player->flags & FL_GODMODE) && !(player->s.eFlags & EF_DEAD) ) {
		...
		G_AddEvent( player, EV_PAIN, player->client->damage_lastMOD );
...
qboolean ClientInactivityTimer( gclient_t *client ) {
	if ( ! g_inactivity.integer || client->sess.sessionTeam == TEAM_SPECTATOR ) {
		...
		if(g_specInactive.integer && client->sess.sessionTeam != TEAM_SPECTATOR){
			if ( level.time > client->inactivityTime ) {
				SetTeam(&g_entities[client-level.clients], "spectator");
				trap_SendServerCommand(-1, va("send_event \"%s was forced to spectate due to inactivity.\""));
				trap_SendServerCommand( client - level.clients, "cp \"You have been spectated due to inactivity.\"");
				return qfalse;
			}
		...
		} else if(g_specInactive.integer && client->sess.sessionTeam == TEAM_SPECTATOR){
			return qfalse;
		} else{
			if ( level.time > client->inactivityTime ) {
...
==================
*/
extern void G_PickHeroes(void);
void ClientTimerActions( gentity_t *ent, int msec ) {
	gclient_t	*client;
	int	accuracyDebounce = 40;
	int	healthDebounce = 500;
	int	ammoDebounce = 250;
	int	vipDebounce = 2000;
	//[QuoteSys]
	int quoteDebounce = 10000;
	//[/QuoteSys]
	//[Stamina]
	int staminaDebounce = 100;
	//[/Stamina]
//	int heroDebounce = 240000;
	//[AnnouncementsSys]
	int messageDebounce = g_amessageDelay.integer;
	//[/AnnouncementsSys]


	client = ent->client;
	client->timeResidual += msec;
	client->timers[ACCURACY_REGEN] += msec;
	client->timers[HEALTH_REGEN] += msec;
	client->timers[AMMO_INSTAGIB_REGEN] += msec;
	client->timers[VIP_TIMER] += msec;
	client->timers[DOMINATION_TIMER] += msec;
	//[QuoteSys]
	client->timers[QUOTE_TIMER] += msec;
	//[/QuoteSys]
	//[Stamina]
	client->timers[STAMINA_TIMER] += msec;
	//[/Stamina]
	//[AnnouncementsSys]
	client->timers[MESSAGE_TIMER] += msec;
	//[/AnnouncementSys]
	/*IFGAME(GT_HERO){
		level.heroPickTime += msec;
		if(level.heroPickTime >= heroDebounce){
			level.heroPickTime = 0;
			G_PickHeroes();
		}
	}*/
	if(pm->cmd.upmove < 0)
		accuracyDebounce = 20;
	else if(!pm->cmd.rightmove){ //BugFix: accuracyDebounce uses -, not =
		if(accuracyDebounce < 0)
			accuracyDebounce = 30;
		else
			accuracyDebounce -= 10;
	}
	...
	if(g_gametype.integer == GT_VIP_ESCORT || g_gametype.integer == GT_COMMANDER){
		if(level.anotherVIP){
			if(client->timers[VIP_TIMER] >= vipDebounce){
				G_PickVIP();
			}
		}
		if(level.anotherBCommander){
			if(client->timers[VIP_TIMER] >= vipDebounce){
				G_PickBComm();
			}
		}
		if(level.anotherRCommander){
			if(client->timers[VIP_TIMER] >= vipDebounce){
				G_PickRComm();
			}
		}
	}
	while( client->timers[HEALTH_REGEN] >= healthDebounce)
	{
		client->timers[HEALTH_REGEN] -= healthDebounce;
		if(ent->health < client->ps.stats[STAT_MAX_HEALTH])
			ent->health++;
	}
	while( client->timers[STAMINA_TIMER] >= staminaDebounce && !(client->pers.cmd.buttons & BUTTON_SPRINT)){
		client->timers[STAMINA_TIMER] -= staminaDebounce;
		if(client->ps.stats[STAT_STAMINA] < g_staminaMax.integer)
			client->ps.stats[STAT_STAMINA] += g_staminaRegenRate.integer;
	}
	if(g_instagib.integer == 1){
		while(client->timers[AMMO_INSTAGIB_REGEN] >= ammoDebounce){
			client->timers[AMMO_INSTAGIB_REGEN] -= ammoDebounce;
	...
	//[QuoteSys]
	while( client->timers[QUOTE_TIMER] >= quoteDebounce){
		client->timers[QUOTE_TIMER] -= quoteDebounce;
		level.warQuote = Q_irand(1,MAX_QUOTES);
	} //[/QuoteSys]

	while( client->timers[MESSAGE_TIMER] >= messageDebounce ){
		client->timers[MESSAGE_TIMER] -= messageDebounce;
		trap_SendServerCommand(ent->s.number, va("chat_noecho \"%s\"", g_amessage1.string));
		trap_SendServerCommand(ent->s.number, va("chat_noecho \"%s\"", g_amessage2.string));
		trap_SendServerCommand(ent->s.number, va("chat_noecho \"%s\"", g_amessage3.string));
	}

	while ( client->timers[ACCURACY_REGEN] >= accuracyDebounce )
	{//every accuracyDebounce ms, increase stamina
		client->timers[ACCURACY_REGEN] -= accuracyDebounce;

		if ( client->ps.stats[STAT_ACCURACY] > 0 ){
			if(client->ps.groundEntityNum != ENTITYNUM_NONE &&		//not in the air
			!BG_InRoll(&client->ps, client->ps.legsAnim) &&		//not in a roll
			!PM_InKnockDown(&client->ps) &&						//not knocked down
			(!BG_SaberInSpecialAttack(client->ps.torsoAnim) &&	//not doing special attack
			!BG_SaberInAttack(client->ps.saberMove) &&			//not attacking
			!BG_SaberInTransitionAny(client->ps.saberMove))){	//not in a transition
				if(pm->cmd.upmove < 0 && !pm->cmd.rightmove) //crouching
					client->ps.stats[STAT_ACCURACY] -= 1;
				else if(pm->cmd.upmove < 0 && pm->cmd.rightmove != 0) // crouch walking
					client->ps.stats[STAT_ACCURACY] -= 1;
				else if(!pm->cmd.rightmove)
					client->ps.stats[STAT_ACCURACY] -= 1; //standing still
				else
					client->ps.stats[STAT_ACCURACY] -= 0.5;
			}
	}
	...
	CAP( ent->client->ps.stats[STAT_ACCURACY], 500 );
	BUMP( ent->client->ps.stats[STAT_ACCURACY], 0 );
}
}
		...
		case EV_ROLL:
			{
				...
				if (knockDownage)
				{
					damage = (int)delta*1.75; //you suffer for falling unprepared. A lot. Makes throws and things useful, and more realistic I suppose.
				}
				else
				{
					...
					{ //longer falls hurt more
						damage = (int)delta*1.75; //good enough for now, I guess
					}
					else
					{
						damage = delta*1.75; //good enough for now, I guess
					}
				}
...
static void G_UpdateTeamBroadcasts ( gentity_t *self )
{
	int i;
	if(g_gametype.integer < GT_TEAM)
		return;

	// Anyone who is teammates with this client should see them
	for ( i = 0; i < level.numConnectedClients; i++ )
	{
		...
		if(ent->client->sess.sessionTeam != self->client->sess.sessionTeam)
		{
			continue;
		}
...
//Raz0r's code follows
gentity_t *currentPMEnt = NULL;
#ifdef AKIMBO
void G_PM_SwitchWeaponClip( playerState_t *ps, byte newweapon, qboolean leftWeapon )
#else
void G_PM_SwitchWeaponClip( playerState_t *ps, byte newweapon )
#endif
{
	...
	if(leftWeapon)
	{
		if(weaponData[ps->weapon2].clipSize )
			ent->client->clipammo2[ent->client->ps.weapon2] = ps->stats[STAT_CLIP2];
	}
	else if ( weaponData[ps->weapon].clipSize )
#else
	if ( weaponData[ps->weapon].clipSize )
#endif
		ent->client->clipammo[ent->client->ps.weapon] = ps->stats[STAT_CLIP];

	// Get the new weapon's ammo stored in STAT_AMMO
	if ( weaponData[newweapon].clipSize )
	{
#ifdef AKIMBO
	if(leftWeapon)
	{
		if(weaponData[ent->client->akimboCounterPart[newweapon]].clipSize)
			ps->stats[STAT_CLIP2] = ent->client->clipammo2[ent->client->akimboCounterPart[newweapon]];
	}
	else
#endif
		ps->stats[STAT_CLIP] = ent->client->clipammo[newweapon];
	}

	//STCTODO: Meep. clipammo2/STAT_AMMO2 may need changing? eh
	//	If so, you might need to use cl->wield[weap]
}
//end Raz
#ifdef AKIMBO
//Only for akimbo'd weapons.
...
void ClientThink_real( gentity_t *ent ) {
	...
	if(!isNPC)
	{
		if(g_gametype.integer == GT_JEDIMASTER)
		{
			if(client->ps.isJediMaster)
			{
				client->ps.eFlags2 |= EF2_LEADERLINES;
				client->ps.llType = LL_KILL;
			}
			else
			{
				client->ps.eFlags2 &= ~EF2_LEADERLINES;
		...
		else if(g_gametype.integer == GT_VIP_ESCORT)
		{
			if(level.vipPlayer == client->ps.clientNum)
			{
				client->ps.eFlags2 |= EF2_LEADERLINES;
				client->ps.llType = LL_SUBDUE;
				client->ps.llMask |= LLM_BLUE;
			}
			else
			{
				client->ps.eFlags2 &= ~EF2_LEADERLINES;
...
#ifdef _RELOADING
	//auto reloading
	if( client && client->ps.stats[STAT_CLIP] <= 0 && client->ps.weapon != WP_MELEE && client->ps.weapon != WP_STUN_BATON && client->ps.weapon != WP_NONE && client->ps.weapon != WP_SABER )
	{
		Cmd_Reload_f(ent);
	}
#endif
	if( client && client->ps.stats[STAT_CLIP] <= 0 && (weaponData[client->ps.weapon].autoReload || ent->r.svFlags & SVF_BOT) && !client->noAutoReload )
	{
#ifdef AKIMBO
...
#ifdef AKIMBO
	if( client && client->ps.stats[STAT_CLIP2] <= 0 && (weaponData[client->ps.weapon2].autoReload || ent->r.svFlags & SVF_BOT) && !client->noAutoReload )
	{
		Cmd_Reload_f(ent, 2);
	}
#endif
...
#define	MIN_NPC_SPEED	16.0f
	...
	{
		weaponData_t *weapon = &weaponData[client->ps.weapon];
		// set speed
		...
		//Having certain weapons equipped alters our speed
		client->ps.speed *= weapon->speedModifier;
		if(client->pers.cmd.buttons & BUTTON_SIGHTS && !(client->pers.cmd.buttons & BUTTON_SPRINT))
			client->ps.speed *= weapon->ADSspeedModifier;
	...
	client->ps.stats[STAT_CLIP] = client->weaponClips[client->ps.weapon];
#ifdef AKIMBO
	client->ps.stats[STAT_CLIP2] = client->weaponClips[client->ps.weapon2];
#endif
...
//Raz start
//STC: Update the networked value for ammo in current clip
...
//		PM will do its changes in there and afterwards we'll fetch the updated value
	if ( weaponData[ent->client->ps.weapon].clipSize )	ent->client->ps.stats[STAT_CLIP]	= ent->client->clipammo[ent->client->ps.weapon];
#ifdef AKIMBO
	if ( weaponData[ent->client->ps.weapon2].clipSize )	ent->client->ps.stats[STAT_CLIP2]	= ent->client->clipammo2[ent->client->ps.weapon2];
#endif
//end raz
	...
	//Raz start
	if ( weaponData[ent->client->ps.weapon].clipSize )	ent->client->clipammo[ent->client->ps.weapon]	= ent->client->ps.stats[STAT_CLIP];
#ifdef AKIMBO
	if ( weaponData[ent->client->ps.weapon2].clipSize )	ent->client->clipammo2[ent->client->ps.weapon2]	= ent->client->ps.stats[STAT_CLIP2];
#endif
	//raz end
		...
		case GENCMD_USE_ELECTROBINOCULARS:
				...
				G_ItemUsable(&ent->client->ps, HI_BINOCULARS) )
			{
				...
				else
				{
					...
					if(g_gametype.integer >= GT_TEAM)
					{
						for( ; ijk < MAX_CLIENTS; ijk++ )
						{
							if(g_entities[ijk].client)
							{
								if(OnSameTeam(ent, &g_entities[ijk]))
								{
									trap_SendServerCommand(ijk, va("radarKSReward %i %i", rewardVariationForReward(HI_BINOCULARS), ent->client->ps.clientNum));
								}
							}
						}
					}
...
//	G_VehicleAttachDroidUnit( ent );
	...
	if ( client->ps.stats[STAT_HEALTH] <= 0 
		...
		if ( level.time > client->respawnTime && !gDoSlowMoDuel ) {
			...
			else if(g_ffaRespawnTimer.integer)
			{
				forceRes = 1;
			}
...
*/
	if ( !(ent->r.svFlags & SVF_BOT) && !g_synchronousClients.integer && ent->client && ent != NULL && ent->s.number != ENTITYNUM_WORLD ) {
...
void ClientEndFrame( gentity_t *ent ) {
	...
	//Raz start
	if ( weaponData[ent->client->ps.weapon].clipSize )
		ent->client->ps.stats[STAT_CLIP]	= ent->client->clipammo[ent->client->ps.weapon];
	else
		ent->client->ps.stats[STAT_CLIP]	= ent->client->ps.ammo[weaponData[ent->client->ps.weapon].ammoIndex];
#ifdef AKIMBO
	if( weaponData[ent->client->ps.weapon2].clipSize )
		ent->client->ps.stats[STAT_CLIP2]	= ent->client->clipammo2[ent->client->ps.weapon2];
	else
		ent->client->ps.stats[STAT_CLIP2]	= ent->client->ps.ammo[weaponData[ent->client->ps.weapon2].ammoIndex];
#endif
	//Raz end
