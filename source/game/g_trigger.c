...
//[NewEntities1]
void trigger_losers_lose( gentity_t *self, gentity_t *other ){
}
void SP_trigger_lose (gentity_t *ent){
	//trigger_lose does things on targets who lose the game, such as scripts, etc.
	ent->wait = -1;
	ent->nextthink = 0;
...
void SP_trigger_ladder ( gentity_t *self )
{
	...
	self->r.contents = CONTENTS_LADDER|CONTENTS_TRIGGER;

	//Add the ladder to the BG ladder listing for pmove --eezstreet
...
GSA/eezstreet ADD
trigger_airspace
Defines the space where airstrikes, etc will spawn
If a smoke grenade goes off while not under this trigger, it will be returned to
the owner of that smoke grenade
...
/*QUAKED trigger_domination_field
Adds domination count to target while in field.
Domination game mode only.

"target" - corresponding base
"target2" - what to call when turned to Red team
"target3" - what to call when turned to Blue team*/
