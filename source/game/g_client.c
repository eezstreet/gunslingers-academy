...
void OJP_Spectator(gentity_t *ent)
{
	if (ent->client->sess.sessionTeam != TEAM_SPECTATOR)
	{
		...
		ent->health = ent->client->ps.stats[STAT_HEALTH] = 200;
...
void JMSaberThink(gentity_t *ent)
{
	...
	{//RACC - someone has the JediMaster saber.
		...
		{ //disconnected?
			...
			ent->s.eFlags &= ~(EF_NODRAW);
			ent->s.modelGhoul2 = 1;
			ent->s.eType = ET_MISSILE; //FIXME: what in..the..flying fuck?
			ent->enemy = NULL;
	...
	//Leader lines and stuff
	ent->s.llType = LL_KATANA;
	G_RunObject(ent);
	ent->s.eFlags2 = EF2_LEADERLINES;
}
...
//	gentity_t *te;
	...
	te = G_TempEntity(vec3_origin, EV_BECOME_JEDIMASTER);
	ge = G_TempEntity(vec3_origin, EV_GLOBAL_TEAM_SOUND);
	te->r.svFlags |= SVF_BROADCAST;
	ge->r.svFlags |= SVF_BROADCAST;
	te->s.eventParm = GTS_NEW_JEDIMASTER;
	ge->s.eventParm = GTS_NEW_JEDIMASTER;
	te->s.trickedentindex = MAX_CLIENTS+1;
	ge->s.trickedentindex = MAX_CLIENTS+1;
	self->enemy = other;
	...
	{
		if(i != FP_FIRST && i != FP_ABSORB && i != FP_TEAM_FORCE && i != FP_TEAM_HEAL && i != FP_DRAIN)
		{
	...
	other->client->ps.fd.forcePowersKnown |= (1 << FP_GRIP);
	other->client->ps.fd.forcePowerLevel[FP_GRIP] = FORCE_LEVEL_3;
...
gentity_t *SelectRandomFurthestSpawnPoint ( vec3_t avoidPoint, vec3_t origin, vec3_t angles, team_t team ) {
	...
	if ( g_gametype.integer == GT_TEAM || g_gametype.integer == GT_HERO
		&& team != TEAM_FREE 
		&& team != TEAM_SPECTATOR )
	{
		char *classname = NULL;
	...
	//eezstreet edit: CTF spawn points
	else if ( g_gametype.integer == GT_CTF || g_gametype.integer == GT_CTY ||
		g_gametype.integer == GT_VIP_ESCORT || g_gametype.integer == GT_COMMANDER //Escort gametypes use CTF spawn points.
		&& team != TEAM_FREE 
		&& team != TEAM_SPECTATOR )
	{
		char *classname = NULL;
...
void respawn( gentity_t *ent ) {
	...
	if(GSA_IsLMSValid())
	{
		GSA_LMSLiveCheck(ent);
		return;
	}
	if (g_ffaRespawnTimer.integer > 0 && g_gametype.integer != GT_SIEGE)
	{
			if (ent->client->tempSpectate <= level.time)
			{
...
===========
*/
void GSA_InitEndGameData(gclient_t *client)
{
	int i;
	for(i = 0; i < HITINFO_MAX; i++){
		client->endGameStats.chestHits[i] = 0;
		client->endGameStats.faceHits[i] = 0;
		client->endGameStats.footHits[i] = 0;
		client->endGameStats.groinHits[i] = 0;
		client->endGameStats.headHits[i] = 0;
		client->endGameStats.legHits[i] = 0;
		client->endGameStats.shoulderHits[i] = 0;
		client->endGameStats.stomachHits[i] = 0;
	}
	for(i = 0; i < KILLMU_MAX; i++){
		client->endGameStats.killMakeUp[i] = 0;
	}
}
...
//	char		*areabits;
	...
	//[/BugFix11]
		...
		//[PrivatePasswordFix]
		if ( g_password.string[0] && Q_stricmp( g_password.string, "none" ) && Q_stricmp( g_password.string, "0") && strcmp( g_password.string, value) != 0) {
	...
	/*if( (Q_irand(1, 3) == 1) && !isBot )
	{
		static char sTemp[1024];
		Q_strncpyz(sTemp, "Visit gunslingersacademy.com for downloads, news and forums!", sizeof(sTemp));
		return sTemp;
	}*/
...
void ClientBegin( int clientNum, qboolean allowTeamReset ) {
	...
	for(i=0; i < WP_NUM_WEAPONS; i++){
		weaponData_z = weaponData[i];
		ent->firingMode[i] = weaponData_z.fireModes[0];
		ent->firingModePos[i] = 0;
	}
	...
	else
	{
		...
		if(g_gametype.integer != GT_SIEGE){
			...
			//[LastManStanding]
			/*if (g_LMS.integer > 0 && ent->client->lives < 1 && BG_IsLMSGametype(g_gametype.integer) && LMS_EnoughPlayers() && client->sess.sessionTeam != TEAM_SPECTATOR)
			...
			else*/
			{
				...
				{//costs a life to switch teams to something other than spectator.
					ent->client->lives--;
					trap_SendServerCommand(client->ps.clientNum, va("cp \"You have %i lives left.\n", ent->client->lives));
				}*/
			}
		}
		else if(g_gametype.integer == GT_SIEGE && client->sess.sessionTeam != TEAM_SPECTATOR){
			ClientSpawn(ent);
		}
...
void ClientSpawn(gentity_t *ent) {
	...
	client = ent->client;
	client->pers.primaryWeapon = client->pers.nextPrimaryWeapon;
	client->pers.sideArm = client->pers.nextSideArm;
	client->pers.explosives = client->pers.nextExplosives;
	client->lastWeaponSwap = level.time;

	//[Stamina]
	//Our stamina is fully restored upon respawn
	client->ps.stats[STAT_STAMINA] = g_staminaMax.integer;
	//[/Stamina]

	ent->previousAttack = 0;
	...
	trap_SendServerCommand(client->ps.clientNum, "clrdraw");
	...
	ent->alreadyPoofed = qfalse; //We died, so reset our poofed status.
	// find a spawn point
...
//	client->areabits = savedAreaBits;
	...
	//[Reloading]
	for(i = 0; i < WP_NUM_WEAPONS; i++){
		ent->client->clipammo[i] = weaponData[i].clipSize;
		ent->client->clipammo2[i] = weaponData[i].clipSize;
	}
	//[/Reloading]
	...
	//We're a hero, bish.
	IFGAME(GT_HERO){
		if(client->ps.clientNum == level.blueHero || client->ps.clientNum == level.redHero){
			/*client->ps.stats[STAT_HOLDABLE_ITEMS] |= ( 1 << BG_GetItemIndexByTag(HI_JETPACK, IT_HOLDABLE));
			client->ps.stats[STAT_HOLDABLE_ITEMS] |= ( 1 << BG_GetItemIndexByTag(HI_SENTRY_GUN, IT_HOLDABLE));
			client->ps.stats[STAT_HOLDABLE_ITEMS] |= ( 1 << BG_GetItemIndexByTag(HI_CLOAK, IT_HOLDABLE));*/
			client->ps.stats[STAT_HOLDABLE_ITEMS] |= ( 1 << HI_JETPACK ) | ( 1 << HI_MEDPAC) | ( 1 << HI_EWEB) | ( 1 << HI_SEEKER);
			client->ps.stats[STAT_HOLDABLE_ITEM] = ( 1 << BG_GetItemIndexByTag(HI_MEDPAC, IT_HOLDABLE) );
	...
	//[instagib]
			...
			switch(g_instagib_restrict.integer){
				case 0:
	...
	{//jediVmerc is incompatible with this gametype, turn it off!
		...
		if(!((g_gametype.integer == GT_HERO) && (level.redHero == ent->s.number || level.blueHero == ent->s.number))){
			if(g_instagib.integer != 1){
				if (g_gametype.integer == GT_TEAM || g_gametype.integer == GT_CTF || g_gametype.integer == GT_CTY || g_gametype.integer == GT_FFA || g_gametype.integer == GT_HOLOCRON || g_gametype.integer == GT_DUEL || g_gametype.integer == GT_POWERDUEL || g_gametype.integer == GT_HERO || (g_gametype.integer == GT_VIP_ESCORT &&
					ent->s.number != level.vipPlayer) ||(g_gametype.integer == GT_COMMANDER && 
					ent->s.number != level.redTeamCommander &&
					ent->s.number != level.blueTeamCommander)) {
					int gun;
					weaponData_t weapon;
					if(weaponData[client->pers.primaryWeapon].weaponType != WT_PRIMARY)
					{
						client->pers.primaryWeapon = PARM_RANDOM;
					}
					if(client->pers.primaryWeapon == PARM_RANDOM)
					{
						int PickRandomGun; 
redoCalculation1:
						PickRandomGun = primaryWeaponList[Q_irand(0, numPrimaryWeapons-1)];
						if(weaponData[PickRandomGun].weaponType != WT_PRIMARY)
							goto redoCalculation1; //INFINITE LOOP INCOMING
						else
							weapon = weaponData[PickRandomGun];
						gun = PickRandomGun;

					}
					else
					{
						gun = client->pers.primaryWeapon;
						weapon = weaponData[gun];
					}
					client->ps.ammo[weapon.ammoIndex] = weapon.startingAmmo;
					client->ps.stats[STAT_WEAPONS] |= (1 << gun);
					if(weaponData[client->pers.sideArm].weaponType != WT_SIDEARM)
					{
						client->pers.sideArm = SARM_RANDOM;
					}
					if(client->pers.sideArm == SARM_RANDOM)
					{
						int PickRandomGun; 
redoCalculation2:
						PickRandomGun = sidearmList[Q_irand(0, numSidearms-1)];
						if(weaponData[PickRandomGun].weaponType != WT_SIDEARM)
							goto redoCalculation2; //INFINITE LOOP INCOMING
						else
							weapon = weaponData[PickRandomGun];
						gun = PickRandomGun;
					...
					{
						gun = client->pers.sideArm;
						weapon = weaponData[gun];
					}
					client->ps.ammo[weapon.ammoIndex] = weapon.startingAmmo;
					client->ps.stats[STAT_WEAPONS] |= (1 << gun);
					if(weaponData[client->pers.explosives].weaponType != WT_EXPLOSIVE)
					{
						client->pers.explosives = SARM_RANDOM;
					}
					if(client->pers.explosives == SARM_RANDOM)
					{
						int PickRandomGun; 
redoCalculation3:
						PickRandomGun = explosiveList[Q_irand(0, numExplosives-1)];
						if(weaponData[PickRandomGun].weaponType != WT_EXPLOSIVE)
							goto redoCalculation3; //INFINITE LOOP INCOMING
						else
							weapon = weaponData[PickRandomGun];
						gun = PickRandomGun;
					...
					{
						gun = client->pers.explosives;
						weapon = weaponData[gun];
					}
					client->ps.ammo[weapon.ammoIndex] = weapon.startingAmmo;
					client->ps.stats[STAT_WEAPONS] |= (1 << gun);
					//Add our ammo for this weapon.
		}
		else if(ent->s.number == level.vipPlayer || ent->s.number == level.redTeamCommander || ent->s.number == level.blueTeamCommander){ //VIPs are stuck with crappy weapons, commanders use M16s, until they pick up something better
			if(g_gametype.integer == GT_VIP_ESCORT){
				client->ps.stats[STAT_WEAPONS] |= (1 << WP_BRYAR_PISTOL);
				client->ps.weapon = WP_BRYAR_PISTOL;
				client->ps.ammo[AMMO_NINE] = 70;
			}
			else{
				client->ps.stats[STAT_WEAPONS] |= (1 << WP_REPEATER);
				client->ps.weapon = WP_REPEATER;
				client->ps.ammo[AMMO_MSIXTEEN] = 750;
			}
		}
		}
	...
	//Always give us a taser if we're RED in VIP Escort
	if(g_gametype.integer == GT_VIP_ESCORT && ent->s.number != level.vipPlayer && ent->client && ent->client->sess.sessionTeam == TEAM_RED)
	{
		client->ps.stats[STAT_WEAPONS] |= (1 << WP_STUN_BATON);
		client->ps.ammo[weaponData[WP_STUN_BATON].ammoIndex] = weaponData[WP_STUN_BATON].startingAmmo;
	}
...
//			client->ps.weapon = WP_MELEE;
...
#ifdef AKIMBO
				if(weaponData[m].akimbo && bgSiegeClasses[client->siegeClass].classflags & CFL_DUALWIELD)
				{
					WP_SetAkimboWeapon(ent, m, m);
				}
#endif
	...
	client->ps.stats[STAT_HOLDABLE_ITEMS] = client->savedkillstreaks;
	if (g_gametype.integer == GT_SIEGE &&
		client->siegeClass != -1 &&
		client->sess.sessionTeam != TEAM_SPECTATOR)
	...
	else if(g_gametype.integer != GT_HERO)
	{
		//client->ps.stats[STAT_HOLDABLE_ITEMS] = 0;
		//client->ps.stats[STAT_HOLDABLE_ITEM] = 0;
		client->ps.stats[STAT_HOLDABLE_ITEMS] &= ~(1 << HI_EWEB);		//don't want this...
...
//			client->ps.weapon = (1 << WP_MELEE);
		...
		else
		{
			...
			if(client->ps.weapon2)
				client->ps.legsAnim = /*WeaponReadyAnim[client->ps.weapon]*/ BOTH_AKIMBO_AIM;
			else
#endif
				client->ps.legsAnim = /*WeaponReadyAnim[client->ps.weapon]*/ GetIDForString(animTable, weaponData[client->ps.weapon].readyAnim);
		}
	...
	if ( level.intermissiontime || (level.lobbyStartTime && g_pregameLobby.integer) ) {
	...
	*/
	...
	//Add old killstreak rewards
	client->ps.stats[STAT_HOLDABLE_ITEMS] = killstreakRewards[client->ps.clientNum];
	client->ps.stats[STAT_HOLDABLE_ITEMS] &= ~(1 << HI_EWEB); //Bad idea to keep these on death.
	if(rewardVariationForReward(HI_BINOCULARS) == 5)
	{
		client->ps.stats[STAT_HOLDABLE_ITEMS] &= ~(1 << HI_BINOCULARS);
	}
	killstreakRewards[client->ps.clientNum] = 0;
}
...
void ClientDisconnect( int clientNum ) {
	...
	//Track what rank we are, and who else is with us
	if(level.clients[ level.sortedClients[i] ].ps.persistant[PERS_RANK] == 0)
	{
		//We're in the lead! The guy below us should be the top
		if(!(level.clients[ level.sortedClients[i+1] ].ps.persistant[PERS_RANK] & RANK_TIED_FLAG))
		{
			trap_SendServerCommand(level.sortedClients[i+1], "forceiswithyou 1");
		}
	}
	...
	if(g_gametype.integer == GT_VIP_ESCORT || g_gametype.integer == GT_COMMANDER){
		if(ent->s.number == level.vipPlayer){
			level.anotherVIP = qtrue;
		}
		else if(ent->s.number == level.redTeamCommander){
			level.anotherRCommander = qtrue;
		}
		else if(ent->s.number == level.blueTeamCommander){
...
#ifdef AKIMBO
		TossClientItems( ent, qfalse, qfalse );
#else
	...
	G_ClearClientLog(clientNum);
	trap_SendServerCommand(-1, "send_event \"Player disconnected.\"");
}
