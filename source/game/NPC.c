...
//eezsteet add
extern void GSA_FriendlyNPCRegister(gentity_t *npc);
extern void GSA_FriendlyNPCDeregister(gentity_t *npc);
//[/CoOp]
...
void NPC_RemoveBody( gentity_t *self )
{
	...
	*/
	...
	//[CoOp] SP Code
		...
		{
			//[CO-OP: New]
			GSA_FriendlyNPCDeregister(self);
			//[/CO-OP: New]
			G_FreeEntity( self );
			return;
		}
	...
	//	case NPCTEAM_MALON:
		...
		case NPCTEAM_ENEMY:
			...
			*/
			...
			if(NPC->enemy){
				if ( NPC->enemy && (NPC->s.weapon == WP_NONE || NPC->health < (NPC->maxHealth / 4)) ||
					//We've got no weapon, or low health
					((NPC->enemy->s.weapon == WP_SABER && DistanceSquared( NPC->r.currentOrigin, NPC->enemy->r.currentOrigin ) < 128*128) && NPC->enemy->client->ps.saberHolstered < 2)
					//They've got a saber and they're heading right for us!
				...
				{//if in battle and have no weapon, run away, fixme: when in BS_HUNT_AND_KILL, they just stand there
					//eezstreet add: They run away if their health falls low and also based on other conditions
					if ( bState != BS_FLEE )
					{
						if(NPC->enemy->s.weapon == WP_SABER && (DistanceSquared( NPC->r.currentOrigin, NPC->enemy->r.currentOrigin ) < 128*128))
						{ //Run, Forrest, Run!
							NPC_StartFlee( NPC->enemy, NPC->enemy->r.currentOrigin, AEL_DANGER_GREAT, 3000, 7000 );
							return;
						}
						else if(NPC->health < (NPC->maxHealth / 4))
						{
