...
float forceJumpHeight[NUM_FORCE_POWER_LEVELS] = 
{
	...
	32,//(+stepheight+crouchdiff = 130)
	32,//(+stepheight+crouchdiff = 226)
	512//(+stepheight+crouchdiff = 418)
};
...
static qboolean PM_CheckJump( void ) 
{
	...
	//Check whether we can actually jump
	if(pm->ps->stats[STAT_STAMINA] < 250)
	{
		return qfalse;
	}
	if(!weaponData[pm->ps->weapon].canJump)
		return qfalse;

	//Reduce jump height based on stamina
	jumpHeight = forceJumpHeight[0];
	if(pm->ps->stats[STAT_STAMINA] < 300)
	{
		jumpHeight /= 3;
	}
	else if(pm->ps->stats[STAT_STAMINA] < 600)
	{
		jumpHeight /= 2;
	}
...
#ifdef AKIMBO
		if ( pm->ps->weaponTime || pm->ps->weaponTime2)
#else
		if ( pm->ps->weaponTime )
#endif
			...
			{//holding jump in air
				...
				{//can still go up
					...
					if(pm->ps->isJediMaster)
						...
						pm->ps->velocity[2] = (jumpHeight - curHeight) / jumpHeight * forceJumpStrength[0];
					pm->ps->velocity[2] /= 10;
					pm->ps->velocity[2] += JUMP_VELOCITY;
					pm->ps->pm_flags |= PMF_JUMP_HELD;
					if(!pm->ps->stats[STAT_STAMINA])
						pm->ps->stats[STAT_STAMINA] -= 30;
				}
				else if ( curHeight > jumpHeight && curHeight < forceJumpHeight[pm->ps->fd.forcePowerLevel[FP_LEVITATION]] - jumpHeight && pm->ps->isJediMaster )
...
#else
						else if ( !pm->ps->weaponTime && !pm->ps->weaponTime2)
#endif
						{
							parts = SETANIM_BOTH;
						}
...
static void PM_WalkMove( void ) {
	...
	if ( pm->ps->pm_flags & PMF_DUCKED ) {
		if ( wishspeed > pm->ps->speed * pm_duckScale ) {
			if(pm->ps->isJediMaster)
				wishspeed = pm->ps->speed * pm_duckScale * 1.2f;
			else
...
static int PM_TryRoll( void )
{
	...
	if( pm->ps->stats[STAT_STAMINA] < 250 )
		return 0;
	if( !weaponData[pm->ps->weapon].canRoll )
		return 0;
	if(pm->ps->torsoTimer > 0 && (pm->ps->weaponTime > 0 || pm->ps->weaponTime2 > 0))
		return 0;
	...
	{ //We want to roll. Perform a trace to see if we can, and if so, send us into one.
		...
		if ( trace.fraction >= 1.0f )
		{
			...
			//Deduct stamina!
#ifdef QAGAME
			pm->ps->stats[STAT_STAMINA] -= (g_staminaMax.integer/4);
			BUMP(pm->ps->stats[STAT_STAMINA], 0);
#endif
			pm->ps->saberMove = LS_NONE;
			return anim;
		}
...
static void PM_CrashLand( void ) {
	...
	{ //saber handles its own anims
		...
		if (weaponData[pm->ps->weapon].altFireScope && pm->ps->zoomMode == 1)
		{
		...
		else
		{
			...
			else
			{
				if(!pm->ps->torsoTimer)
				{
					...
					if(pm->ps->weapon2)
					{
						PM_StartTorsoAnim( BOTH_AKIMBO_AIM );
					}
					else
#endif
						PM_StartTorsoAnim( /*WeaponReadyAnim[pm->ps->weapon]*/ GetIDForString(animTable, weaponData[pm->ps->weapon].readyAnim) );
				}
			}
		}
	}
...
static void PM_Footsteps( void ) {
				...
				else
				{
					...
					else
					{
						if (pm->ps->weapon != WP_SABER || !PM_AdjustStandAnimForSlope())
						{
							...
							{
								PM_ContinueLegsAnim(PM_LegsSlopeBackTransition(/*WeaponReadyLegsAnim[pm->ps->weapon]*/ GetIDForString(animTable, weaponData[pm->ps->weapon].legsReadyAnim)));
							}
						}
					}
				}
	...
	else if ( pm->ps->pm_flags & PMF_DUCKED )
	{
		int rolled = 0;
		vec3_t absVector;
		VectorCopy(pm->ps->velocity, absVector);

		bobmove = 0.5;	// ducked characters bob much faster
		absVector[0] = abs(absVector[0]);
		absVector[1] = abs(absVector[1]);
		absVector[2] = abs(absVector[2]);

		if ( ( (PM_RunningAnim( pm->ps->legsAnim )&&abs(VectorLengthSquared(absVector))>=20000/*200*200*/) || PM_CanRollFromSoulCal( pm->ps ) ) &&
	...
	else
	{
		...
		{//running
			if(pm->cmd.buttons & BUTTON_SPRINT && pm->cmd.forwardmove > 0 && pm->cmd.upmove == 0 && pm->ps->stats[STAT_STAMINA] > 0)
				bobmove = 0.7f;
			else
				bobmove = 0.4f;	// faster speeds bocb faster
			if ( pm->ps->clientNum >= MAX_CLIENTS &&
				pm_entSelf &&
				pm_entSelf->s.NPC_class == CLASS_WAMPA )
			{
...
#ifdef QAGAME
#ifdef AKIMBO
void G_PM_SwitchWeaponClip(playerState_t *ps, int newweapon, qboolean leftWeapon);
void G_PM_StoreWeaponClip( playerState_t *ps);
#else
...
void PM_FinishWeaponChange( void ) {
...
#ifdef AKIMBO
	//Equip our akimbo-counterpart weapon
	{
		gentity_t *gent = (gentity_t *)pm_entSelf;
		if(gent->client->isAkimbo[weapon])
		{ //This slot uses Akimbo. Find what weapon it corresponds to.
			G_PM_SwitchWeaponClip(pm->ps, weapon, qtrue);
		}
		else if(gent->client->isAkimbo[pm->ps->weapon])
		{
			G_PM_StoreWeaponClip(pm->ps);
		}
		G_PM_SwitchWeaponClip(pm->ps, weapon, qfalse);
		pm->ps->weapon2 = gent->client->akimboCounterPart[weapon];
	}
#else
	...
	{
		gentity_t *gent = (gentity_t *)pm_entSelf;
		pm->ps->stats[STAT_FMODE] = gent->firingMode[weapon];
		pm->ps->stats[STAT_FPOS] = gent->firingModePos[weapon];
		pm->ps->stats[STAT_CLIP] = gent->client->weaponClips[weapon];
	}
#endif
#endif
#endif
...
==============
*/
extern int PMW_GetFiringMode(int weaponNum, int firingMode, qboolean alt);
...
#ifdef _RELOADING
#ifdef _BGRELOADING
void PM_Reload( pmove_t *pm ){
	int leftover = weaponData[pm->ps->weapon].clipSize - pm->ps->stats[STAT_CLIP];
#ifdef QAGAME
	gentity_t *gent = (gentity_t *)pm_entSelf;
#endif
	if(pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] <= 0 || pm->ps->stats[STAT_CLIP] == weaponData[pm->ps->weapon].clipSize)
		return;
	if(weaponData[pm->ps->weapon].clipSize <= 0)
		return;

	if(((pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] + leftover) - weaponData[pm->ps->weapon].clipSize) >= 0){
		pm->ps->stats[STAT_CLIP] = weaponData[pm->ps->weapon].clipSize;
		pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] -= weaponData[pm->ps->weapon].clipSize;
		//pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] += leftover;
#ifdef QAGAME
	gent->client->weaponClips[pm->ps->weapon] = weaponData[pm->ps->weapon].clipSize;
#endif
	} else{
		pm->ps->stats[STAT_CLIP] = pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex];
#ifdef QAGAME
		gent->client->weaponClips[pm->ps->weapon] = pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex];
#endif
		pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] = 0;
	}

	//PM_AddEventWithParm( EV_RELOADING, pm->ps->weapon );
	pm->ps->weaponstate = WEAPON_DROPPING;
...
#ifdef QAGAME
extern void G_KillstreakOptions(gentity_t *ent, unsigned int itemNum);
#endif
...
static void PM_Weapon( void )
{
...
#ifdef AKIMBO
	byte weapon = pm->ps->weapon;
	byte weapon2 = pm->ps->weapon2;
	qboolean firingAkimbo = qfalse;
	qboolean firingBoth = qfalse;
	int			addTimeSecond; //only used if firing both weapons
	qboolean killAfterAnim = qfalse;
	...
	{
		gentity_t *gent = (gentity_t *)pm_entSelf;
		pm->ps->stats[STAT_CLIP] = gent->client->weaponClips[pm->ps->weapon];
	}
#endif
#endif
#endif
	...
	/*if (weaponData[pm->ps->weapon].altFireScope //not using disruptor
	...
	if (pm->ps->forceHandExtend == HANDEXTEND_WEAPONREADY &&
		PM_CanSetWeaponAnims() /*&& !pm->cmd.buttons & BUTTON_SPRINT &&
	...
	if ((BG_InSpecialJump(pm->ps->legsAnim) ||
		BG_InRoll(pm->ps, pm->ps->legsAnim) ||
		PM_InRollComplete(pm->ps, pm->ps->legsAnim)) && !weaponData[pm->ps->weapon].canShootWhileRolling)
	{
		/*
	...
	if ((BG_InSpecialJump(pm->ps->legsAnim) ||
		BG_InRoll(pm->ps, pm->ps->legsAnim) ||
		PM_InRollComplete(pm->ps, pm->ps->legsAnim)) && !weaponData[pm->ps->weapon2].canShootWhileRolling && pm->ps->weapon2)
	{
		/*
		if (pm->cmd.weapon != WP_MELEE &&
			pm->ps->weapon != WP_MELEE &&
		...
		*/
		if (pm->ps->weaponTime2 < pm->ps->legsTimer)
		{
			pm->ps->weaponTime2 = pm->ps->legsTimer;
		}
	}
#endif
	...
	if (PM_CanSetWeaponAnims())
	{
			...
			pm->ps->weapon == WP_DET_PACK)
		{
			if (pm->ps->weapon == WP_THERMAL)
			{
				if ((pm->ps->torsoAnim) == /*WeaponAttackAnim[pm->ps->weapon]*/ GetIDForString(animTable, weaponData[pm->ps->weapon].attackAnim) &&
			...
			{
#ifdef AKIMBO
				if((pm->ps->torsoAnim >= BOTH_AKIMBO_SHOT_PISTOL_RIGHT && pm->ps->torsoAnim <= BOTH_AKIMBO_SHOT_MP_BOTH) &&
					(pm->ps->weaponTime+pm->ps->weaponTime2)-700 <= 0 && pm->ps->weapon2)
				{
					PM_StartTorsoAnim( BOTH_AKIMBO_AIM );
				}
				else
	...
	if ( pm->cmd.buttons & BUTTON_USE_HOLDABLE ) {
		if ( ! ( pm->ps->pm_flags & PMF_USE_ITEM_HELD ) ) {
			...
			else
			{
				...
				{
					if (bg_itemlist[pm->ps->stats[STAT_HOLDABLE_ITEM]].giTag != HI_JETPACK &&
...
#ifdef	QAGAME
				G_KillstreakOptions((gentity_t *)pm_entSelf, bg_itemlist[pm->ps->stats[STAT_HOLDABLE_ITEM]].giTag);
#endif
	...
	*/
...
#ifdef AKIMBO
	if(pm->ps->weaponReloading && (pm->ps->weaponTime + pm->ps->weaponTime2 <= 0))
	{
		pm->ps->weaponReloading = 0;
	}
#endif
...
#ifdef AKIMBO
	if(pm->ps->weaponTime2 > 0)
		pm->ps->weaponTime2 -= pml.msec;
#endif
	...
	if ( pm->ps->weapon != WP_NONE &&
		...
		(weaponData[pm->ps->weapon].clipSize <= 0 || (pm->ps->ammo[ weaponData[pm->ps->weapon].ammoIndex ] + pm->ps->stats[STAT_CLIP]) > 0))
	{
		//eezstreet end
		if ( pm->ps->clientNum < MAX_CLIENTS && pm->ps->ammo[ weaponData[pm->ps->weapon].ammoIndex ] != -1 )
		{
			if( pm->ps->stats[STAT_CLIP] > 0 &&
				...
				weaponData[pm->ps->weapon].clipSize > 0){
					pm->ps->stats[STAT_CLIP] -= amount;
			}
#ifdef _BGRELOADING
			else if( pm->ps->stats[STAT_CLIP] <= 0 &&
				pm->ps->ammo[ weaponData[pm->ps->weapon].ammoIndex ] > 0 &&
				weaponData[pm->ps->weapon].clipSize > 0)
				PM_Reload(pm);
#endif
			// enough energy to fire this weapon?
				...
				((pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] + pm->ps->stats[STAT_CLIP]) < weaponData[pm->ps->weapon].energyPerShot ||
				(pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] + pm->ps->stats[STAT_CLIP]) < weaponData[pm->ps->weapon].altEnergyPerShot) &&
				weaponData[pm->ps->weapon].clipSize > 0)
...
#ifdef AKIMBO
	if ( ((pm->ps->weaponTime + pm->ps->weaponTime2) <= 0) || pm->ps->weaponstate != WEAPON_FIRING ) {
#else
	if ( pm->ps->weaponTime <= 0 || pm->ps->weaponstate != WEAPON_FIRING ) {
#endif
		if ( pm->ps->weapon != pm->cmd.weapon ) {
			PM_BeginWeaponChange( pm->cmd.weapon );
			callCount++; //DEBUG: Determine whenever we get a double switch
		}
	}
	if ( !(pm->cmd.buttons & (BUTTON_ATTACK|BUTTON_ALT_ATTACK)))
	{
		if(pm->ps->weaponTime <= 0)
			pm->ps->userInt1 &= ~UI1_ATTACKRELEASE;
#ifdef AKIMBO
		if(pm->ps->weaponTime2 <= 0)
			pm->ps->userInt1 &= ~UI1_AKIMBOATTACKRELEASE;
#endif
	}
...
#else
	if(!(pm->cmd.buttons & BUTTON_ATTACK) && !(pm->cmd.buttons & BUTTON_ALT_ATTACK) && (pm->ps->weaponstate == WEAPON_FIRING || pm->ps->weaponstate == WEAPON_READY))
	{
		killAfterAnim = qtrue;
		goto feelsBadMan;
	}
	if(pm->ps->weapon2 != WP_NONE && pm->cmd.buttons & BUTTON_ATTACK && pm->ps->weaponTime2 > 0)
	{
		if(!(pm->cmd.buttons & BUTTON_ALT_ATTACK))
			return;
	}
	else if(pm->ps->weapon2 != WP_NONE && pm->cmd.buttons & BUTTON_ALT_ATTACK && pm->ps->weaponTime > 0)
	{
		if(!(pm->cmd.buttons & BUTTON_ATTACK))
			return;
	}
	else if(pm->ps->weapon2 == WP_NONE && pm->ps->weaponTime > 0 && (pm->ps->weaponstate == WEAPON_FIRING || pm->ps->weaponstate == WEAPON_READY))
		return;
	else if( pm->ps->weaponReloading )
	{
		//Extra special case: We're firing an akimbo weapon while our other one is reloading
		switch(pm->ps->weaponReloading)
		{
			case 1:
			default:
				if(pm->ps->weaponTime2 > 0)
	...
	else if(akimbo && attack && pm->ps->weaponTime2 <= 0)
		goto feelsBadMan;
	else if(akimbo && altAttack && pm->ps->weaponTime <= 0)
		goto feelsBadMan;
	else if((pm->ps->weaponTime + pm->ps->weaponTime2) > 0)
		return;
#endif
#ifdef AKIMBO
feelsBadMan:
	...
	//Ugh...PM_Weapon() is beginning to look so nasty.
#ifdef AKIMBO
	{
		if(akimbo)
		{
			if(altAttack && pm->ps->userInt1 & UI1_ATTACKRELEASE)
			{
				return;
			}
			if(attack && pm->ps->userInt1 & UI1_AKIMBOATTACKRELEASE)
				return;
			if(attack)
			{
				if(pm->ps->weaponTime2 > 0 && !altAttack)
					return;
				firingAkimbo = qtrue;
			}
			if(altAttack && attack)
		...
		{
			if ( pm->ps->userInt1 & UI1_ATTACKRELEASE )
				return;
		}
	}
#else
	...
	if (PM_CanSetWeaponAnims() &&
		...
		pm->ps->torsoTimer <= 0 &&
		(pm->ps->torsoAnim) != /*WeaponReadyAnim[pm->ps->weapon]*/ GetIDForString(animTable, weaponData[pm->ps->weapon].readyAnim) &&
	...
	if (((pm->ps->torsoAnim) == TORSO_WEAPONREADY4) &&
		(weaponData[pm->ps->weapon].altFireScope || pm->ps->zoomMode != 1))
	{
		if (pm->ps->weapon == WP_EMPLACED_GUN)
		{
			PM_StartTorsoAnim( BOTH_GUNSIT1 );
	...
	// check for fire
#ifdef AKIMBO
	if(weaponData[pm->ps->weapon].altFireScope && pm->cmd.buttons & BUTTON_ALT_ATTACK)
	{
		pm->ps->weaponTime = weaponData[pm->ps->weapon].altFireTime[0];
	}
	else if(pm->ps->weapon2 && ! (pm->cmd.buttons & BUTTON_ATTACK))
	{
		if(pm->ps->userInt1 & UI1_AKIMBOATTACKRELEASE){
			//pm->ps->weaponTime = weaponData[pm->ps->weapon2].fireModeDelays[PMW_GetFiringMode(pm->ps->weapon, pm->ps->stats[STAT_FMODE], qtrue)];
			pm->ps->userInt1 |= UI1_AKIMBOVALIDFIRE;
			pm->ps->userInt1 |= UI1_VALIDFIRE;
		}
		else
		{
			if(!UI1_AKIMBOVALIDFIRE){
				pm->ps->weaponstate = WEAPON_READY;
				pm->ps->weaponTime2 = 0;
			}
			if(pm->ps->weaponTime2 == 0){
	...
	{
		if(pm->ps->userInt1 & UI1_ATTACKRELEASE){
			pm->ps->weaponTime = weaponData[pm->ps->weapon].fireModeDelays[PMW_GetFiringMode(pm->ps->weapon, pm->ps->stats[STAT_FMODE], qtrue)];
			pm->ps->userInt1 |= UI1_VALIDFIRE;
		}
		else{
			if(!UI1_VALIDFIRE){
		...
		pm->ps->userInt1 &= ~UI1_ONESHOTOFF;
		pm->ps->userInt1 &= ~UI1_TWOSHOTOFF;
		pm->ps->userInt1 &= ~UI1_THREESHOTOFF;
		return;
	}
#else
	if(weaponData[pm->ps->weapon].altFireScope && pm->cmd.buttons & BUTTON_ALT_ATTACK)
	...
	{ 
		...
		if(!(gent->r.svFlags & SVF_BOT)){
			if(pm->ps->weapon2)
			{
				if(firingBoth)
				{ //Firing both
					if(weaponData[pm->ps->weapon].alwaysSemi)
					{
						pm->ps->userInt1 |= UI1_ATTACKRELEASE;
					}
					if(weaponData[pm->ps->weapon2].alwaysSemi)
					{
						if(!(pm->ps->userInt1 & UI1_AKIMBOATTACKRELEASE))
							pm->ps->userInt1 |= UI1_AKIMBOATTACKRELEASE;
					}
				}
				else if(firingAkimbo)
				{ //Firing left-handed
					if(weaponData[pm->ps->weapon2].alwaysSemi)
					{
						if(!(pm->ps->userInt1 & UI1_AKIMBOATTACKRELEASE))
			...
			{
				if(pm->ps->stats[STAT_FMODE] == FM_SEMIAUTO || //We're currently in semi automatic mode or singles.
						pm->ps->stats[STAT_FMODE] == FM_SINGLE ||
						pm->ps->stats[STAT_FMODE] == FM_DOUBLE ||
						(weaponData[pm->ps->weapon].alwaysSemi && pm->cmd.buttons & BUTTON_ATTACK) ||
						(weaponData[pm->ps->weapon].altAlwaysSemi && pm->cmd.buttons & BUTTON_ALT_ATTACK)){
						pm->ps->userInt1 |= UI1_ATTACKRELEASE;
				} else if(pm->ps->stats[STAT_FMODE] == FM_THREEBURST ||
						pm->ps->stats[STAT_FMODE] == FM_FOURBURST){
							if(pm->ps->userInt1 & UI1_ATTACKRELEASE){
								//Do nothing.
							} else if(!(pm->ps->userInt1 & UI1_ONESHOTOFF)){
								pm->ps->userInt1 |= UI1_ONESHOTOFF;
							} else if(!(pm->ps->userInt1 & UI1_TWOSHOTOFF)){
							pm->ps->userInt1 |= UI1_TWOSHOTOFF;
							} else if(!(pm->ps->userInt1 & UI1_THREESHOTOFF)){
								pm->ps->userInt1 |= UI1_THREESHOTOFF;
								if(pm->ps->stats[STAT_FMODE] == FM_THREEBURST){
									pm->ps->userInt1 |= UI1_ATTACKRELEASE;
								}
							} else if(pm->ps->stats[STAT_FMODE] == FM_FOURBURST &&
								!(pm->ps->userInt1 & UI1_FOURSHOTOFF)) {
									pm->ps->userInt1 |= UI1_FOURSHOTOFF;
									pm->ps->userInt1 |= UI1_ATTACKRELEASE;
							} else if(!(pm->ps->userInt1 & UI1_ATTACKRELEASE)){
								//We're on a four round burst, and this is our last shot.
								pm->ps->userInt1 |= UI1_ATTACKRELEASE;
							}
				}
			}
		...
		if(!(gent->r.svFlags & SVF_BOT)){
			...
			if(firingBoth)
			{
				if(weaponData[weapon2].alwaysSemi)
				{
					pm->ps->userInt1 |= UI1_AKIMBOATTACKRELEASE;
				}
				if(weaponData[weapon].alwaysSemi)
				{
					pm->ps->userInt1 |= UI1_ATTACKRELEASE;
				}
			}
			else if(firingAkimbo && weaponData[weapon2].alwaysSemi)
			{
				pm->ps->userInt1 |= UI1_AKIMBOATTACKRELEASE;
			}
			else
...
#endif
		if(pm->cmd.buttons & BUTTON_ATTACK){
			gent->previousAttack = 1;
		} else if(pm->cmd.buttons & BUTTON_ALT_ATTACK){
			gent->previousAttack = 2;
		}
	}
#else
#ifdef AKIMBO
		...
		else if(firingAkimbo && weaponData[pm->ps->weapon2].alwaysSemi)
		{
			pm->ps->userInt1 |= UI1_AKIMBOATTACKRELEASE;
		}
#endif
		else if(pm->ps->stats[STAT_FMODE] == FM_SEMIAUTO ||
				pm->ps->stats[STAT_FMODE] == FM_SINGLE ||
				pm->ps->stats[STAT_FMODE] == FM_DOUBLE ||
				(weaponData[pm->ps->weapon].alwaysSemi && pm->cmd.buttons & BUTTON_ATTACK) ||
				(weaponData[pm->ps->weapon].altAlwaysSemi && pm->cmd.buttons & BUTTON_ALT_ATTACK)){
	...
	if (pm->ps->weapon == WP_EMPLACED_GUN)
	{
		addTime = weaponData[pm->ps->weapon].fireTime[pm->ps->stats[STAT_FPOS]];
	...
	{ //a vehicle NPC that has a pilot
		...
		else
		{
			...
			{
				G_CheapWeaponFire(pm->ps->clientNum, EV_FIRE_LEFT_WEAPON);
			}
#endif
		}
		//[CloakingVehicles]
	...
	if (weaponData[pm->ps->weapon].altFireScope &&
		(pm->cmd.buttons & BUTTON_ALT_ATTACK) &&
		!pm->ps->zoomLocked)
	{
		return;
	...
	else if (weaponData[pm->ps->weapon].altFireScope && pm->ps->zoomMode == 1)
	//if (pm->ps->weapon == WP_DISRUPTOR && pm->ps->zoomMode == 1)
	//[/CoOp]
	{
		PM_StartTorsoAnim( BOTH_ATTACK4 );
	}
	else if (pm->ps->weapon == WP_MELEE || pm->ps->weapon == WP_STUN_BATON)
	{ //special anims for standard melee attacks
		...
		{ //if riding a vehicle don't do this stuff at all
			...
			else if(pm->ps->weapon == WP_STUN_BATON && (pm->cmd.buttons & BUTTON_ALT_ATTACK))
				PM_StartTorsoAnim( BOTH_ATTACK2 );
			else
			{ //just punch
	...
	else
	{
		...
		if(pm->ps->weapon2)
		{
			if(firingAkimbo)
			{
				PM_StartTorsoAnim( GetIDForString(animTable, va("%s_LEFT", weaponData[pm->ps->weapon].akimboAttackAnim)) );
			}
			else if(firingBoth)
			{
				PM_StartTorsoAnim( GetIDForString(animTable, va("%s_BOTH", weaponData[pm->ps->weapon].akimboAttackAnim)) );
			}
			else
			{
				PM_StartTorsoAnim( GetIDForString(animTable, va("%s_RIGHT", weaponData[pm->ps->weapon].akimboAttackAnim)) );
			}
			PM_SetTorsoAnimTimer( 300 );
		}
		else
#endif
		{
			PM_StartTorsoAnim( /*WeaponAttackAnim[pm->ps->weapon]*/ GetIDForString(animTable, weaponData[pm->ps->weapon].attackAnim) );
			PM_SetTorsoAnimTimer( 300 );
		}
	}
	{
		...
		else
		{
			...
			if(pm->ps->weapon2)
			{
				amount = weaponData[pm->ps->weapon2].energyPerShot;
				altAttack = qfalse;
				firingAkimbo = qtrue;
			}
			else {
		...
		if(pm->ps->clientNum < MAX_CLIENTS)
		{
			...
			{
				amount = weaponData[pm->ps->weapon2].energyPerShot;
				if(weaponData[pm->ps->weapon2].clipSize)
				{
					if(pm->ps->weaponTime2 <= 0)
					{
						if ( pm->ps->stats[STAT_CLIP2] - amount >= 0 )
						{
							pm->ps->stats[STAT_CLIP2] -= amount;
						}
						else
						{
				...
				else
				{
					if(pm->ps->weaponTime2 <= 0)
					{
						if ( pm->ps->ammo[weaponData[pm->ps->weapon2].ammoIndex] - amount >= 0 )
						{
							pm->ps->ammo[weaponData[pm->ps->weapon2].ammoIndex] -= amount;
						}
						else
						{
							PM_AddEvent( EV_NOAMMO );
				...
				amount = weaponData[pm->ps->weapon].energyPerShot;
				if(weaponData[pm->ps->weapon].clipSize)
				{
#ifdef QAGAME
					if(g_infiniteAmmo.integer != 2)
					{
#endif
					if(pm->ps->weaponTime <= 0)
					{
						if ( pm->ps->stats[STAT_CLIP] - amount >= 0 )
						{
							pm->ps->stats[STAT_CLIP] -= amount;
						}
						else
		...
		{
			// enough energy to fire this weapon?
			if (altAttack && (pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] - amount) >= 0)
			{
				pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] -= amount;
			}
			else if(!altAttack && (pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] - amount) >= 0){
				pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] -= amount;
			}
	...
	if ( pm->cmd.buttons & BUTTON_ALT_ATTACK ) 	{
		...
		else
		{
			...
			if(pm->ps->weapon2 != WP_NONE)
			{
				...
				{ //do not fire melee events at all when on vehicle
					if(pm->cmd.buttons & BUTTON_ATTACK && pm->ps->weapon2 != WP_NONE)
					{
						if(pm->ps->weaponTime2 <= 0 && pm->ps->weaponTime <= 0)
						{
							PM_AddEvent( EV_FIRE_BOTH_GUNS );
							addTime = weaponData[pm->ps->weapon].fireTime[pm->ps->stats[STAT_FPOS]];
							addTimeSecond = weaponData[pm->ps->weapon2].fireTime[pm->ps->stats[STAT_FPOS]];
							firingBoth = qtrue;
						}
						else if(pm->ps->weaponTime2 > 0 && pm->ps->weaponTime <= 0)
						{
							PM_AddEvent( EV_FIRE_WEAPON );
							addTime = weaponData[pm->ps->weapon].fireTime[pm->ps->stats[STAT_FPOS]];
							firingBoth = qfalse;
						...
						{
							PM_AddEvent( EV_FIRE_LEFT_WEAPON );
							firingAkimbo = qtrue;
							firingBoth = qfalse;
							addTime = weaponData[pm->ps->weapon2].fireTime[pm->ps->stats[STAT_FPOS]];
						}
						else
							return;
					}
				...
				//addTime = weaponData[pm->ps->weapon].fireTime[pm->ps->stats[STAT_FPOS]]; //DO NOT do this shit!
			}
			else {
#endif
			if (pm->ps->weapon != WP_MELEE ||
			...
			addTime = weaponData[pm->ps->weapon].altFireTime[pm->ps->stats[STAT_FPOS]];
#ifdef AKIMBO
			}
#endif
		}
		...
		if(firingAkimbo)
		{
			addTime = weaponData[weapon2].fireTime[pm->ps->stats[STAT_FPOS]];
		}
		else
		{
#endif
	...
	*/
	...
	{
		pm->ps->weaponTime += addTime;
		pm->ps->weaponTime2 += addTimeSecond;
		BG_AccuracyDrain( pm, (weaponData[weapon2].accuracyDrain[pm->ps->stats[STAT_FPOS]] + weaponData[weapon].accuracyDrain[pm->ps->stats[STAT_FPOS]]) );
	}
	else if(firingAkimbo)
	{
		pm->ps->weaponTime2 += addTime;
		BG_AccuracyDrain( pm, weaponData[weapon2].accuracyDrain[pm->ps->stats[STAT_FPOS]] );
	}
	else {
#endif
	pm->ps->weaponTime += addTime;
	BG_AccuracyDrain( pm, weaponData[pm->ps->weapon].accuracyDrain[pm->ps->stats[STAT_FPOS]] );
#ifdef AKIMBO
	}
#endif
}
...
//-------------------------------------------
{
	...
	if(!pm->ps->weapon2)
	{
		if(pm->ps->stats[STAT_FMODE] == FM_THREEBURST && pm->ps->userInt1 & UI1_ONESHOTOFF && !(pm->ps->userInt1 & UI1_THREESHOTOFF))
		{
			if ( pm->ps->eFlags & EF_ALT_FIRING && !weaponData[pm->ps->weapon].altAlwaysSemi )
					pm->cmd.buttons |= BUTTON_ALT_ATTACK;
			else if( pm->ps->eFlags & EF_FIRING && !weaponData[pm->ps->weapon].alwaysSemi )
					pm->cmd.buttons |= BUTTON_ATTACK;
		}
		else if(pm->ps->stats[STAT_FMODE] == FM_THREEBURST && pm->ps->userInt1 & UI1_ONESHOTOFF && !(pm->ps->userInt1 & UI1_FOURSHOTOFF))
		{
			if ( pm->ps->eFlags & EF_ALT_FIRING && !weaponData[pm->ps->weapon].altAlwaysSemi )
					pm->cmd.buttons |= BUTTON_ALT_ATTACK;
			else if( pm->ps->eFlags & EF_FIRING && !weaponData[pm->ps->weapon].alwaysSemi )
	...
	{
		if( pm->cmd.buttons & BUTTON_SIGHTS || (pm->ps->zoomMode && weaponData[pm->ps->weapon].altFireScope) )
		{
			if ( !(pm->ps->userInt3 & IRONSIGHTS_MSB) )
			{
				//Com_Printf ("%d: Bringing ironsights up\n", pm->cmd.serverTime);
				pm->ps->userInt3 = pm->cmd.serverTime | IRONSIGHTS_MSB;
			}
		}
		else if ( pm->ps->userInt3 & IRONSIGHTS_MSB )
		{
...
#else
		amount = pm->ps->stats[STAT_CLIP] - weaponData[pm->ps->weapon].energyPerShot;
#endif
	}
	...
	if ( weaponData[pm->ps->weapon].altFireScope && pm->ps->weaponstate == WEAPON_READY )
	{
			...
			pm->cmd.upmove <= 0 && !pm->cmd.forwardmove && !pm->cmd.rightmove*/)
		{
			...
			if(weaponData[pm->ps->weapon].zoomType == 1)
			{
				//Using ruptor-style zoom
				if ( !pm->ps->zoomMode && pm->ps->pm_type != PM_DEAD )
				{
					// not already zooming, so do it now
					pm->ps->zoomMode = 1;
					pm->ps->zoomLocked = qfalse;
					pm->ps->zoomFov = weaponData[pm->ps->weapon].zoomStartFOV;//cg_fov.value;
				...
				{ //check for == 1 so we can't turn binoculars off with disruptor alt fire
					...
					pm->ps->zoomLocked = qfalse;
					PM_AddEvent(EV_DISRUPTOR_ZOOMSOUND);
					pm->ps->weaponTime = 0; //why the f*ck was this 1000...
				}
			}
			else if(weaponData[pm->ps->weapon].zoomType == 2)
			{
				//Using solid-state zoom
				if(!pm->ps->zoomMode && pm->ps->pm_type != PM_DEAD )
				{
					pm->ps->zoomMode = 1;
					pm->ps->zoomLocked = qfalse;
					pm->ps->zoomFov = weaponData[pm->ps->weapon].zoomStartFOV;
					pm->ps->zoomLockTime = pm->cmd.serverTime + 50;
					PM_AddEvent(EV_DISRUPTOR_ZOOMSOUND);
				}
				else if(pm->ps->zoomMode == 1 && pm->ps->zoomFov == weaponData[pm->ps->weapon].zoomStartFOV)
				{
					pm->ps->zoomFov = weaponData[pm->ps->weapon].zoomEndFOV;
					pm->ps->zoomLocked = qfalse;
					PM_AddEvent(EV_DISRUPTOR_ZOOMSOUND);
				}
			...
			else if(weaponData[pm->ps->weapon].zoomType == 3)
			{
				if(!pm->ps->zoomMode && pm->ps->pm_type != PM_DEAD )
				{
					pm->ps->zoomMode = 1;
		...
		else if ( !(pm->cmd.buttons & BUTTON_ALT_ATTACK ) && pm->ps->zoomLockTime < pm->cmd.serverTime && weaponData[pm->ps->weapon].zoomType == 1)
		{
			...
			if ( pm->ps->zoomMode )
			{
				...
				{ //approximate what level the client should be zoomed at based on how long zoom was held
					pm->ps->zoomFov = ((pm->cmd.serverTime+50) - pm->ps->zoomLockTime) * weaponData[pm->ps->weapon].zoomSpeed;
					/*if (pm->ps->zoomFov > 50)
					{
						pm->ps->zoomFov = 50;
					}
					...
					CAP(pm->ps->zoomFov, weaponData[pm->ps->weapon].zoomEndFOV);
					BUMP(pm->ps->zoomFov, 1);
				}
	...
	*/
	...
	if ( !(pm->ps->pm_flags & PMF_RESPAWNED) && 
			...
			( pm->cmd.buttons & (BUTTON_ATTACK|BUTTON_ALT_ATTACK)) && 
			( amount >= 0 || pm->ps->weapon == WP_SABER || (pm->ps->weapon == WP_STUN_BATON && pm->cmd.buttons & BUTTON_ATTACK) ||
			pm->ps->weapon == WP_MELEE))
	{
		if ( pm->cmd.buttons & BUTTON_ALT_ATTACK )
		{
...
{//racc - this function adjusts the client's movement speed based on the situation.
	...
	//[Stamina]
	pm->ps->eFlags &= ~EF_SPRINTING;
	if (cmd->forwardmove > 0 && cmd->buttons&BUTTON_SPRINT && pm->ps->groundEntityNum != ENTITYNUM_NONE && cmd->upmove == 0 &&
		ps->stats[STAT_STAMINA] > 20){
			ps->stats[STAT_STAMINA] -= 2;
			pm->ps->weaponstate = WEAPON_DROPPING;
			ps->speed *= 1.7;
			BG_AccuracyDrain( pm, 100 );
			pm->ps->eFlags |= EF_SPRINTING;
	} else if(cmd->buttons & BUTTON_SPRINT && (pm->ps->groundEntityNum != ENTITYNUM_NONE || cmd->upmove != 0 || ps->stats[STAT_STAMINA] <= 20)){
	...
	{
		if(ps->fd.forcePowerLevel[FP_SPEED] == 1)
			ps->speed *= 4.5;
		else
			ps->speed *= 1.7;
	}
					   ...
					   entityState_t *emplaced, int *crazySmoothFactor)
{
	...
	if ((( cent->legsAnim ) != BOTH_STAND1) || 
		( cent->torsoAnim ) != /*WeaponReadyAnim[cent->weapon]*/ GetIDForString(animTable, weaponData[cent->weapon].readyAnim)  ) 
	{
...
void PmoveSingle (pmove_t *pmove) {
	...
	*/
	...
	/*if (weaponData[pm->ps->weapon].altFireScope && pm->ps->weaponstate == WEAPON_CHARGING_ALT)
	{ //not allowed to move while charging the disruptor
		if (pm->cmd.forwardmove ||
			pm->cmd.rightmove ||
			pm->cmd.upmove > 0)
