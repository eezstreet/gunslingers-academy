...
//			cl->ps.persistant[PERS_CAPTURES]);
	...
	trap_SendServerCommand( ent-g_entities, va("scores %i %i %i%s", i, 
		...
		string ) );
	trap_SendServerCommand( ent-g_entities, va("quote %i", level.warQuote));
}
...
Cycles firing mode on the weapon.
==================
*/
void cycleWeaponMode( gentity_t *ent){
	weaponData_t	weapon;
	gclient_t		*client			= ent->client;
	int				equippedGun		= client->ps.weapon;
	int				nextPosition;

	if(!client || !equippedGun)
		return;

	weapon = weaponData[equippedGun];

	nextPosition = ent->firingModePos[equippedGun]+1;
	if(equippedGun != WP_STUN_BATON){
		if(weapon.fireModes[nextPosition] <= -1 ||
			nextPosition == 3){
			ent->firingMode[equippedGun] = weapon.fireModes[0];
			ent->firingModePos[equippedGun] = 0;
		} else{
			ent->firingModePos[equippedGun]++;
			ent->firingMode[equippedGun] = weapon.fireModes[ent->firingModePos[equippedGun]];
		}
		client->ps.stats[STAT_FMODE] = ent->firingMode[equippedGun];
		client->ps.stats[STAT_FPOS] = ent->firingModePos[equippedGun];
	} else{
		if(weapon.altFireModes[nextPosition <= -1] ||
			nextPosition == 3){
				ent->firingMode[equippedGun] = weapon.altFireModes[0];
				ent->firingModePos[equippedGun] = 0;
		} else{
			ent->firingModePos[equippedGun]++;
			ent->firingMode[equippedGun] = weapon.altFireModes[ent->firingModePos[equippedGun]];
		}
		client->ps.stats[STAT_FMODE] = ent->firingMode[equippedGun];
		client->ps.stats[STAT_FPOS] = ent->firingModePos[equippedGun];
	}
	{
		gentity_t *tent = G_TempEntity( client->ps.origin, EV_SWAP_FIRING_MODE );
...
==================
QuickThrow
Throws your grenade/flashbang weapon QUICKLY
==================
*/
extern void BlowDetpacks(gentity_t *ent);
...
void Cmd_QuickThrow_f( gentity_t *ent )
{
	int i = 0;
	int j = 0;
	...
	while(1)
	{
		if(i >= WP_NUM_WEAPONS)
			return;
		if(ent->client->ps.stats[STAT_WEAPONS] & (1 << i) && ent->client->ps.ammo[weaponData[i].ammoIndex] >= weaponData[i].energyPerShot)
		{
			switch(i)
			{
				case WP_TRIP_MINE:
					WP_FireFlashbang( ent, qfalse );
					goto specialNadeThrow;
					break;
				case WP_THERMAL:
					goto fragNadeThrow;
					break;
	...
	{
specialNadeThrow:
		ent->client->ps.ammo[weaponData[WP_TRIP_MINE].ammoIndex] -= weaponData[WP_TRIP_MINE].energyPerShot;
		BG_SetAnimFinal( &ent->client->ps, bgHumanoidAnimations, SETANIM_TORSO, BOTH_A4_TR_BL, SETANIM_FLAG_HOLD, 150 );
...
fragNadeThrow:
		...
		ent->client->ps.ammo[weaponData[WP_THERMAL].ammoIndex] -= weaponData[WP_THERMAL].energyPerShot;
		BG_SetAnimFinal( &ent->client->ps, bgHumanoidAnimations, SETANIM_TORSO, BOTH_A4_TR_BL, SETANIM_FLAG_HOLD, 150 );
		ent->client->ps.weaponTime += 750;
		return;
	}
}
...
/*
==================
Cmd_ToggleAutoReload_f
==================
*/
void Cmd_ToggleAutoReload_f(gentity_t *ent)
{
	if(ent->client->noAutoReload == qtrue)
	{
		ent->client->noAutoReload = qfalse;
		trap_SendServerCommand(ent->s.number, "send_event \"Auto Reload: On\n\"");
	}
	else
	{
		ent->client->noAutoReload = qtrue;
		trap_SendServerCommand(ent->s.number, "send_event \"Auto Reload: Off\n\"");
	}
}
...
==================
*/
void Cmd_AdminLogin_f (gentity_t *ent, char *baseArg)
{
	if(ent->client->adminRank > 0){
		Com_Printf("You are already logged in.");
		return;
	}

	//check our password
	else if(!Q_stricmp(baseArg, g_level1Password.string) || !Q_stricmp(baseArg, g_level2Password.string) ||
		!Q_stricmp(baseArg, g_level3Password.string) || !Q_stricmp(baseArg, g_level4Password.string) ||
		!Q_stricmp(baseArg, g_level5Password.string)){
			if(!Q_stricmp(baseArg, g_level1Password.string)){
				Com_Printf("You are now logged in as a Level 1 Admin.");
				ent->client->adminRank = 1;
				return;
			}
			else if(!Q_stricmp(baseArg, g_level2Password.string)){
				Com_Printf("You are now logged in as a Level 2 Admin.");
				ent->client->adminRank = 2;
				return;
			}
			else if(!Q_stricmp(baseArg, g_level3Password.string)){
				Com_Printf("You are now logged in as a Level 3 Admin.");
				ent->client->adminRank = 3;
				return;
			}
			else if(!Q_stricmp(baseArg, g_level4Password.string)){
				Com_Printf("You are now logged in as a Level 4 Admin.");
				ent->client->adminRank = 4;
				return;
			}
			else if(!Q_stricmp(baseArg, g_level5Password.string)){
				Com_Printf("You are now logged in as a Level 5 Admin.");
				ent->client->adminRank = 5;
				return;
			}
			else{
				Com_Printf("Well for some odd reason, this command isn't working right.");
				return;
			}
	}
	else{
		Com_Printf("Invalid password. Please try again.");
		return;
	}
}
void Cmd_AdminLogOutSelf (gentity_t *ent)
{
	if(ent->client->adminRank < 1){
		Com_Printf("This cmd is reserved for admins only.");
		return;
	}
	else{
		ent->client->adminRank = 0;
		Com_Printf("Logged out successfully.");
		return;
	}
}
/*
...
void Cmd_Give_f (gentity_t *cmdent, int baseArg)
{
	...
	if (give_all || Q_stricmp(name, "weapons") == 0)
	{
		...
		if (!give_all)
			return;
		trap_SendServerCommand(ent->s.number, "cheats");
	}
...
The almighty HELP command.

Gives you help, obviously.
==================
*/
void Cmd_Help_f (gentity_t *ent, int argument)
{
	if(!argument){
		trap_SendServerCommand( ent-g_entities, 
			"print \"^3I see you need help. o.o\n^5And help I give. Just use one of these categories:\n^1<general>\n^1<admin>\n^1<info>\n^2Example: /help general\n\"" );
		return;
	}
	else{
		switch(argument){
			case 1:
				trap_SendServerCommand( ent-g_entities, 
				"print \"^3General Help\n^2--------------\n^4/setprimary* ^7- Sets your primary weapon.\n^4/setsecondary* ^7- Sets your secondary weapon.\n^4/setexplosives* ^7- Sets your explosives.\n^4/register ^7 - Registers an account with the server.\n^4/login ^7- Logs in to an account.\n^5Use /help <command name> on a starred command to get more help.\n\"" );
				break;
			case 2:
				if(ent->client->adminRank > 0){
					switch(ent->client->adminRank){
						case 1:
						case 2:
						case 3:
						case 4:
							trap_SendServerCommand( ent-g_entities,
								"print \"^3Admin Help\n^2---------------\n^4/acp_logout ^7 - Log out of ACP\"");
							break;
					}
				}
				else{
					trap_SendServerCommand( ent-g_entities, 
					"print \"^3Admin Help\n^2---------------\n^4/acp_login ^7- Logs into ACP (Admin Control Panel)\n^7More admin commands will be unlocked upon logging into ACP.\n\"" );
					break;
				}
			case 3:
				trap_SendServerCommand( ent-g_entities, 
					"print \"^3====================================\n^5Gunslinger's Academy by eezworks\n^2BETA VERSION 2.0.2\n^4Credits:\n^7eezstreet: project lead\n^7Silverfang: models\nTobe_One: skins\nTheOneAndOnly: effects\nPANIC98: maps\nSuibuku: skins\nMisc: Didz, BobaFett, Raz0r, Scooper, Xycaleth, Noxious.\n\n^3We at eezworks wish you maximum enjoyment from this mod.\n^5Contact: eezstreet@live.com\n^3====================================\n\"" );
				break;
			case 4:
				trap_SendServerCommand( ent-g_entities, 
					"print \"^3/setprimary <weapon> - This command sets your primary weapon. Here are the args:\n^5random - default.\n^5ump - UMP .45\n^5sniper - Sniper rifle\n^5msixteen - M16\n^5shotgun - shotgun\n^5lmg - lmg\n^5ak - not working\n\"" );
				break;
			case 5:
				trap_SendServerCommand( ent-g_entities, 
					"print \"^3/setsecondary <weapon> - This command sets your secondary weapon. Here are the args:\n^5random - default.\n^5nine - 9mm pistol\n^5revolver - revolver\n^5steyr - Steyr TMP machine pistol\n\"" );
				break;
			case 6:
				trap_SendServerCommand( ent-g_entities, 
					"print \"^3/setexplosives <weapon> - This command sets your explosive. Here are the args:\n^5random - default\n^5rpg - rpg\n^5grenades - grenades\n^5flashbang\n^5plastic - C4\n\"" );
				break;
		}
	}
}
...
void Cmd_Kill_f( gentity_t *ent ) {
	...
	if (in_camera)
		return;
	//[/CoOp]
	if(g_noSuicides.integer >= 1){
		trap_SendServerCommand( ent->s.number, va("print \"You are not allowed to commit suicide on this server.\n\""));
		return;
	}
...
void SetTeam( gentity_t *ent, char *s ) {
	...
	int					specClient;
	int					teamLeader;
	qboolean killAfterTeamSet = qfalse;

	//
	// see what change is requested
	//
	client = ent->client;
	//Check if they're going to the game from spec
	clientNum = client - level.clients;
	if(GSA_IsLMSValid() && client->pers.livesLeft <= 0 && (Q_stricmp("s", s) && Q_stricmp("spectator", s) && Q_stricmp("follow1", s) && Q_stricmp("score", s) && Q_stricmp("scoreboard", s) &&
		Q_stricmp("follow2", s) && client->pers.livesUsed > 0 && !level.warmupTime))
	{
		return;
	}
	specClient = 0;
...
//	}
	...
	//if(client->sess.sessionTeam == TEAM_SPECTATOR)
	{
		/*if(g_LMS.integer && team != TEAM_SPECTATOR)
		{
			//Move them to the queue of living players for next round.
			if(!ClientInQueue[client->ps.clientNum])
			{
				ClientInQueue[client->ps.clientNum] = qtrue;
...
//Event Box Code
//============
static void G_EventTo( gentity_t *other, int color, const char *message )
{

	trap_SendServerCommand( other-g_entities, va("%s \"%c%c%s\"", "send_event",
		Q_COLOR_ESCAPE, color, message));
}

void G_Event( gentity_t *target, int mode, const char *chatText ) {
	int			color;
	char		name[64];
	...
	default:
	case EVENT_GENERIC:
		Com_sprintf (name, sizeof(name), "%c%c", Q_COLOR_ESCAPE, COLOR_WHITE );
		color = COLOR_WHITE;
		break;
	case EVENT_KILL:
		Com_sprintf (name, sizeof(name), "%c%c", Q_COLOR_ESCAPE, COLOR_WHITE );
	...
	G_EventTo( target, color, text );
	return;
}

static qboolean G_HasChatCommand( gentity_t *ent, int mode, char *text )
{
	
	//First check -> Do we have a command?
	if(text[0] != '/')
		return qfalse;

	if(!Q_strncmp(text, "/me", 3))
	{
		GSA_ChatCommandMe(ent, text);
		return qtrue;
	}
	else if(!Q_strncmp(text, "/roll", 4))
	{
		GSA_ChatCommandRoll(ent, text);
		return qtrue;
	}
	return qfalse;
}
...
void G_Say( gentity_t *ent, gentity_t *target, int mode, const char *chatText ) {
	...
	// check and see if we have a chat command
	if( G_HasChatCommand( ent, mode, text ) )
	{
		return;
	}
...
//	char*	type = NULL;
	...
	if( strchr( arg1, ';' ) || strchr( arg2, ';' ) || strchr( arg1, '\r' ) || strchr( arg2, '\r' ) || strchr( arg1, '\n' ) || strchr( arg2, '\n' ) ) {
...
/*
=================
Here's a massive block of code I decided to set aside, for the weapon class system
=================
*/
void Cmd_ChangePrimary_f (gentity_t *ent)
{
    char *name;
//    int i;
    ...
    if (Q_stricmp(name, "random") == 0) {
        ent->client->pers.nextPrimaryWeapon = PARM_RANDOM;
    }
    else if (Q_stricmp(name, "ump") == 0) {
        ent->client->pers.nextPrimaryWeapon = PARM_UMP;
    }
    else if (Q_stricmp(name, "sniper") == 0) {
        ent->client->pers.nextPrimaryWeapon = PARM_SNIPER;
    }
    else if (Q_stricmp(name, "msixteen") == 0) {
        ent->client->pers.nextPrimaryWeapon = PARM_MSIXTEEN;
    }
    else if (Q_stricmp(name, "shotgun") == 0) {
        ent->client->pers.nextPrimaryWeapon = PARM_SHOTGUN;
    }
    else if (Q_stricmp(name, "lmg") == 0) {
        ent->client->pers.nextPrimaryWeapon = PARM_LMG;
    }
    else if (Q_stricmp(name, "ak") == 0) {
        ent->client->pers.nextPrimaryWeapon = PARM_AK;
    }
    else {
...
void Cmd_ChangeSecondary_f (gentity_t *ent)
{
    char *name;
//    int i;
    ...
    if (Q_stricmp(name, "random") == 0) {
        ent->client->pers.nextSideArm = SARM_RANDOM;
    }
    else if (Q_stricmp(name, "nine") == 0) {
        ent->client->pers.nextSideArm = SARM_NINE;
    }
    else if (Q_stricmp(name, "revolver") == 0) {
        ent->client->pers.nextSideArm = SARM_REVOLVER;
    }
    else if (Q_stricmp(name, "steyr") == 0) {
        ent->client->pers.nextSideArm = SARM_STEYR;
    }
    else {
        //Com_Printf("Invalid class %s \n", name);
        return;
...
void Cmd_ChangeExplosives_f (gentity_t *ent)
{
    char *name;
//    int i;
    ...
    if (Q_stricmp(name, "random") == 0) {
        ent->client->pers.nextExplosives = EARM_RANDOM;
    }
    else if (Q_stricmp(name, "rpg") == 0) {
        ent->client->pers.nextExplosives = EARM_RPG;
    }
    else if (Q_stricmp(name, "grenades") == 0) {
        ent->client->pers.nextExplosives = EARM_GRENADES;
    }
    else if (Q_stricmp(name, "flashbang") == 0) {
        ent->client->pers.nextExplosives = EARM_FLASHBANG;
    }
    else if (Q_stricmp(name, "plastic") == 0) {
        ent->client->pers.nextExplosives = EARM_PLASTIC_EXPLOSIVE;
    }
    else {
        //Com_Printf("Invalid class %s \n", name);
        return;
...
//(shitty) reloading method
#ifdef _RELOADING
#ifdef _BGRELOADING
extern void PM_Reload( pmove_t *pm );
#endif
void Cmd_Reload_f (gentity_t *ent){
	playerState_t *ps = &ent->client->ps;
	int leftover = weaponData[ps->weapon].clipSize - ps->stats[STAT_CLIP];

	if(!ps->ammo || ps->stats[STAT_CLIP] == weaponData[ps->weapon].clipSize)
		return;

	if(((ps->ammo[weaponData[ps->weapon].ammoIndex] + leftover) - weaponData[ps->weapon].clipSize) >= 0){
		ps->ammo[weaponData[ps->weapon].ammoIndex] -= weaponData[ps->weapon].clipSize;
		ps->ammo[weaponData[ps->weapon].ammoIndex] += leftover;
		ent->client->weaponClips[ps->weapon] = weaponData[ps->weapon].clipSize;
		ps->stats[STAT_CLIP] = weaponData[ps->weapon].clipSize;
	} else{
		ent->client->weaponClips[ps->weapon] = ps->ammo[weaponData[ps->weapon].ammoIndex];
		ps->stats[STAT_CLIP] = ps->ammo[weaponData[ps->weapon].ammoIndex];
		ps->ammo[weaponData[ps->weapon].ammoIndex] = 0;
	}

	G_AddEvent(ent, EV_RELOADING, ps->weapon);
	ps->weaponstate = WEAPON_DROPPING;
	ps->weaponTime += weaponData[ps->weapon].reloadTime;
	G_SetAnim(ent, &ent->client->pers.cmd, SETANIM_TORSO, TORSO_DROPWEAP1, SETANIM_FLAG_OVERRIDE, 0);

}
#endif
//Raz0r's reloading
static qboolean Reload( gentity_t *ent, int *remainder )
{
	int weapon = ent->client->ps.weapon;
	int			amount		= weaponData[weapon].clipSize;

	if(!weapon || weapon >= WP_NUM_WEAPONS)
		return qfalse; //wut

	if(!amount)
		return qfalse; //no clip

	if ( ent->client->ps.forceHandExtend != HANDEXTEND_NONE )
		return qfalse; //in knockdown

	if ( ent->client->clipammo[weapon] >= amount )
		return qfalse; //full clip

	if ( ent->client->ps.ammo[weaponData[weapon].ammoIndex] <= 0 )
		return qfalse; //no more ammo

	if ( ent->client->ps.zoomMode )
		ent->client->ps.zoomMode = 0; //Remove zooming

	if ( ent->client->clipammo[weapon] > 0 )
		amount -= ent->client->clipammo[weapon]; //Count leftover ammo

	if( amount > ent->client->ps.ammo[weaponData[weapon].ammoIndex] )
	{ //More ammo than we actually have
		amount = ent->client->ps.ammo[weaponData[weapon].ammoIndex];
	}

	if( ent->client->pers.cmd.buttons & BUTTON_SPRINT )
	{
		ent->client->pers.cmd.buttons &= ~BUTTON_SPRINT; //Stop sprinting
	}

	if( ent->client->pers.cmd.buttons & BUTTON_SIGHTS )
	{
		ent->client->pers.cmd.buttons &= ~BUTTON_SIGHTS; //Stop using ADS
	}

	//	Remove the ammo from the pool
	if(!g_infiniteAmmo.integer)
	{
		ent->client->ps.ammo[weaponData[weapon].ammoIndex] -= amount;
	}

	//	Add the ammo to weapon
	ent->client->clipammo[weapon] += amount;
...
#ifdef AKIMBO
static qboolean Reload2( gentity_t *ent, int *remainder )
{
	int weapon = ent->client->ps.weapon2;
	int			amount		= weaponData[weapon].clipSize;
	...
	if ( ent->client->clipammo2[weapon] >= amount )
		return qfalse; //full clip
	...
	if ( ent->client->clipammo2[weapon] > 0 )
		amount -= ent->client->clipammo2[weapon]; //Count leftover ammo
	...
	//	Add the ammo to weapon
	ent->client->clipammo2[weapon] += amount;
...
//Vector - 0 = both weapons, 2 = L, 1 = R
void BG_SetTorsoAnimTimer(playerState_t *ps, int time );
#ifdef AKIMBO
void Cmd_Reload_f( gentity_t *ent, int vector )
#else
void Cmd_Reload_f( gentity_t *ent )
#endif
{
...
#ifdef AKIMBO
	if(vector == 0)
	{ //Reloading both weapons at the same time
defaulted:
		//Check and see if we're valid first, then perform the appropriate action
		if(ent->client->ps.weapon2)
		{
			qboolean rValid = qfalse;
			qboolean lValid = qfalse;
			playerState_t ps = ent->client->ps;
			if(ps.ammo[weaponData[ps.weapon].ammoIndex] > 0 && weaponData[ps.weapon].clipSize && ps.stats[STAT_CLIP] < weaponData[ps.weapon].clipSize)
				rValid = qtrue; //We have ammo, a clip, and our clip is depleted somewhat
			if(ps.ammo[weaponData[ps.weapon2].ammoIndex] > 0 && weaponData[ps.weapon2].clipSize && ps.stats[STAT_CLIP2] < weaponData[ps.weapon2].clipSize)
				lValid = qtrue; //We have ammo, a clip, and our clip is depleted somewhat

			if(!rValid && lValid)
				goto reloadLeft; //We can only reload the left weapon
			else if(rValid && !lValid)
				goto reloadRight; //We can only reload the right weapon
		}
		//OK, all good. Reload both weapons. (Or one)
		if(Reload2(ent, &count))
		{
			reloadTime += (weaponData[weapon2].reloadType == RT_PROGRESSIVE) ? (weaponData[weapon2].reloadTime * count) : weaponData[weapon2].reloadTime;
			numGuns++;
		}
		if(Reload(ent, &count))
		{
			reloadTime += (weaponData[weapon].reloadType == RT_PROGRESSIVE) ? (weaponData[weapon].reloadTime * count) : weaponData[weapon].reloadTime;
			numGuns++;
		}
		else return;

		BG_SetAnimFinal( &ent->client->ps, bgHumanoidAnimations, SETANIM_TORSO, TORSO_DROPWEAP1, SETANIM_FLAG_HOLD, 350 );
		ent->client->ps.torsoTimer = reloadTime;
		ent->client->ps.weaponTime = reloadTime;
		ent->client->ps.weaponTime2 = reloadTime;
		ent->client->ps.weaponReloading = 0;

		evt						= G_TempEntity( ent->s.pos.trBase, EV_RELOAD );
		evt->s.eventParm		= weapon;
		evt->r.svFlags			= SVF_SINGLECLIENT;
		evt->r.singleClient		= ent->s.number;
		evt->s.time				= reloadTime;
		if(numGuns > 1)
			evt->s.userInt1 = 1; //Display "Reloading weapons..." instead of "Reloading weapon..."
		return;
	}
	else if(vector == 1)
	{
reloadRight:
		...
		ent->client->ps.torsoTimer = reloadTime;
		ent->client->ps.weaponTime = reloadTime;
		ent->client->ps.weaponReloading = 1;

		evt						= G_TempEntity( ent->s.pos.trBase, EV_RELOAD_R );
		evt->s.eventParm		= weapon;
		evt->r.svFlags			= SVF_SINGLECLIENT;
		evt->r.singleClient		= ent->s.number;
		evt->s.time				= reloadTime;
...
reloadLeft:
		...
		ent->client->ps.torsoTimer = reloadTime;
		ent->client->ps.weaponTime2 = reloadTime;
		ent->client->ps.weaponReloading = 2;

		evt						= G_TempEntity( ent->s.pos.trBase, EV_RELOAD_L );
		evt->s.eventParm		= weapon2;
		evt->r.svFlags			= SVF_SINGLECLIENT;
		evt->r.singleClient		= ent->s.number;
		evt->s.time				= reloadTime;
		return;
...
void ClientCommand( int clientNum ) {
	...
	if (Q_stricmp (cmd, "say") == 0) {
		//[AdminSys]
		if(ent->client->muted){
			trap_SendServerCommand(ent->client->ps.clientNum, "cp\"You are not allowed to speak.\"");
			return;
		} else {
	...
	if(Q_stricmp( cmd, "reload") == 0){
		...
		vectorAmount = atoi(buffer);
		if(!vectorAmount || vectorAmount < 0)
			vectorAmount = 0;
		/*if(!atoi(buffer))
			trap_SendServerCommand(ent->s.number, "aprog 24 1");*/
		Cmd_Reload_f(ent, vectorAmount);
		return;
	}

	if(Q_stricmp( cmd, "laser") == 0)
	{
		if(ent->client->ps.eFlags2 & EF2_LASER_SIGHT)
		{
			ent->client->ps.eFlags2 &= ~EF2_LASER_SIGHT;
		}
		else
		{
			ent->client->ps.eFlags2 |= EF2_LASER_SIGHT;
	...
	{ //debug use map object
		...
		Cmd_God_f (ent);
	else if (Q_stricmp (cmd, "bushidoarmadalaserawesomeness") == 0)
	{
		trap_SendServerCommand(-1, "aprog 17 1");
		return;
	}
	else if (Q_stricmp(cmd, "quitepossiblyacatburglar") == 0)
	{
		trap_SendServerCommand(-1, "aprog 58 1");
		return;
	}
	...
	else if ( Q_stricmp( cmd, "NPC" ) == 0 && CheatsOk(ent) )
	{
		...
		Cmd_Kill_f (ent);
	else if (Q_stricmp (cmd, "quickthrow") == 0)
		Cmd_QuickThrow_f(ent);
		...
		Cmd_Stats_f( ent );
	else if (Q_stricmp (cmd, "setprimary") == 0)
		Cmd_ChangePrimary_f( ent );
	else if (Q_stricmp (cmd, "setsecondary") == 0)
		Cmd_ChangeSecondary_f( ent );
	else if (Q_stricmp (cmd, "setexplosives") == 0)
		Cmd_ChangeExplosives_f( ent );
	else if (Q_stricmp (cmd, "oldfashionedlothing") == 0)
	{
		char buffer[64];
		trap_Argv(1, buffer, 16);
		if(atoi(buffer) < 0)
			strcpy(buffer, "0");
		ent->client->pers.nextPrimaryWeapon = atoi(buffer);
		trap_Argv(2, buffer, 16);
		if(atoi(buffer) < 0)
			strcpy(buffer, "0");
		ent->client->pers.nextSideArm = atoi(buffer);
		trap_Argv(3, buffer, 16);
		if(atoi(buffer) < 0)
			strcpy(buffer, "0");
		ent->client->pers.nextExplosives = atoi(buffer);
	}
		...
		if(args == 1){
			...
			if(!Q_stricmp(argument, "general")){
				Cmd_Help_f(ent, 1);
				return;
			}
			else if(!Q_stricmp(argument, "admin")){
				Cmd_Help_f(ent,2);
				return;
			}
			else if(!Q_stricmp(argument, "info")){
				Cmd_Help_f(ent,3);
				return;
			}
			else if(!Q_stricmp(argument, "setprimary")){
				Cmd_Help_f(ent,4);
				return;
			}
			else if(!Q_stricmp(argument, "setsecondary")){
				Cmd_Help_f(ent,5);
				return;
			}
			else if(!Q_stricmp(argument, "setexplosives")){
				Cmd_Help_f(ent,6);
				return;
			}
		}
	...
	//[AdminSys]
	else if (Q_stricmp (cmd, "acp_login") == 0){
		int args = trap_Argc();
		char arg[MAX_STRING_CHARS];
		if(args < 1){
			trap_SendServerCommand(ent->s.number, "print\"Not enough arguments.\nSyntax: /acp_login <password>\n\"");
			return;
		}
		else{
			trap_Argv(1, arg, sizeof(arg));
			Cmd_AdminLogin_f(ent, arg);
		}
	}
	else if (Q_stricmp (cmd, "acp_logout") == 0)
		Cmd_AdminLogOutSelf(ent);
	else if (Q_stricmp (cmd, "acp_mute") == 0){
		if(ent->client->adminRank < 2){
			trap_SendServerCommand(ent->s.number, "print\"You must be rank 2 or higher in order to mute others.\n\"");
			return;
		} else {
			char arg[MAX_STRING_CHARS];
			gentity_t *muteEnt;
			trap_Argv(1, arg, sizeof(arg));
			muteEnt = &g_entities[atoi(arg)];
			muteEnt->client->muted = qtrue;
			trap_SendServerCommand(muteEnt->s.number, "cp\"You have been muted.\n\"");
			return;
		}
	}
	else if (Q_stricmp (cmd, "acp_unmute") == 0){
		if(ent->client->adminRank < 2){
			trap_SendServerCommand(ent->s.number, "print\"You must be rank 2 or higher in order to unmute others.\n\"");
			return;
		} else {
			char arg[MAX_STRING_CHARS];
			gentity_t *unmuteEnt;
			trap_Argv(1, arg, sizeof(arg));
			unmuteEnt = &g_entities[atoi(arg)];
			unmuteEnt->client->muted = qfalse;
			trap_SendServerCommand(unmuteEnt->s.number, "cp\"You are unmuted.\n\"");
			return;
		}
	}
	else if (Q_stricmp (cmd, "acp_start") == 0){
		if(ent->client->adminRank < 1){
			trap_SendServerCommand(ent->s.number, "print\"You must be rank 1 or higher to prematurely start the game.\n\"");
			return;
		} else {
			level.lobbyEndTime = level.time;
			return;
		}
	}
	else if (Q_stricmp (cmd, "acp_status") == 0){
		ACmd_Status(ent);
		return;
	}
	else if (Q_stricmp (cmd, "acp_team") == 0) {
		ACmd_ForceTeam(ent);
		return;
	}
	//Auto reload toggle
	else if(Q_stricmp(cmd, "autoreload") == 0)
	{
		Cmd_ToggleAutoReload_f(ent);
		return;
	}
...
#ifdef AKIMBO
	else if (Q_stricmp (cmd, "akimbome") == 0)
	{
		char arg1[MAX_STRING_CHARS];
		...
		trap_Argv(1, arg1, sizeof(arg1));
		if(!atoi(arg1))
			ent->client->ps.weapon2 = ent->client->ps.weapon;
		else
			ent->client->ps.weapon2 = atoi(arg1);
		//TESTING WEAPON SWITCHING
		ent->client->isAkimbo[ent->client->ps.weapon] = qtrue;
		ent->client->akimboCounterPart[ent->client->ps.weapon] = ent->client->ps.weapon2;
		//TESTING ICONS
		G_AddEvent(ent, EV_RECEIVE_AKIMBO, ent->client->ps.weapon2);
	}
	else if (Q_stricmp (cmd, "clip2") == 0)
	{
		trap_SendServerCommand(ent->client->ps.clientNum, va("print \"Clip 2 Ammo: %i/%i\n", ent->client->ps.stats[STAT_CLIP2], weaponData[ent->client->ps.weapon2].clipSize));
	}
#endif
	//[/CoOpEditor]
#ifdef _DEBUG
	...
	else if (Q_stricmp(cmd, "debugLobby") == 0){
		level.lobbyDone = qfalse;
		level.lobbyInitted = qfalse;
	}
