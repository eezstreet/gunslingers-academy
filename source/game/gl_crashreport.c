...
static const char *JKG_Crash_GetCrashlogName() {
	...
	strftime(Buff, 1024,"GSA-CLCrashlog_%Y-%m-%d_%H-%M-%S.log",timeinfo);
	return Buff;
}
...
{
	iCom_Error(0, "Gunslinger's Academy has encountered a fatal error and was forced to shut down.\nOur apologies for the inconvenience.");
...
static LONG WINAPI UnhandledExceptionHandler (struct _EXCEPTION_POINTERS *EI /*ExceptionInfo*/) {
	...
	Com_Printf("Please contact the GSA development team with this crash log and specify the circumstances in which this crash occurred.\n");
