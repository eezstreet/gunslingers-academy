...
void AddTeamScore(vec3_t origin, int team, int score) {
	...
	if( g_gametype.integer == GT_TEAM || g_gametype.integer == GT_DOMINATION || g_gametype.integer == GT_HERO){
	...
	for (i = 0; i < g_maxclients.integer; i++) {
		...
		if (player->client->sess.sessionTeam !=
			...
			if (player->client->pers.teamState.lastreturnedflag + 
				...
				other->client->pers.teamState.assists += 5;

				player->client->ps.persistant[PERS_ASSIST_COUNT] += 5;
				player->client->rewardTime = level.time + REWARD_SPRITE_TIME;
				trap_SendServerCommand(-1, va("send_event \"%s assisted in the capture.\"", player->client->pers.netname));
				trap_SendServerCommand(-1, va("print \"%s assisted in the capture.\"", player->client->pers.netname));
				...
				player->client->ps.persistant[PERS_ASSIST_COUNT] += 5;
				player->client->rewardTime = level.time + REWARD_SPRITE_TIME;
				trap_SendServerCommand(-1, va("send_event \"%s assisted in the kill.\"", player->client->pers.netname));
				trap_SendServerCommand(-1, va("print \"%s assisted in the capture.\"", player->client->pers.netname));
			}
		}
	}
