...
//[VIP/Commander Escort]
//==========================================================
/*QUAKED target_escape_point (1 0 0) (-8 -8 -8) (8 8 8)
VIP/Commander Escort only.
Escape point to flee to.
*/
void Use_target_escape_point( gentity_t *ent, gentity_t *other, gentity_t *activator){
	if( activator->s.number != level.vipPlayer && activator->s.number != level.redTeamCommander && activator->s.number != level.blueTeamCommander){
		return; //non-VIPs don't do anything
	}
	if( g_gametype.integer == GT_VIP_ESCORT ){
		AddTeamScore(activator->client->ps.origin, TEAM_BLUE, 1);
		LogExit("The VIP has reached the safe zone.");
		//BeginIntermission();
	}else if(g_gametype.integer == GT_COMMANDER ){
		if(ent->s.teamowner == activator->client->sess.sessionTeam){
			AddTeamScore(activator->client->ps.origin, activator->client->sess.sessionTeam, 1);
			LogExit(va("%s Commander has reached the safe zone.", TeamName(activator->client->sess.sessionTeam)));
			//BeginIntermission();
		}
	}
	ent->use = NULL;
...
void SP_target_escape_point( gentity_t *ent ) {
	ent->use = Use_target_escape_point;
}
...
void Use_Target_Print (gentity_t *ent, gentity_t *other, gentity_t *activator)
{
	...
	if (ent->message[0] == '@' && ent->message[1] != '@')
	{
		if( ent->spawnflags & 16 ){
			trap_SendServerCommand( -1, va("chat \"%s\"", ent->message ));
		}
		else if( ent->spawnflags & 32 ){
			trap_SendServerCommand( -1, va("chat \"%s\"", ent->message ));
			trap_SendServerCommand( -1, va("cps \"%s\"", ent->message ));
		}
		else if(ent->spawnflags & (32|16)){
			trap_SendServerCommand( -1, va("cps \"%s\"", ent->message ));
		}
		else{
			trap_SendServerCommand( -1, va("cps \"%s\"", ent->message ));
...
/*QUAKED target_event (1 0 0) (-8 -8 -8) (8 8 8) redteam blueteam private
"message"	text to print
"wait"		don't fire off again if triggered within this many milliseconds ago
If "private", only the activator gets the message.  If no checks, all clients get the message.
*/
void Use_Target_Event (gentity_t *ent, gentity_t *other, gentity_t *activator)
{
	if (!ent || !ent->inuse)
	{
		Com_Printf("ERROR: Bad ent in Use_Target_Event");
		return;
	}
	...
	{//private, to one client only
		if (!activator || !activator->inuse)
		{
			Com_Printf("ERROR: Bad activator in Use_Target_Event");
		}
		if ( activator && activator->client )
		{//make sure there's a valid client ent to send it to
			if (ent->message[0] == '@' && ent->message[1] != '@')
			{
				trap_SendServerCommand( activator-g_entities, va("send_event \"%s\"", ent->message ));
			}
			else
			{
				trap_SendServerCommand( activator-g_entities, va("send_event \"%s\"", ent->message ));
	...
	if ( ent->spawnflags & 3 ) {
		if ( ent->spawnflags & 1 ) {
			if (ent->message[0] == '@' && ent->message[1] != '@')
			{
				G_TeamCommand( TEAM_RED, va("send_event \"%s\"", ent->message) );
			}
			else
			{
				G_TeamCommand( TEAM_RED, va("send_event \"%s\"", ent->message) );
		...
		if ( ent->spawnflags & 2 ) {
			if (ent->message[0] == '@' && ent->message[1] != '@')
			{
				G_TeamCommand( TEAM_BLUE, va("send_event \"%s\"", ent->message) );
			}
			else
			{
				G_TeamCommand( TEAM_BLUE, va("send_event \"%s\"", ent->message) );
	...
	if (ent->message[0] == '@' && ent->message[1] != '@')
	{
		trap_SendServerCommand( -1, va("send_event \"%s\"", ent->message ));
	}
	else
	{
		trap_SendServerCommand( -1, va("send_event \"%s\"", ent->message ));
...
void SP_target_event( gentity_t *ent ) {
	ent->use = Use_Target_Event;
}
//==========================================================
