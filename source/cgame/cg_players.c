...
extern qboolean CG_FileExists(const char *fileName);
void CG_LoadCISounds(clientInfo_t *ci, qboolean modelloaded)
{
	...
	for ( i = 0 ; i < MAX_CUSTOM_SOUNDS ; i++ )
	{
		...
		else if(!ci->sounds[i] && !((Q_stricmp(cg_customSoundNames[i], "*pain25") == 0) ||
									Q_stricmp(cg_customSoundNames[i], "*pain50") == 0 ||
									Q_stricmp(cg_customSoundNames[i], "*pain75") == 0 ||
									Q_stricmp(cg_customSoundNames[i], "*pain100") == 0 ||
									Q_stricmp(cg_customSoundNames[i], "*gasp") == 0 ||
									Q_stricmp(cg_customSoundNames[i], "*taunt") == 0))
		{
			int x = strlen(soundName)-1;
			while ( soundName[x] >= '0' && soundName[x] <= '9') {
				x--;
			}
			soundName[x + 1] = '\0';
			Q_strcat(soundName, sizeof(soundName), va("%i", Q_irand(1,dummy)));

			//Com_sprintf(soundName, sizeof(soundName), "%s", Q_irand(1, dummy));
			//COM_StripExtension(soundName, soundName);
			//soundpath[strlen(soundpath)-1] = '\0';
			//Q_strcat( soundpath, sizeof(soundpath), va ("%i", Q_irand(1, dummy)));
	...
	*/
	...
	{ //load the Duel sounds then
		...
		{
			//eezstreet add
			/*if(Q_stricmp(cg_customDuelSoundNames[i], "*anger1") == 0	||
				Q_stricmp(cg_customDuelSoundNames[i], "*victory1") == 0	||
				Q_stricmp(cg_customDuelSoundNames[i], "*deflect1") == 0	||
				Q_stricmp(cg_customDuelSoundNames[i], "*gloat1") == 0	||
				Q_stricmp(cg_customDuelSoundNames[i], "*taunt1") == 0){
					dummy = 1;
			}
			...
			if (!ci->duelSounds[i]){ //failed the load, try a sound that we have
				int x = strlen(soundName)-1;
				while ( soundName[x] >= '0' && soundName[x] <= '9') {
					x--;
				}
			...
			//eezstreet end
			if (!ci->duelSounds[i])
			{ //still failed the load, try one out of the generic path
				if (isFemale)
				{
...
#ifdef AKIMBO
				cg_entities[clientNum].ghoul2weapon2 = NULL; //force a refresh
#endif
			}
			j++;
		}
	...
	*ci = newInfo;
...
#ifdef AKIMBO
		cg_entities[i].ghoul2weapon2 = NULL;
#endif
		i++;
	}
	...
	{	// Copy the new ghoul2 model to the centity.
			...
			cg_entities[clientNum].currentState.weapon == WP_SABER)
		{
			...
			{
#ifdef AKIMBO
				CG_CopyG2WeaponInstance(&cg_entities[clientNum], cg_entities[clientNum].currentState.weapon, cg_entities[clientNum].ghoul2, qfalse);
#else
...
#ifdef AKIMBO
			if(cg_entities[clientNum].currentState.weapon2)
			{
				if (cg_entities[clientNum].ghoul2 && ci->ghoul2Model)
				{
					CG_CopyG2WeaponInstance(&cg_entities[clientNum], cg_entities[clientNum].currentState.weapon2, cg_entities[clientNum].ghoul2, qtrue);
					cg_entities[clientNum].ghoul2weapon2 = CG_G2AkimboWeaponInstance(&cg_entities[clientNum], cg_entities[clientNum].currentState.weapon2);
				}
			}
#endif
...
void CG_PlayerAnimEventDo( centity_t *cent, animevent_t *animEvent )
{
	...
	case AEV_SOUND:
		{	// are there variations on the sound?
			if ( animEvent->eventData[AED_SOUNDINDEX_START] == 0 && animEvent->stringData )
			{
...
static void CG_PlayerAnimation( centity_t *cent, int *legsOld, int *legs, float *legsBackLerp,
	...
	else if (cmd.buttons & BUTTON_SPRINT && cent->currentState.eFlags & EF_SPRINTING){
		speedScale = 1.7f;
	}
...
qboolean CG_RagDoll(centity_t *cent, vec3_t forcedAngles)
{
	...
	int ragAnim = BOTH_DEATH1;//BOTH_DEAD1; //BOTH_DEATH1;
...
#elif 0 //with my new method of doing things I want it to continue the anim
		{
			float currentFrame;
...
#endif
		CG_G2SetBoneAngles(cent->ghoul2, 0, "upper_lumbar", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 500, cg.time);
		CG_G2SetBoneAngles(cent->ghoul2, 0, "lower_lumbar", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 500, cg.time);
		CG_G2SetBoneAngles(cent->ghoul2, 0, "thoracic", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 500, cg.time);
		CG_G2SetBoneAngles(cent->ghoul2, 0, "cervical", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 500, cg.time);
...
===============
*/
void CG_PlayerFloatSprite( centity_t *cent, qhandle_t shader, qboolean throughWalls, int size ) {
	int				rf;
	refEntity_t		ent;
...
//	int		team;
	...
	if ( cent->currentState.eFlags & EF_CONNECTION ) {
		CG_PlayerFloatSprite( cent, cgs.media.connectionShader, qfalse, 20 );
		return;
	}

	for(i = 0; i < cg.leaderLineCount; i++)
	{
		if(cg.leaderLines[i].entNum == cent->currentState.clientNum)
		{
			...
			if(InFieldOfVision(cg.snap->ps.viewangles, 100, angles2) && distance < 16384.0f)
			{
				//If we're within our field of vision and within our LOS, draw a sprite above their heads
				if(cent->currentState.clientNum < MAX_CLIENTS)
				{
					CG_PlayerFloatSprite( cent, CG_LeaderLineGraphicForType(cg.leaderLines[i].type), qfalse, 50 );
				}
				else
				{
					//Make our own function for this, or hijack CG_PlayerFloatSprite???
				}
			}
		}
	}
	...
	{
		CG_PlayerFloatSprite( cent, cgs.media.vchatShader, qfalse, 20 );
	}
	else if ( cent->currentState.eType != ET_NPC && //don't draw talk balloons on NPCs
		(cent->currentState.eFlags & EF_TALK) )
	{
		CG_PlayerFloatSprite( cent, cgs.media.balloonShader, qfalse, 20 );
		return;
	}
}
...
static qboolean CG_PlayerShadow( centity_t *cent, float *shadowPlane ) {
	...
	*shadowPlane = 0;
	...
	if ( cg_shadows.integer != 1 || cg.renderingThirdPerson) {	// no mark for stencil or projection shadows
		return qtrue;
	}
	...
	else
	{
		...
		{//allow eye motion
			...
			for (i = 0; i < 2; i++ )
			{
				...
				{
					fov = weaponData [ cg.predictedPlayerState.weapon ].ADSfov;
				}
				else
				{
					fov = cg_fov.value;
...
void CG_Player( centity_t *cent ) {
	...
	if (cgs.gametype >= GT_TEAM)
	{
		...
		if ( cg.snap->ps.clientNum != cent->currentState.number && ( cent->muzzleFlashTime > cg.time - 2000 || cent->aMuzzleFlashTime > cg.time - 2000) )
		{
			CG_AddRadarEnt(cent);
		}
	}
...
#ifdef AKIMBO
			if(cent->currentState.weapon2)
				cent->ghoul2weapon2 = CG_G2AkimboWeaponInstance(cent, cent->currentState.weapon2);
#endif
		}
	}
	...
	if(cent->currentState.eFlags2 & EF2_LEADERLINES)
	{
		CG_AddLeaderLineEnt(cent, cent->currentState.llMask, cent->currentState.llType);
	}
...
// minimal_add:
			...
			!(cent->currentState.eFlags & EF_DEAD))
		{
			...
			{ //check for per-map team shaders
				if (team == SIEGETEAM_TEAM1)
				{
					...
					{
						CG_PlayerFloatSprite( cent, cgSiegeTeam1PlShader, qfalse, 20);
					}
					else
					{ //if there isn't one fallback to default
						CG_PlayerFloatSprite( cent, cgs.media.teamRedShader, qfalse, 20);
					}
				}
				else
				{
					...
					{
						CG_PlayerFloatSprite( cent, cgSiegeTeam2PlShader, qfalse, 20);
					}
					else
					{ //if there isn't one fallback to default
						CG_PlayerFloatSprite( cent, cgs.media.teamBlueShader, qfalse, 20);
					}
				}
			}
			else
		...
		cent->currentState.number != cg.snap->ps.clientNum)
	{
		...
		{ //ally in powerduel, so draw the icon
			CG_PlayerFloatSprite( cent, cgs.media.powerDuelAllyShader, qtrue, 20);
		}
	...
	if (cg_shadows.integer == 2 && (renderfx & RF_THIRD_PERSON) && cg.renderingThirdPerson)
	{ //can see own shadow
	...
	if (cg.snap && cent->currentState.number == cg.snap->ps.clientNum && cgs.clientinfo[cg.predictedPlayerState.clientNum].team != TEAM_SPECTATOR)
	{
			...
			|| cent->currentState.weapon == WP_MELEE) && !cg.predictedPlayerState.zoomMode)	
		{
			...
			//Turn off arm and hand surfaces.
			trap_G2API_SetSurfaceOnOff(cent->ghoul2, "head", 0x00000002);
			trap_G2API_SetSurfaceOnOff(cent->ghoul2, "head_helmet", 0x00000002);
			trap_G2API_SetSurfaceOnOff(cent->ghoul2, "head_helmet_l_tube", 0x00000002);
			trap_G2API_SetSurfaceOnOff(cent->ghoul2, "head_helmet_r_tube", 0x00000002);
			trap_G2API_SetSurfaceOnOff(cent->ghoul2, "head_eyesmouth", 0x00000002);
			trap_G2API_SetSurfaceOnOff(cent->ghoul2, "head_mouth", 0x00000002);
			trap_G2API_SetSurfaceOnOff(cent->ghoul2, "head_teeth", 0x00000002);
			trap_G2API_SetSurfaceOnOff(cent->ghoul2, "head_forehead", 0x00000002);
			trap_G2API_SetSurfaceOnOff(cent->ghoul2, "head_face", 0x00000002);
			trap_G2API_SetSurfaceOnOff(cent->ghoul2, "head_eyes_mouth", 0x00000002);
			trap_G2API_SetSurfaceOnOff(cent->ghoul2, "heada", 0x00000002);
			trap_G2API_SetSurfaceOnOff(cent->ghoul2, "heada_mouth", 0x00000002);
			trap_G2API_SetSurfaceOnOff(cent->ghoul2, "heada_teeth", 0x00000002);
			trap_G2API_SetSurfaceOnOff(cent->ghoul2, "heada_face", 0x00000002);
			if(cent->currentState.weapon != WP_MELEE && cent->currentState.weapon != WP_SABER)
			{
				trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_arm", 0x00000002);
				trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_hand", 0x00000002);
				trap_G2API_SetSurfaceOnOff(cent->ghoul2, "r_arm", 0x00000002);
				trap_G2API_SetSurfaceOnOff(cent->ghoul2, "r_arma", 0x00000002);
				trap_G2API_SetSurfaceOnOff(cent->ghoul2, "r_handa", 0x00000002);
				trap_G2API_SetSurfaceOnOff(cent->ghoul2, "r_hand", 0x00000002);
				trap_G2API_SetSurfaceOnOff(cent->ghoul2, "r_handb", 0x00000002);
				trap_G2API_SetSurfaceOnOff(cent->ghoul2, "r_armb", 0x00000002);
				trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_handb", 0x00000002);
				trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_armb", 0x00000002);
				trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_handa", 0x00000002);
				trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_arma", 0x00000002);
				trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_handa_watch", 0x00000002);
				trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_handb_watch", 0x00000002);
				trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_hand_wrist", 0x00000002);
				trap_G2API_SetSurfaceOnOff(cent->ghoul2, "r_hand_wrist", 0x00000002);
				trap_G2API_SetSurfaceOnOff(cent->ghoul2, "r_arm_shoulder", 0x00000002);
				trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_arm_shoulder", 0x00000002);
				trap_G2API_SetSurfaceOnOff(cent->ghoul2, "r_arm_sleeve", 0x00000002);
				trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_arm_sleeve", 0x00000002);
				trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_hand_plate", 0x00000002);
				trap_G2API_SetSurfaceOnOff(cent->ghoul2, "r_hand_plate", 0x00000002);
				trap_G2API_SetSurfaceOnOff(cent->ghoul2, "r_arm_armor", 0x00000002);
				trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_arm_armor", 0x00000002);
				trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_hand_armor", 0x00000002);
				trap_G2API_SetSurfaceOnOff(cent->ghoul2, "r_hand_armor", 0x00000002);
			}
			else
			{
				goto surfOnArms;
...
SkipTrueView:
	if(cg.renderingThirdPerson /*|| cg.predictedPlayerState.clientNum != cent->currentState.clientNum*/)
	{
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "head", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "head_helmet", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "head_helmet_l_tube", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "head_helmet_r_tube", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "head_eyesmouth", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "head_eyes_mouth", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "head_teeth", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "head_forehead", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "head_face", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "heada", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "heada_mouth", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "heada_teeth", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "heada_face", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "head_mouth", 0);
surfOnArms:
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_arm", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_hand", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "r_arm", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "r_hand", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "r_arma", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "r_handa", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "r_handb", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "r_armb", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_handb", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_armb", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_handa", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_arma", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_handa_watch", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_handb_watch", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_hand_wrist", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "r_hand_wrist", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "r_arm_shoulder", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_arm_shoulder", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "r_arm_sleeve", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_arm_sleeve", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_hand_plate", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "r_hand_plate", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "r_arm_armor", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_arm_armor", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "l_hand_armor", 0);
		trap_G2API_SetSurfaceOnOff(cent->ghoul2, "r_hand_armor", 0);
	}
...
stillDoSaber:
	...
	if(cent->currentState.isJediMaster)
	{
		cg.leaderLines[0].entNum = cent->currentState.number;
	}
	...
	if(cg.snap->ps.zoomMode == 2){
		...
		//legs.customShader = cgs.media.forceShell;
		legs.customShader = cgs.media.refractionShader;
		
		trap_R_AddRefEntityToScene( &legs );
	}
	else if(cent->currentState.number < MAX_CLIENTS){
		if (cent->currentState.isJediMaster && cg.snap->ps.clientNum != cent->currentState.number && !(cent->currentState.powerups & (1 << PW_CLOAKED)))
		{
			...
			legs.renderfx &= ~RF_NODEPTH;
			//CG_AddLeaderLineEnt(cent, 0, LL_KILL);
			cg.leaderLines[0].entNum = cent->currentState.number;
			cg.leaderLines[0].type = LL_KILL;
			cg.leaderLines[0].visibilityMask = 0;
			cg.leaderLineCount = 1;
		}
		
		else if(client->team == cgs.clientinfo[cg.clientNum].team &&
			cent->currentState.number != cg.clientNum &&
			(cent->currentState.number == cgs.redHero ||
			cent->currentState.number == cgs.blueHero) &&
			cgs.gametype == GT_HERO){
				//Your team's hero is wallhacked to be blue.
			legs.shaderRGBA[0] = 50;
			legs.shaderRGBA[1] = 50;
			legs.shaderRGBA[2] = 255;
			...
			cent->currentState.number != cg.clientNum && cgs.gametype >= GT_TEAM){
			//We're on the same team, so wallhack us.
			legs.shaderRGBA[0] = 150;
			legs.shaderRGBA[1] = 255;
			legs.shaderRGBA[2] = 150;
			if(cent->vChatTime > cg.time)
			{
				legs.shaderRGBA[0] = 255;
				legs.shaderRGBA[1] = 255;
			...
			legs.renderfx |= RF_NODEPTH;
			if(cent->vChatTime > cg.time)
				legs.customShader = cgs.media.yellowSaberGlowShader;
			else
			...
			cent->currentState.number == cgs.redHero) &&
			cgs.gametype == GT_HERO){
			//We're the enemy hero. Wallhack us.
			legs.shaderRGBA[0] = 255;
			legs.shaderRGBA[1] = 100;
		...
		else if(((cgs.gametype >= GT_TEAM && client->team != cgs.clientinfo[cg.clientNum].team) ||
			cgs.gametype < GT_TEAM) && cg.reconTime >= cg.time && cent->currentState.clientNum != cg.clientNum)
		{	//Recon team active. Hax hax hax.
			legs.shaderRGBA[0] = 255;
			legs.shaderRGBA[1] = 100;
			legs.shaderRGBA[2] = 100;
...
/*			if (cg.snap->ps.fd.pd[FP_SEE]->startingRank < FORCE_LEVEL_2)
			{ //All ranks can see through walls
			}*/
		}
		legs.renderfx &= ~RF_NODEPTH;
		legs.renderfx &= ~RF_RGB_TINT;
