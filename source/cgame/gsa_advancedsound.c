...
qboolean __fastcall GSA_DetermineIfWAVFile(advSFX_t *file)
{
...
// This function requires the file to be open in order for it to work properly
void GSA_ProcessWAVFile(advSFX_t *file)
{
	char buffer[4];
...
sfxHandle_t __stdcall GSA_RegisterAdvSound( const char *fileName )
{
	...
	if(GSA_DetermineIfWAVFile(&file))
	{
		// Format: WAV
		file.advFormat = ADVFMT_WAV;
		GSA_ProcessWAVFile(&file);
		trap_FS_FCloseFile(file.handle);
	}
#ifdef __GSAMP3
	else if(GSA_DetermineIfMP3File(&file))
	{
		// Format: MP3
...
#endif
#ifdef __GSAOGG
	else if(GSA_DetermineIfOGGFile(&file))
	{
		// Format: OGG Vorbis
...
void GSA_InitOpenAL( void )
{
	oaldevice = alcOpenDevice(NULL);
	if(!oaldevice)
	{
		Com_Error(ERR_FATAL, "GSA_InitOpenAL: no sound device");
		return;
	}
	...
	if(!oaldc)
	{
		Com_Error(ERR_FATAL, "GSA_InitOpenAL: no device context");
		return;
	}
}

void GSA_CleanupOpenAL( void )
{
...
static void GSA_AddToSoundsPlayed(sfxHandle_t soundIndex, int bufferID, int srcID)
{
	ALint bits;
...
void GSA_PlaySoundIndex(sfxHandle_t soundIndex, qboolean looping, int channel)
{
	...
	{
		Com_Error(ERR_FATAL, "Error Gensource");
		return;
	}
	...
	if(!format)
	{
		Com_Printf("^1ERROR: Bad bitspersecond\n");
		return;
	}
	...
	if(alGetError() != AL_NO_ERROR)
	{
		Com_Error(ERR_FATAL, "Could not init OpenAL buffer data.");
		return;
	}
	...
	if(alGetError() != AL_NO_ERROR)
	{
		Com_Printf("^1Error playing sound %s\n", sfx.fileName);
		return;
	}

	// Add to the array of playing sounds
	GSA_AddToSoundsPlayed(soundIndex, buffer, src);
}
...
void GSA_StopSoundIndex(int index)
{
	// Great. Got our sound. Now let's get down to business.
