...
//	CG_DrawLoadingIcons();
	...
	if ( !atoi( buf ) ) {
		...
		UI_DrawProportionalString( 320, y, buf,
			UI_CENTER|UI_GIANTFONT|UI_DROPSHADOW, colorWhite );
		y += iPropHeight;
	...
	s = Info_ValueForKey(  info, "g_instagib" );
	if( s[0] == '1' ) {
		UI_DrawProportionalString( 320, y, "Instagib",
			UI_CENTER|UI_INFOFONT|UI_DROPSHADOW, colorWhite );
		y += iPropHeight;
	}
	s = Info_ValueForKey(  info, "g_LMS" );
	z = Info_ValueForKey(  info, "g_LMSlives" );
	if( s[0] == '1' ) {
		UI_DrawProportionalString( 320, y, "Last Man Standing",
			UI_CENTER|UI_INFOFONT|UI_DROPSHADOW, colorWhite );
		y += iPropHeight;
		UI_DrawProportionalString( 320, y, va("%i lives", atoi(z)),
			UI_CENTER|UI_INFOFONT|UI_DROPSHADOW, colorWhite );
		y += iPropHeight;
	}
...
//		s = "Jedi Master";
	...
	case GT_CTY:
		...
		break;
	case GT_DOMINATION:
			s = CG_GetStringEdString("MENUS", "DOMINATION");//"Domination";
			break;
	case GT_VIP_ESCORT:
			s = CG_GetStringEdString("MENUS", "VIP_ESCORT");//"VIP Escort";
			break;
	case GT_COMMANDER:
			s = CG_GetStringEdString("MENUS", "COMMANDER");//"Commander Escort";
			break;
	default:
			s = CG_GetStringEdString("MENUS", "SAGA");//"Team FFA";
	...
	// Display the rules based on type
		//y += iPropHeight;
	//[ServerRules]
	s = Info_ValueForKey(  info, "g_serverRules" );
	UI_DrawProportionalString( 320, y, s,
			UI_CENTER|UI_INFOFONT|UI_DROPSHADOW, colorWhite );
			y += (iPropHeight*2);
			//[/ServerRules]
	...
	case GT_TEAM:
		...
		break;
	case GT_HERO:
		UI_DrawProportionalString( 320, y, va( "%s", (char *)CG_GetStringEdString("MP_INGAME", "RULES_HERO_1")),
			UI_CENTER|UI_INFOFONT|UI_DROPSHADOW, colorWhite );
		y += iPropHeight;
		UI_DrawProportionalString( 320, y, va( "%s", (char *)CG_GetStringEdString("MP_INGAME", "RULES_HERO_2")),
			UI_CENTER|UI_INFOFONT|UI_DROPSHADOW, colorWhite );
		y += iPropHeight;
		break;
	case GT_SIEGE:
	...
	case GT_CTY:
		...
		break;
	case GT_DOMINATION:
		UI_DrawProportionalString( 320, y, va( "%s", (char *)CG_GetStringEdString("MP_INGAME", "RULES_DOM_1")),
			UI_CENTER|UI_INFOFONT|UI_DROPSHADOW, colorWhite );
		y += iPropHeight;
		UI_DrawProportionalString( 320, y, va( "%s", (char *)CG_GetStringEdString("MP_INGAME", "RULES_DOM_2")),
			UI_CENTER|UI_INFOFONT|UI_DROPSHADOW, colorWhite );
		y += iPropHeight;
		break;
	case GT_VIP_ESCORT:
		UI_DrawProportionalString( 320, y, va( "%s", (char *)CG_GetStringEdString("MP_INGAME", "RULES_VIP_1")),
			UI_CENTER|UI_INFOFONT|UI_DROPSHADOW, colorWhite );
		y += iPropHeight;
		UI_DrawProportionalString( 320, y, va( "%s", (char *)CG_GetStringEdString("MP_INGAME", "RULES_VIP_2")),
			UI_CENTER|UI_INFOFONT|UI_DROPSHADOW, colorWhite );
		y += iPropHeight;
		break;
	case GT_COMMANDER:
		UI_DrawProportionalString( 320, y, va( "%s", (char *)CG_GetStringEdString("MP_INGAME", "RULES_COM_1")),
			UI_CENTER|UI_INFOFONT|UI_DROPSHADOW, colorWhite );
		y += iPropHeight;
		UI_DrawProportionalString( 320, y, va( "%s", (char *)CG_GetStringEdString("MP_INGAME", "RULES_COM_2")),
			UI_CENTER|UI_INFOFONT|UI_DROPSHADOW, colorWhite );
		y += iPropHeight;
		break;
	default:
...
void CG_LoadBar(void)
{
	const int numticks = 9, tickwidth = 10, tickheight = 14;
	const int tickpadx = 20, tickpady = 50;
	...
	float phase = (float)((cg.loadLCARSStage == 0) ? 1.0f : 1.0f/(10 - cg.loadLCARSStage));
	...
	// Draw bar
	//CG_DrawPic(barleft, ticktop-(tickheight*2), tickwidth*cg.loadLCARSStage, tickheight*3, cgs.media.loadBarLED);

	// Draw right cap
	//CG_DrawPic(tickleft+tickwidth*cg.loadLCARSStage, ticktop, capwidth, tickheight, cgs.media.loadBarLEDCap);

	// Now draw the load overlay.
	//CG_DrawPic(0,0, SCREEN_WIDTH, SCREEN_HEIGHT, cgs.media.loadBarShader);

	CG_FillRect( 100, 450, 450, 10, colorBlack );
	CG_FillRect( 102, 452, ((448) * phase), 6, colorWhite );
}
