...
static void CG_AddTestModel (void) {
	...
	if ( cg.testGun ) {
		...
		for (i=0 ; i<3 ; i++) {
			switch(cg.snap->ps.weapon){ //eezstreet: alter the weapon's position properly
				case WP_FLECHETTE:
					cg.testModelEntity.origin[i] += cg.refdef.viewaxis[0][i] * cg_gun_x.value;
					cg.testModelEntity.origin[i] += cg.refdef.viewaxis[1][i] * cg_gun_y.value;
					cg.testModelEntity.origin[i] += cg.refdef.viewaxis[2][i] * (cg_gun_z.value + 2);
					break;
				case WP_ROCKET_LAUNCHER:
					cg.testModelEntity.origin[i] += cg.refdef.viewaxis[0][i] * cg_gun_x.value;
					cg.testModelEntity.origin[i] += cg.refdef.viewaxis[1][i] * cg_gun_y.value;
					cg.testModelEntity.origin[i] += cg.refdef.viewaxis[2][i] * (cg_gun_z.value + 4);
					break;
				default:
					cg.testModelEntity.origin[i] += cg.refdef.viewaxis[0][i] * cg_gun_x.value;
					cg.testModelEntity.origin[i] += cg.refdef.viewaxis[1][i] * cg_gun_y.value;
...
static void CG_CalcIdealThirdPersonViewLocation(void)
{
	...
	if(cmd.buttons & BUTTON_SIGHTS && !(cmd.buttons & BUTTON_SPRINT))
	{
		thirdPersonRange -= (cg_fov.integer/6);
	}
...
//			kickPerc = kickTime/600.0f;
	...
	delta = cg.bobfracsin * (cg_bobpitch.value + weaponData[cg.predictedPlayerState.weapon].bobbing[PITCH]) * speed;
	if (cg.predictedPlayerState.pm_flags & PMF_DUCKED)
		delta *= weaponData[cg.predictedPlayerState.weapon].bobCrouchModifier;		// crouching
	angles[PITCH] += delta;
	delta = cg.bobfracsin * (cg_bobroll.value + weaponData[cg.predictedPlayerState.weapon].bobbing[ROLL]) * speed;
	if (cg.predictedPlayerState.pm_flags & PMF_DUCKED)
		delta *= weaponData[cg.predictedPlayerState.weapon].bobCrouchModifier;		// crouching accentuates roll

	if (cg.bobcycle & 1)
		delta = -delta;
	angles[ROLL] += delta;

	delta = cg.bobfracsin * 0.001 * speed;
	if (cg.bobcycle & 1)
		delta = -delta;
	angles[YAW] += delta;
...
static int CG_CalcFov( void ) {
	...
	int cmdNum;
	int weaponADS = weaponData[cg.predictedPlayerState.weapon].ADSfov;
	usercmd_t	cmd;
	...
	*/
	...
	if(weaponADS > 0.0f)
	{
		phase = CG_IronSightsPhase(&cg.predictedPlayerState);
		if(phase >= 1.0f)
		{
			cgFov = weaponADS;
		}
		...
		{
			cgFov = cgFov - phase * (cgFov - weaponADS);
		}
	}
	...
	if ( cg.predictedPlayerState.pm_type == PM_INTERMISSION ) {
		...
		else if (cg.predictedPlayerState.zoomMode)
		{
			...
			{
				if(weaponData[cg.predictedPlayerState.weapon].zoomType == 1)
				{
				...
				{
					fov_x = zoomFov = cg.predictedPlayerState.zoomFov;
				}
			}
			else
			{
				double phase;
				phase = CubicBezierInterpolate(min(cg.time - cg.zoomStartTime, weaponData[cg.predictedPlayerState.weapon].ADSTime)/(double)weaponData[cg.predictedPlayerState.weapon].ADSTime, 0.0, 0.0, 1.0, 1.0);
				if(phase >= 1.0f)
				{
					fov_x = weaponData[cg.predictedPlayerState.weapon].ADSfov;
				}
				else if(phase > 0)
				{
					fov_x = cgFov - phase * (cgFov - weaponData[cg.predictedPlayerState.weapon].ADSfov);
				}
				//fov_x = phase * weaponData[cg.predictedPlayerState.weapon].ADSfov;
			}
		}
		else 
		{
...
void CG_DrawSkyBoxPortal(const char *cstr)
{
	...
	if (!fov_x)
	{
		...
		else if(cmd.buttons & BUTTON_SIGHTS && cg.time < cg.reloadTimeDuration + cg.reloadTimeStart && !(cmd.buttons & BUTTON_SPRINT))
		{
			fov_x = weaponData[ cg.predictedPlayerState.weapon ].ADSfov;
		}
		else
...
#ifdef ACHIEVEMENTS
extern void GSA_SaveAC(void);
#endif
extern void Xbox_ControllerStatus(void);
//[/ROQFILES]
void CG_DrawActiveFrame( int serverTime, stereoFrame_t stereoView, qboolean demoPlayback ) {
...
#else
		CG_AddViewWeapon( &cg.predictedPlayerState, qfalse );
		if(cg.predictedPlayerState.weapon2)
			CG_AddViewWeapon( &cg.predictedPlayerState, qtrue );
#endif
	}
	//CG_AddViewWeapon( &cg.predictedPlayerState );
	//[/CoOp]
	...
	if( cg.time - cg.acSaveTime  > 30000 )
	{
#ifdef ACHIEVEMENTS
		GSA_SaveAC();
#endif
