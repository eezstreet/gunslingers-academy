...
void CG_BuildSolidList( void ) {
	...
	for ( i = 0 ; i < snap->numEntities ; i++ ) {
		...
		//eezstreet edit: Some kind of breakage here regarding Zalchion's demo. Regardless, it's an essential check.
		if(snap->entities[ i ].number > 1024)
		{
			continue;
		}
...
static void CG_TouchItem( centity_t *cent ) {
	...
	/*if(item->giType == IT_WEAPON && !(cmd.buttons & BUTTON_USE)){
		return; //Y U NO HOLD USE?
	}*/
...
*/
	...
	if ( item->giType == IT_WEAPON ) {
		if(cg.weaponSwapTime && cmd.buttons & BUTTON_USE){
			int current;
			usercmd_t cmd;

			current = trap_GetCurrentCmdNumber();
			trap_GetUserCmd(current, &cmd);
			BG_AddPredictableEventToPlayerstate( EV_WEAPON_PICKUP, cent->currentState.number , &cg.predictedPlayerState);
		...
		BG_AddPredictableEventToPlayerstate( EV_AMMO_PICKUP, cent->currentState.number , &cg.predictedPlayerState);
	else
