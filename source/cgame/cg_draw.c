...
int MenuFontToHandle(int iMenuFont)
{
	switch (iMenuFont)
	{
		...
		case FONT_DIGITAL:	return cgDC.Assets.qhDigitalFont;
...
================
CG_DrawBeta
Puts a special "developer" watermark on the game screen
================
*/
static void CG_DrawBeta( void ){
	if(r_beta.integer){
		vec4_t				color;
		...
		CG_DrawPic( 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, cgs.media.betaShader );
	}
}
#endif
...
{
	vec4_t				color;
	int flashTime =		(cg.flashTime - cg.time);
	float mathMagic;
	if(flashTime < 1) //right off the bat we want to tell if they are flashed
		return;
	mathMagic = 1.0f; //default to shut the compiler up

	//so now we subtract from the flashTime
	if(flashTime > 10000){
		//do nothing, our alpha is maxed out
	}
	else if(flashTime < 10000 && flashTime > 0){
		mathMagic = (float)flashTime/10000;
		if(mathMagic > 1.0f){
			mathMagic = 1.0f;
		}
	}
	...
	//set the color to the base default of white
	color[0] = 1.0f;
	color[1] = 1.0f;
	color[2] = 1.0f;
	//alpha uses subtraction based on the flashTime to determine transparency
	color[3] = mathMagic;
	//use our vec4_t to set the color
	trap_R_SetColor( color );
	//draw this thing already!
...
/*
================
CG_DrawBloodSplatter

Used for close-range blood splatter
================
*/
static void CG_DrawBloodSplatter(void)
{
	vec4_t color;

	if(cg.time > cg.bloodSprayTime)
		return;
	color[0] = 1.0f;
	color[1] = 1.0f;
	color[2] = 1.0f;
	color[3] = ((float)((cg.bloodSprayTime-cg.time)/(float)(cg.bloodSprayTime-cg.bloodSprayStartTime)));
	trap_R_SetColor( color );
	CG_DrawPic( 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, trap_R_RegisterShader("gfx/2d/blood_direct"));
}
/*
================
CG_DrawZoomMask
...
//	int val[5];
	...
	else if ( cg.predictedPlayerState.zoomMode)
	{
		...
		// Draw target mask
		trap_R_SetColor( colorTable[CT_WHITE] );
		CG_DrawPic( 80, 0, 480, 480, cgs.media.disruptorMask );
		CG_DrawRect(0, 0, 80, 480, 60, colorTable[CT_BLACK]);
		CG_DrawRect(640, 0, 80, 480, -60, colorTable[CT_BLACK]);
...
#ifndef AKIMBO
static const char *reloadStr = "Reloading weapon...";
#endif
static void STC_DrawReload( void )
{
	float	phase;
	int		x;
	static vec4_t colorBack		= { 0.0f, 0.0f, 0.0f, 0.7f };
	static vec4_t colorFront	= { 1.0f, 1.0f, 1.0f, 0.7f };
...
#ifdef AKIMBO
	if(cg.reloadingBothGuns)
	{
		x = ( 320 - (trap_R_Font_StrLenPixels( "Reloading weapons...", 1, 0.8f ) / 2) );
		trap_R_Font_DrawString( x, 190, "Reloading weapons...", colorWhite, 1, -1, 0.8f );
	}
	else
	{
		x = ( 320 - (trap_R_Font_StrLenPixels( "Reloading weapon...", 1, 0.8f ) / 2) );
		trap_R_Font_DrawString( x, 190, "Reloading weapon...", colorWhite, 1, -1, 0.8f );
	}
#else
	x = ( 320 - (trap_R_Font_StrLenPixels( reloadStr, 1, 0.8f ) / 2) );
	trap_R_Font_DrawString( x, 190, reloadStr, colorWhite, 1, -1, 0.8f );
#endif

	phase = (float)(cg.time - cg.reloadTimeStart) / (float)cg.reloadTimeDuration;
	CG_FillRect( 243, 220, 154, 10, colorBlack );
	CG_FillRect( 245, 222, 150 * phase, 6, colorWhite );
}

#ifdef AKIMBO
static void STC_DrawRReload( void )
{
	float	phase;
	int		x;
	static vec4_t colorBack		= { 0.0f, 0.0f, 0.0f, 0.7f };
	...
	if ( !cg.reloadTimeRightStart )
		return;

	if ( cg.time > cg.reloadTimeRightStart + cg.reloadTimeRightDuration )
	{
		cg.reloadTimeRightStart = 0;
		cg.reloadTimeRightDuration = 0;
		return;
	}

	x = ( 320 - (trap_R_Font_StrLenPixels( "Reloading right weapon...", 1, 0.8f ) / 2) );
	trap_R_Font_DrawString( x, 230, "Reloading right weapon...", colorWhite, 1, -1, 0.8f );

	phase = (float)(cg.time - cg.reloadTimeRightStart) / (float)cg.reloadTimeRightDuration;
	CG_FillRect( 243, 250, 154, 10, colorBlack );
	CG_FillRect( 245, 252, 150 * phase, 6, colorWhite );
}
static void STC_DrawLReload( void )
{
	float	phase;
	int		x;
	static vec4_t colorBack		= { 0.0f, 0.0f, 0.0f, 0.7f };
	...
	if ( !cg.reloadTimeLeftStart )
		return;

	if ( cg.time > cg.reloadTimeLeftStart + cg.reloadTimeLeftDuration )
	{
		cg.reloadTimeLeftStart = 0;
		cg.reloadTimeLeftDuration = 0;
		return;
	}

	x = ( 320 - (trap_R_Font_StrLenPixels( "Reloading left weapon...", 1, 0.8f ) / 2) );
	trap_R_Font_DrawString( x, 285, "Reloading left weapon...", colorWhite, 1, -1, 0.8f );

	phase = (float)(cg.time - cg.reloadTimeLeftStart) / (float)cg.reloadTimeLeftDuration;
	CG_FillRect( 243, 280, 154, 10, colorBlack );
	CG_FillRect( 245, 282, 150 * phase, 6, colorWhite );
}
#endif
...
void CG_DrawHealth( menuDef_t *menuHUD )
{
	...
	int				currValue,inc;
//	itemDef_t		*focusItem;
	float *tintColor = colorTable[CT_WHITE];
	float percent;
	...
	ps = &cg.snap->ps;
	memcpy(&calcColor, tintColor, sizeof(vec4_t));
	...
	if(healthAmt < 75){
		if(healthAmt == 0)
			...
			percent = (float) 1.0f-(currValue / 75.0f);
			calcColor[3] *= percent;
			trap_R_SetColor( calcColor );
			CG_DrawPic(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, cgs.media.bloodSight);
			calcColor[3] = 1.0f;
			trap_R_SetColor( calcColor );
			/*if(healthAmt < 50)
			{
				CG_DrawPic(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, cgs.media.bloodSight1);
			}
			if(healthAmt < 35)
			{
				CG_DrawPic(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, cgs.media.bloodSight2);
			}
			if(healthAmt < 25)
			{
				CG_DrawPic(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, cgs.media.bloodSight3);
			}*/
		}
	}
...
================
*/
static void CG_DrawFiringMode( centity_t *cent)
{
	playerState_t	*ps;
	char stringEd[10];
	weaponData_t weapon;
	...
	weapon = weaponData[ps->weapon];

	if(!weapon.fireModeDisplay)
		return;

	memset(stringEd, 0, sizeof(stringEd));
	switch(ps->stats[STAT_FMODE]){
		case FM_NONE:
		case FM_MAX:
		default:
			return;
			break;
		case FM_FULLAUTO:
			strcpy(stringEd, va("%s", CG_GetStringEdString("GMENUS", "FM_FULLAUTO")));
			break;
		case FM_SEMIAUTO:
			strcpy(stringEd, va("%s", CG_GetStringEdString("GMENUS", "FM_SEMIAUTO")));
			break;
		case FM_THREEBURST:
		case FM_FOURBURST:
			strcpy(stringEd, va("%s", CG_GetStringEdString("GMENUS", "FM_BURST")));
			break;
		case FM_SINGLE:
			strcpy(stringEd, va("%s", CG_GetStringEdString("GMENUS", "FM_SINGLE")));
			break;
		case FM_DOUBLE:
			strcpy(stringEd, va("%s", CG_GetStringEdString("GMENUS", "FM_DOUBLE")));
			break;
	}
	/*UI_DrawScaledProportionalString( 520, 465, stringEd,
			UI_SMALLFONT|UI_DROPSHADOW, colorTable[CT_WHITE], 0.34 );*/
	UI_DrawScaledProportionalString( data.hud.general.fmx, data.hud.general.fmy, stringEd,
			UI_SMALLFONT|UI_DROPSHADOW, colorTable[CT_WHITE], 0.34 );
}
...
static void CG_DrawAmmo( centity_t	*cent,menuDef_t *menuHUD)
{
	...
	value2 = ps->stats[STAT_CLIP];
	if ((value + value2) <= 0)	// No ammo //eezstreet edit: this was < 0, but I changed it so it looks better on screen.
	{
		return;
	} else if (value <= weaponData[cent->currentState.weapon].ammoLow){
		color[0] = 0.45f;
		color[1] = 0.05f;
		color[2] = 0.05f;
	}
	...
	else
	{
		...
		if (focusItem)
		{
			...
			if (value <= weaponData[cent->currentState.weapon].ammoLow){
				color[0] = 0.45f;
				color[1] = 0.05f;
				color[2] = 0.05f;
				color[3] = 1.0f;
			...
			{
				VectorCopy4(data.hud.ammo.totalColor, color);
			}
			trap_R_SetColor( (const float *)color );
			/*CG_DrawNumField (
				focusItem->window.rect.x, 
				focusItem->window.rect.y, 
				3, 
				...
				qtrue);*/
			CG_DrawNumField (
				data.hud.ammo.totalTextX, 
				data.hud.ammo.totalTextY, 
				data.hud.ammo.totalTextW, 
				value, 
				data.hud.ammo.totalCharW, 
				data.hud.ammo.totalCharH, 
				data.hud.ammo.totalFont,
				data.hud.ammo.totalDrawZeroes);
			if((value + value2) <= weaponData[cent->currentState.weapon].ammoLow){
				UI_DrawScaledProportionalString(focusItem->window.rect.x-50, focusItem->window.rect.y-5, "LOW AMMO", NUM_FONT_SMALL|UI_DROPSHADOW, color, 0.25f);
			}
			if (value2 <= weaponData[cent->currentState.weapon].ammoClipLow){
				color[0] = 0.45f;
				color[1] = 0.05f;
				color[2] = 0.05f;
				color[3] = 1.0f;
			...
			{
				VectorCopy4(data.hud.ammo.clipColor, color);
			}
			trap_R_SetColor( (const float *)color );
			/*CG_DrawNumField (
				focusItem->window.rect.x, 
				focusItem->window.rect.y-15, 
				3, 
				value2, 
				focusItem->window.rect.w, 
				focusItem->window.rect.h, 
				...
				qtrue);*/
			CG_DrawNumField (
				data.hud.ammo.clipTextX, 
				data.hud.ammo.clipTextY, 
				data.hud.ammo.clipTextW, 
				value2, 
				data.hud.ammo.clipCharW, 
				data.hud.ammo.clipCharH, 
				data.hud.ammo.clipFont,
				data.hud.ammo.clipDrawZeroes);
#ifdef AKIMBO
			if(cg.predictedPlayerState.weapon2 != WP_NONE)
			{
				value =ps->ammo[weaponData[cent->currentState.weapon2].ammoIndex];
				value2 = ps->stats[STAT_CLIP2];
				if (value <= weaponData[cent->currentState.weapon2].ammoLow){
					color[0] = 0.45f;
					color[1] = 0.05f;
					color[2] = 0.05f;
					color[3] = 1.0f;
				...
				trap_R_SetColor( (const float *)color );
				CG_DrawNumField (
					data.hud.ammo.aTotalTextX, 
					data.hud.ammo.aTotalTextY, 
					data.hud.ammo.aTotalTextW, 
					value, 
					data.hud.ammo.aTotalCharW, 
					data.hud.ammo.aTotalCharH, 
					data.hud.ammo.totalFont,
					data.hud.ammo.totalDrawZeroes);
				if((value + value2) <= weaponData[cent->currentState.weapon2].ammoLow){
					UI_DrawScaledProportionalString(50, focusItem->window.rect.y-5, "LOW AMMO", NUM_FONT_SMALL|UI_DROPSHADOW, color, 0.25f);
				}
				if (value2 <= weaponData[cent->currentState.weapon2].ammoClipLow){
					color[0] = 0.45f;
					color[1] = 0.05f;
					color[2] = 0.05f;
					color[3] = 1.0f;
				...
				trap_R_SetColor( (const float *)color ); 
				CG_DrawNumField (
					data.hud.ammo.aClipTextX, 
					data.hud.ammo.aClipTextY, 
					data.hud.ammo.aClipTextW, 
					value2, 
					data.hud.ammo.aClipCharW, 
					data.hud.ammo.aClipCharH, 
					data.hud.ammo.clipFont,
					data.hud.ammo.clipDrawZeroes);
			}
#endif
	...
	// Draw tics
	for (i=data.hud.ammo.numTics;i>=0;i--)
	{
		if(data.hud.ammo.teamTics){
			if (cg.snap->ps.persistant[PERS_TEAM] == TEAM_RED ){
				Vector4Copy(color, redhudtint);
			}
			else if (cg.snap->ps.persistant[PERS_TEAM] == TEAM_BLUE ){
				Vector4Copy(color, bluehudtint);
			}
		}

		//focusItem = Menu_FindItemByName(menuHUD, ammoTicName[i]);
		if(data.hud.ammo.fadeTics){
			/*if (!focusItem)
			{
				continue;
			}*/
		...
		CG_DrawPic(	data.hud.ammo.ammoTicPosX[i],
					data.hud.ammo.ammoTicPosY[i],
					data.hud.ammo.ammoTicWidth[i],
					data.hud.ammo.ammoTicHeight[i],
					cgs.media.ammoTics[i]);
...
/*
================
CG_CorrectForAspectRatio
================
*/
//Corrects w/h according to aspect ratio
/*
================
CG_DrawHUD
================
...
*/
	...
	//Draw background.
	trap_R_SetColor((const float *)colorTable[CT_WHITE]);
	CG_DrawPic(data.hud.general.bgX, data.hud.general.bgY, data.hud.general.bgW, data.hud.general.bgH, trap_R_RegisterShader(data.hud.general.bgLoc));
	//1.33 ratio for stuff
	//CG_DrawPic(SCREEN_WIDTH-150, SCREEN_HEIGHT-195, 150, 195, trap_R_RegisterShader("gfx/hud/background-onlyblack"));
#ifdef AKIMBO
	if(cg.predictedPlayerState.weapon2 != WP_NONE)
		CG_DrawPic(0, data.hud.general.bgY, data.hud.general.bgW, data.hud.general.bgH, trap_R_RegisterShader(va("%s-flipped", data.hud.general.bgLoc)));
#endif
	//Draw stamina bar
	if(cgd_drawStaminaBar.integer > 0)
	{
		const char	*info;
		info = CG_ConfigString( CS_SERVERINFO );
		UI_DrawProportionalString(100, SCREEN_HEIGHT-30, va("Stamina: %f%%", ((float)(cg.predictedPlayerState.stats[STAT_STAMINA] / atof(Info_ValueForKey(info, "g_staminaMax"))) * 100)), NUM_FONT_SMALL, colorWhite); 
	}

	if(data.hud.ammo.activeClip)
	{
		CG_DrawPic(data.hud.ammo.clipX, data.hud.ammo.clipY, data.hud.ammo.clipW, data.hud.ammo.clipH, trap_R_RegisterShader(va("%s-clip", data.hud.ammo.clipHeader)));
#ifdef AKIMBO
		if(cg.predictedPlayerState.weapon2 != WP_NONE)
		{
			CG_DrawPic(data.hud.ammo.aClipX, data.hud.ammo.aClipY, data.hud.ammo.aClipW, data.hud.ammo.aClipH, trap_R_RegisterShader(va("%s-clip", data.hud.ammo.clipHeader)));
		}
#endif
	}
	if(data.hud.ammo.activeTotal)
	{
		CG_DrawPic(data.hud.ammo.totalX, data.hud.ammo.totalY, data.hud.ammo.totalW, data.hud.ammo.totalH, trap_R_RegisterShader(va("%s-total", data.hud.ammo.clipHeader)));
#ifdef AKIMBO
		if(cg.predictedPlayerState.weapon2 != WP_NONE)
		{
			CG_DrawPic(data.hud.ammo.aTotalX, data.hud.ammo.aTotalY, data.hud.ammo.aTotalW, data.hud.ammo.aTotalH, trap_R_RegisterShader(va("%s-total", data.hud.ammo.clipHeader)));
		}
#endif
	}

	//Draw quickthrow, if we need to.
	if(data.hud.general.activeNade)
	{
		if(cg.predictedPlayerState.stats[STAT_WEAPONS] & (1 << WP_THERMAL) && cg.predictedPlayerState.ammo[weaponData[WP_THERMAL].ammoIndex] > 0)
		{
			CG_DrawPic(data.hud.general.nadeX, data.hud.general.nadeY, data.hud.general.nadeW, data.hud.general.nadeH, trap_R_RegisterShader(va("%s-frag", data.hud.general.nadeHeader)));
			//CG_DrawPic(585, 440, 32, 32, trap_R_RegisterShader(va("%s-frag", data.hud.general.nadeHeader)));
			//CG_DrawNumField(601, 456, 3, cg.predictedPlayerState.ammo[weaponData[WP_THERMAL].ammoIndex], 8, 8, NUM_FONT_CHUNKY, qfalse);
			trap_R_SetColor((const float *)data.hud.general.nadeTextColor);
			CG_DrawNumField(data.hud.general.nadeTextX, data.hud.general.nadeTextY, data.hud.general.nadeTextW, cg.predictedPlayerState.ammo[weaponData[WP_THERMAL].ammoIndex], data.hud.general.nadeCharW, data.hud.general.nadeCharH, data.hud.general.nadeFont, qfalse);
			//CG_DrawPic(535, 400, 32, 32, trap_R_RegisterShader("gfx/hud/rounds-clip"));
			//CG_DrawPic(550, 370, 32, 32, trap_R_RegisterShader("gfx/hud/rounds-total"));
			//CG_DrawNumField(571, 386, 3, cg.predictedPlayerState.ammo[weaponData[cent->currentState.weapon].ammoIndex], 8, 8, NUM_FONT_CHUNKY, qfalse);
			//CG_DrawNumField(551, 416, 3, cg.predictedPlayerState.stats[STAT_CLIP], 8, 8, NUM_FONT_CHUNKY, qfalse);
		}
		else if(cg.predictedPlayerState.stats[STAT_WEAPONS] & (1 << WP_TRIP_MINE) && cg.predictedPlayerState.ammo[weaponData[WP_TRIP_MINE].ammoIndex] > 0)
		{
			CG_DrawPic(data.hud.general.nadeX, data.hud.general.nadeY, data.hud.general.nadeW, data.hud.general.nadeH, trap_R_RegisterShader(va("%s-flash", data.hud.general.nadeHeader)));
			trap_R_SetColor((const float *)data.hud.general.nadeTextColor);
			CG_DrawNumField(data.hud.general.nadeTextX, data.hud.general.nadeTextY, data.hud.general.nadeTextW, cg.predictedPlayerState.ammo[weaponData[WP_TRIP_MINE].ammoIndex], data.hud.general.nadeCharW, data.hud.general.nadeCharH, data.hud.general.nadeFont, qfalse);
		}
		else if(cg.predictedPlayerState.stats[STAT_WEAPONS] & (1 << WP_AUXILIARY6) && cg.predictedPlayerState.ammo[weaponData[WP_AUXILIARY6].ammoIndex] > 0)
		{
			CG_DrawPic(data.hud.general.nadeX, data.hud.general.nadeY, data.hud.general.nadeW, data.hud.general.nadeH, trap_R_RegisterShader(va("%s-stun", data.hud.general.nadeHeader)));
			trap_R_SetColor((const float *)data.hud.general.nadeTextColor);
			CG_DrawNumField(data.hud.general.nadeTextX, data.hud.general.nadeTextY, data.hud.general.nadeTextW, cg.predictedPlayerState.ammo[weaponData[WP_AUXILIARY6].ammoIndex], data.hud.general.nadeCharW, data.hud.general.nadeCharH, data.hud.general.nadeFont, qfalse);
		}
		else if(cg.predictedPlayerState.stats[STAT_WEAPONS] & (1 << WP_AUXILIARY7) && cg.predictedPlayerState.ammo[weaponData[WP_AUXILIARY7].ammoIndex] > 0)
		{
			CG_DrawPic(data.hud.general.nadeX, data.hud.general.nadeY, data.hud.general.nadeW, data.hud.general.nadeH, trap_R_RegisterShader(va("%s-smoke", data.hud.general.nadeHeader)));
			trap_R_SetColor((const float *)data.hud.general.nadeTextColor);
			CG_DrawNumField(data.hud.general.nadeTextX, data.hud.general.nadeTextY, data.hud.general.nadeTextW, cg.predictedPlayerState.ammo[weaponData[WP_AUXILIARY7].ammoIndex], data.hud.general.nadeCharW, data.hud.general.nadeCharH, data.hud.general.nadeFont, qfalse);
		}
		else if(cg.predictedPlayerState.stats[STAT_WEAPONS] & (1 << WP_AUXILIARY8) && cg.predictedPlayerState.ammo[weaponData[WP_AUXILIARY8].ammoIndex] > 0)
		{
			CG_DrawPic(data.hud.general.nadeX, data.hud.general.nadeY, data.hud.general.nadeW, data.hud.general.nadeH, trap_R_RegisterShader(va("%s-gas", data.hud.general.nadeHeader)));
			trap_R_SetColor((const float *)data.hud.general.nadeTextColor);
			CG_DrawNumField(data.hud.general.nadeTextX, data.hud.general.nadeTextY, data.hud.general.nadeTextW, cg.predictedPlayerState.ammo[weaponData[WP_AUXILIARY8].ammoIndex], data.hud.general.nadeCharW, data.hud.general.nadeCharH, data.hud.general.nadeFont, qfalse);
		}
	}
...
void CG_DrawForceSelect( void ) 
{
	...
	if(cgs.gametype != GT_HOLOCRON && cgs.gametype != GT_JEDIMASTER)
		return;
	else if(cgs.gametype == GT_JEDIMASTER && !cg.snap->ps.isJediMaster)
		return;
...
//const int bits = cg.snap->ps.stats[ STAT_ITEMS ];
	...
	if (cgs.media.invenIcons[cg.itemSelect] && BG_IsItemSelectable(&cg.predictedPlayerState, cg.itemSelect))
	{
		...
		if (bg_itemlist[itemNdex].classname)
		{
			...
			{
				UI_DrawProportionalString(320, y+45, bg_itemlist[itemNdex].classname, UI_CENTER | UI_BIGFONT, textColor);
			}*/
			//TODO: language specific settings
			UI_DrawProportionalString(320, y+45, killstreakData[rewardIndexForGiTag(bg_itemlist[itemNdex].giTag)].displayText, UI_CENTER | UI_SMALLFONT, textColor);
		}
	}
...
static float CG_DrawMiniScoreboard ( float y ) 
{
	...
	{
		strcpy ( temp, va("%s: ", /*CG_GetStringEdString("MP_INGAME", "RED")*/ Info_ValueForKey(info, "g_redteam")));
		Q_strcat ( temp, MAX_QPATH, cgs.scores1==SCORE_NOT_PRESENT?"-":(va("%i",cgs.scores1)) );
		Q_strcat ( temp, MAX_QPATH, va(" %s: ", Info_ValueForKey(info, "g_blueteam")) );
...
static float CG_DrawEnemyInfo ( float y ) 
{
	...
	if( cgs.gametype == GT_HERO ){
		if(cgs.redHero >= 0 && cgs.blueHero >= 0){
			if( cg.clientNum == cgs.redHero || cg.clientNum == cgs.blueHero ){
				title = CG_GetStringEdString("GSA_INGAME", "YOUAREHERO");
				clientNum = cg.clientNum;
			}
			else if(cg.snap->ps.persistant[PERS_TEAM] == TEAM_RED){
				clientNum = cgs.redHero;
				title = CG_GetStringEdString("GSA_INGAME", "YOURHERO");
			} else if(cg.snap->ps.persistant[PERS_TEAM] == TEAM_BLUE){
				clientNum = cgs.blueHero;
				title = CG_GetStringEdString("GSA_INGAME", "YOURHERO");
			} else{
				//We're not on a team, so we have no use to be here.
				return y;
			}
		}
	}
...
float	cg_radarRange = 6000.0f;

#define RADAR_RADIUS			75
#define RADAR_X					(SCREEN_WIDTH - (RADAR_RADIUS*2))
...
#pragma region players
			{
				...
				if(cl->team != cgs.clientinfo[cg.predictedPlayerState.clientNum].team)
				{
					VectorCopy4(colorRed, color);
					if(cent->aMuzzleFlashTime >= (cg.time - 2000))
					{
						color[3] = /*sin*/((((float)cg.time - (float)cent->aMuzzleFlashTime)/(2000.0f))/**M_PI*/);
						color[3] = 1.0  - color[3];
						color[0] = color[3];
					}
					else if(cent->muzzleFlashTime >= (cg.time - 2000))
					{
						color[3] = /*sin*/((((float)cg.time - (float)cent->muzzleFlashTime)/(2000.0f))/**M_PI*/);
						color[3] = 1.0  - color[3];
						color[0] = color[3];
					}
					trap_R_SetColor( color );
...
=================
*/
qhandle_t CG_LeaderLineGraphicForType(int llType)
{
	//TODO: precache
	switch(llType)
	{
		case LL_CAPTURE:
			return trap_R_RegisterShader("gfx/2d/leader/capture");
			break;
		case LL_CAPTUREA:
			return trap_R_RegisterShader("gfx/2d/leader/captureA");
			break;
		case LL_CAPTUREB:
			return trap_R_RegisterShader("gfx/2d/leader/captureB");
			break;
		case LL_CAPTUREC:
			return trap_R_RegisterShader("gfx/2d/leader/captureC");
			break;
		case LL_ESCAPE:
			return trap_R_RegisterShader("gfx/2d/leader/escape");
			break;
		case LL_KATANA:
			return trap_R_RegisterShader("gfx/2d/leader/katana");
			break;
		case LL_KILL:
			return trap_R_RegisterShader("gfx/2d/leader/kill");
			break;
		case LL_OBJECTIVE:
			return trap_R_RegisterShader("gfx/2d/leader/objective");
			break;
		case LL_RETURN:
			return trap_R_RegisterShader("gfx/2d/leader/return");
			break;
		case LL_SUBDUE:
			return trap_R_RegisterShader("gfx/2d/leader/subdue");
			break;
	}
	return 0;
}

extern void CG_DeleteLeaderLineEnt(unsigned short number);
static void CG_DrawLeaderLines ( void )
{
	centity_t *cent;
	unsigned short i;
	trap_R_SetColor(NULL);

	for(i = 0; i < cg.leaderLineCount; i++)
	{
		vec3_t dirPlayer;
		vec3_t angles;
		vec3_t angles2;
		...
		if(cg.leaderLines[i].entNum == -1)
			continue;

		if(cgs.gametype >= GT_TEAM)
		{
			//Perform team-based checks here
			if(cg.predictedPlayerState.persistant[PERS_TEAM] == TEAM_BLUE)
			{
				if(cg.leaderLines[i].visibilityMask & LLM_RED)
				{
					//On BLUE, and this leader line shouldn't be visible
					CG_DeleteLeaderLineEnt(i);
					continue;
				}
			}
			else if(cg.predictedPlayerState.persistant[PERS_TEAM] == TEAM_RED)
			{
				if(cg.leaderLines[i].visibilityMask & LLM_BLUE)
				{
					//On RED, and this leader line shouldn't be visible.
					CG_DeleteLeaderLineEnt(i);
					continue;
				}
			}
		}
		if(cgs.gametype != GT_JEDIMASTER && cg.leaderLines[i].type == LL_KATANA)
		{
			//What the..? well, don't want this to happen.
			CG_DeleteLeaderLineEnt(i);
			continue;
		}
		cent = &cg_entities[cg.leaderLines[i].entNum];

		VectorSubtract( cent->currentState.origin, cg.predictedPlayerState.origin, angles2 );
		distance = VectorLength ( angles2 );
		vectoangles ( angles2, angles2 );

		//else
		if(!InFieldOfVision(cg.predictedPlayerState.viewangles, 100, angles2) || distance >= 16384.0f)
		{
			VectorSubtract( cg.predictedPlayerState.origin, cent->lerpOrigin, dirPlayer );
			VectorNormalize( dirPlayer );

			vectoangles(dirPlayer, angles);
			...
			{
				CAP(x, SCREEN_WIDTH-80);
			}

			BUMP(y, 40);
			else
			{
				CAP(y, SCREEN_HEIGHT-40);
			}

			//Draw rotated arrows pointing us to where the leader line is at
			CG_DrawRotatePic2(x, y, 32, 16, ((angle * 180.0f)/M_PI)+90, cgs.media.leaderLineArrowShader);

			//Draw the actual leader line...this is NOT rotated!, but it has a lower cap than the arrow
			BUMP(x, 90);
			else //mind = blown yet?
			{
				CAP(x, SCREEN_WIDTH-130);
			}

			BUMP(y, 45);
			else
			{
				CAP(y, SCREEN_HEIGHT-100);
			}
			CG_DrawPic(x, y, 64, 64, CG_LeaderLineGraphicForType(cg.leaderLines[i].type));
		}
	}
}
...
/*
=================
CG_DrawGrenadeIndicator
=================
*/
extern void CG_DeleteGrenadeIndicatorEnt(unsigned short number);
static void CG_DrawGrenadeIndicator ( void )
{
	unsigned int i;
	centity_t *cent;
	...
	for(i = 0; i < cg.grenadeIndicatorCount+1; i++)
	{
		vec3_t dirPlayer;
		vec3_t angles;
		float angle;
		...
		if(cg.grenadeIndicators[i] == -1)
			continue;
		else if(cg.grenadeIndicators[i] < MAX_CLIENTS) //FIXME: ??
		{
			CG_DeleteGrenadeIndicatorEnt(cg.grenadeIndicators[i]);
			continue;
		}
		cent = &cg_entities[cg.grenadeIndicators[i]];
		if(cent->currentState.eType != ET_MISSILE || cent->currentState.weapon != WP_THERMAL)
		{
			CG_DeleteGrenadeIndicatorEnt(cg.grenadeIndicators[i]);
			continue;
		}
		...
		//Distances first
		if(VectorDistance( cent->currentState.origin, cg.predictedPlayerState.origin ) > 512.0f)
			continue;
		//VectorSubtract( cg.predictedPlayerState.origin, cent->currentState.origin, dirPlayer );
		VectorSubtract( cg.predictedPlayerState.origin, cent->lerpOrigin, dirPlayer );
		VectorNormalize( dirPlayer );
		...
		CG_DrawRotatePic2(x, y, 32, 90, ((angle * 180.0f)/M_PI)+90, cgs.media.grenadeIndicatorShader);
		//...dear god.
		/*AngleVectors( cg.predictedPlayerState.viewangles, dirLook, NULL, NULL );

		dirLook[2] = 0;
		anglePlayer = atan2(dirPlayer[1], dirPlayer[0]);
		VectorNormalize(dirLook);
		angleLook = atan2(dirPlayer[1], dirLook[0]);
		angle = angleLook - anglePlayer;

		angleDeg = angle * 180.0f / M_PI;
		angle -= M_PI / 2;

		CG_DrawRotatePic2((320 + 200 * cos(angle)), (240 + 200 * sin(angle)), 32, 90, angleDeg, cgs.media.grenadeIndicatorShader);*/
	}
	return;
}
...
static float CG_DrawTimer( float y ) {
	...
	{
		//Poorly optimized, but it really doesn't matter too much.
		s = va( "%i:%.2i", T_hour(), T_minute() );
		w = CG_DrawStrlen( s ) * BIGCHAR_WIDTH;
...
static void CG_DrawTeamOverlay( float y, qboolean right, qboolean upper ) {
	int x, w, h, xx, yy;
	int i, len;
	const char *p;
	vec4_t		hcolor;
	...
	if(cg_compactteamoverlay.integer)
		imageSize = 10;
	else
		imageSize = 20;
	...
	pwidth = 0;
	count = (numSortedTeamPlayers > 16) ? 16 : numSortedTeamPlayers;
	for (i = 0; i < count; i++) {
	...
	w = (pwidth + lwidth + 4 + 7) * (TINYCHAR_WIDTH/2);
...
//	CG_DrawPic( x + xOffset, y, w, h, cgs.media.teamStatusBar );
	//Old drawing method stuff above, ignore that.
	trap_R_SetColor( NULL );
	...
	for (i = 0; i < count; i++) {
		...
		if ( ci->infoValid && ci->team == cg.snap->ps.persistant[PERS_TEAM]) {
			...
			xx = x;
			if(cg_compactteamoverlay.integer)
				yy = y + TINYCHAR_HEIGHT/2;
			else
				yy = y + TINYCHAR_HEIGHT;

			CG_DrawStringExt( x, y,
				ci->name, hcolor, qfalse, qfalse,
				(TINYCHAR_WIDTH/3), (TINYCHAR_HEIGHT/2), TEAM_OVERLAY_MAXNAME_WIDTH);

			//CG_DrawPic( x + xOffset, (y + yOffset)-20, 20, 20, cgs.media.teamOverlay.dead );
			if(ci->health > 0 && ci->health < 126){
				switch(ci->curWeapon){
					case WP_BRYAR_OLD:
						if(ci->health >= 80){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.ak_green );
						}
						else if(ci->health < 80 && ci->health >= 50){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize,imageSize, imageSize, cgs.media.teamOverlay.ak_yellow );
						}
						else if(ci->health < 50 && ci->health >= 25){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.ak_orange );
						}
						else if(ci->health < 25){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.ak_red );
						}
						break;
					case WP_THERMAL:
					case WP_TRIP_MINE:
						if(ci->health >= 80){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.grenade_green );
						}
						else if(ci->health < 80 && ci->health >= 50){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.grenade_yellow );
						}
						else if(ci->health < 50 && ci->health >= 25){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.grenade_orange );
						}
						else if(ci->health < 25){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.grenade_red );
						}
						break;
					case WP_DET_PACK:
						if(ci->health >= 80){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.c4_green );
						}
						else if(ci->health < 80 && ci->health >= 50){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.c4_yellow );
						}
						else if(ci->health < 50 && ci->health >= 25){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.c4_orange );
						}
						else if(ci->health < 25){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.c4_red );
						}
						break;
					case WP_REPEATER:
						if(ci->health >= 80){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.m16_green );
						}
						else if(ci->health < 80 && ci->health >= 50){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.m16_yellow );
						}
						else if(ci->health < 50 && ci->health >= 25){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.m16_orange );
						}
						else if(ci->health < 25){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.m16_red );
						}
						break;
					case WP_FLECHETTE:
						if(ci->health >= 80){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.shotgun_green );
						}
						else if(ci->health < 80 && ci->health >= 50){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.shotgun_yellow );
						}
						else if(ci->health < 50 && ci->health >= 25){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.shotgun_orange );
						}
						else if(ci->health < 25){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.shotgun_red );
						}
						break;
					case WP_CONCUSSION:
						if(ci->health >= 80){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.saw_green );
						}
						else if(ci->health < 80 && ci->health >= 50){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.saw_yellow );
						}
						else if(ci->health < 50 && ci->health >= 25){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.saw_orange );
						}
						else if(ci->health < 25){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.saw_red );
						}
						break;
					case WP_BOWCASTER:
						if(ci->health >= 80){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.ump_green );
						}
						else if(ci->health < 80 && ci->health >= 50){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.ump_yellow );
						}
						else if(ci->health < 50 && ci->health >= 25){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.ump_orange );
						}
						else if(ci->health < 25){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.ump_red );
						}
						break;
					case WP_DISRUPTOR:
						if(ci->health >= 80){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.sniper_green );
						}
						else if(ci->health < 80 && ci->health >= 50){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.sniper_yellow );
						}
						else if(ci->health < 50 && ci->health >= 25){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.sniper_orange );
						}
						else if(ci->health < 25){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.sniper_red );
						}
						break;
					default:
						if(ci->health >= 80){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.misc_green );
						}
						else if(ci->health < 80 && ci->health >= 50){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.misc_yellow );
						}
						else if(ci->health < 50 && ci->health >= 25){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.misc_orange );
						}
						else if(ci->health < 25){
							CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.misc_red );
						}
						break;
				}
			}
			else{
				CG_DrawPic( x + xOffset, (y + yOffset)-imageSize, imageSize, imageSize, cgs.media.teamOverlay.dead );
			}
			...
			if (lwidth) {
				...
				if (len > lwidth)
					len = lwidth;
				if(cg_compactteamoverlay.integer){
						xx = x + TINYCHAR_WIDTH + TINYCHAR_WIDTH * (pwidth/2);
					CG_DrawStringExt( x + xOffset, (y+TINYCHAR_HEIGHT/4),
						p, hcolor, qfalse, qfalse, (TINYCHAR_WIDTH/6), (TINYCHAR_HEIGHT/4),
...
//					((lwidth/2 - len/2) * TINYCHAR_WIDTH);
					xx = x + TINYCHAR_WIDTH * 2 + TINYCHAR_WIDTH * pwidth;
					CG_DrawStringExt( x + xOffset, (y+TINYCHAR_HEIGHT/2),
						p, hcolor, qfalse, qfalse, (TINYCHAR_WIDTH/3), (TINYCHAR_HEIGHT/2),
						TEAM_OVERLAY_MAXLOCATION_WIDTH);
				}
			}
...
/*			xx += TINYCHAR_WIDTH * 3;
	...
	return; //return zero no matter what, as there's no need to shift anything on the right.
//#endif
}
...
static void CG_DrawUpperRight( void ) {
	...
	//[/CoOp]
		CG_DrawTeamOverlay( y, qfalse, qtrue );
	} 
...
static void CG_WeaponSwap( int y ){
	cg.weaponSwapTime = cg.time + 50;
	cg.weaponSwapY = y+20;
	cg.weaponSwapCharWidth = 10;
}

void CG_WeaponSwapInit( int weapon ){
	int useKey = trap_Key_GetKey("+use");
	int ADSKey = trap_Key_GetKey("+button13");
	switch(weapon){
		case WP_MELEE:
		case WP_NONE:
		case WP_EMPLACED_GUN:
		case WP_TURRET:
			strcpy(cg.weaponSwap, "You dunn fucked up.");
			break;
		default:
#ifdef AKIMBO
			if(weaponData[weapon].akimbo && weaponData[cg.predictedPlayerState.weapon].akimbo)
			{
				strcpy(cg.weaponSwap2, va("^7Press ^3%s ^7+ ^3%s ^7to equip ^3%s^7 in left hand.", bindingname[useKey].bindname, bindingname[ADSKey].bindname, weaponData[weapon].weaponName));
			}
			else
			{
				strcpy(cg.weaponSwap2, "");
			}
#endif
			strcpy(cg.weaponSwap, va("^7Press ^3%s ^7to swap for ^3%s^7", bindingname[useKey].bindname, weaponData[weapon].weaponName));
			break;
	}
	cg.weaponSwapWeapon = weapon;
	cg.weaponSwapHeight = weaponData[weapon].weaponSwapW / 2;
	cg.weaponSwapWidth = weaponData[weapon].weaponSwapL / 2;
	CG_WeaponSwap(SCREEN_HEIGHT * 0.85);
}

void CG_WeaponAddAmmoInit( int weapon ){
	int useKey = trap_Key_GetKey("+use");
	strcpy(cg.weaponSwap, va("^7Press ^3%s ^7to get ammo for ^3%s^7", bindingname[useKey].bindname, weaponData[weapon].weaponName));
	cg.weaponSwapWeapon = weapon;
	cg.weaponSwapHeight = weaponData[weapon].weaponSwapW / 2;
	cg.weaponSwapWidth = weaponData[weapon].weaponSwapL / 2;
	CG_WeaponSwap(SCREEN_HEIGHT * 0.85);
...
static void CG_DrawWeaponSwap( void ) {
	char	*start;
	int		l;
	int		x, y, w;
	int h;
	float	*color;
	const float scale = 0.4; //0.5

	if ( !cg.weaponSwapTime ) {
		return;
	}

	color = CG_FadeColor( cg.weaponSwapTime, 50 * 1 );
	if ( !color ) {
		return;
	}
	...
	y = cg.weaponSwapY - BIGCHAR_HEIGHT / 2;
	...
	while ( 1 ) 
	{
...
#ifdef AKIMBO
		w = CG_Text_Width(cg.weaponSwap2, scale, FONT_MEDIUM);
		h = CG_Text_Height(cg.weaponSwap2, scale, FONT_MEDIUM);
		x = (SCREEN_WIDTH - w) / 2;
		CG_Text_Paint(x, y + (h*2), scale, color, cg.weaponSwap2, 0, 0, ITEM_TEXTSTYLE_SHADOWEDMORE, FONT_MEDIUM);
		...
		CG_DrawPic(x +(w/2)-(cg.weaponSwapWidth/2), y - cg.weaponSwapHeight, cg.weaponSwapWidth, cg.weaponSwapHeight, cgs.media.weaponSwapIcons[cg.weaponSwapWeapon]);
...
===============================================================================
*/
void CG_LockPrint( const char *str, int y, int charWidth, float scale )
{
...
A center print that appears below the crosshair
==============
*/
void CG_SubCenterPrint( const char *str, int y, int charWidth, float scale ) {
	char	*s;
	//[BugFix19]
	int		i = 0;
	//[/BugFix19]
	...
	Q_strncpyz( buffer, data.hud.general.centerPrintHeader, sizeof(buffer) );
	Q_strcat(buffer, sizeof(buffer), str);
	Q_strcat(buffer, sizeof(buffer), data.hud.general.centerPrintFooter);

	Q_strncpyz( cg.subCenterPrint, buffer, sizeof(cg.subCenterPrint) );

	cg.subCenterPrintTime = cg.time;
	cg.subCenterPrintY = y;
	cg.subCenterPrintCharWidth = (charWidth * data.hud.general.cbScale);
	cg.subCenterPrintScale = scale;

	// count the number of lines for centering
	cg.subCenterPrintLines = 1;
	s = cg.subCenterPrint;
	while( *s ) 
	{
		//[BugFix19]
...
/*
===================
CG_DrawSubCenterString
===================
*/
static void CG_DrawSubCenterString( void ) {
	char	*start;
	int		l;
	int		x, y, w;
	int h;
	float	*color;
	const float scale = cg.subCenterPrintScale; //0.5

	if ( !cg.subCenterPrintTime ) {
		return;
	}

	color = CG_FadeColor( cg.subCenterPrintTime, 1000 * cg_centertime.value );
	if ( !color ) {
		return;
	}
	...
	start = cg.subCenterPrint;

	y = cg.subCenterPrintY - cg.subCenterPrintLines * BIGCHAR_HEIGHT / 2;
	...
	while ( 1 ) 
	{
		...
		if(!data.hud.general.customCBFont){
			w = CG_Text_Width(linebuffer, scale, data.hud.general.centerPrintFont);
			h = CG_Text_Height(linebuffer, scale, data.hud.general.centerPrintFont);
			x = (SCREEN_WIDTH - w) / 2;
			CG_Text_Paint(x, y + h, scale, color, linebuffer, 0, 0, ITEM_TEXTSTYLE_SHADOWEDMORE, data.hud.general.centerPrintFont);
			y += h + 6;
		} else{
			w = trap_R_Font_StrLenPixels(linebuffer, trap_R_RegisterFont(data.hud.general.cbFontCustomLoc), scale);
			h = trap_R_Font_HeightPixels(trap_R_RegisterFont(data.hud.general.cbFontCustomLoc), scale);
			x = (SCREEN_WIDTH - w) / 2;
			trap_R_Font_DrawString(x, y + h, linebuffer, color, trap_R_RegisterFont(data.hud.general.cbFontCustomLoc), 128, scale);
			y += h + (0.5 * trap_R_Font_HeightPixels(trap_R_RegisterFont(data.hud.general.cbFontCustomLoc), scale));
		}
...
void CG_CenterPrint( const char *str, int y, int charWidth ) {
	...
	Q_strncpyz( cg.centerPrint, buffer, sizeof(cg.centerPrint) );

	cg.centerPrintTime = cg.time;
	cg.centerPrintY = y;
	cg.centerPrintCharWidth = (charWidth * data.hud.general.cbScale);
...
static void CG_DrawCenterString( void ) {
	...
	int h;
	float	*color;
	const float scale = data.hud.general.cbScale; //0.5
...
//[/CoOp]
extern float CG_IronSightsPhase( const playerState_t *ps );
static void CG_DrawCrosshair( vec3_t worldPoint, int chEntValid ) {
	...
	float		chX, chY;
//	weaponData	weaponData[WP_NUM_WEAPONS];
	int cmdNum;
	usercmd_t	cmd;
	...
	if(cg.predictedPlayerState.eFlags2 & EF2_LASER_SIGHT)
		return;
	...
	else if(chEntValid && cg_entities[cg.crosshairClientNum].currentState.shouldtarget)
	{
		//TROLOLOLOLO
	}
	else
	{
		...
		if(weaponData[cg.predictedPlayerState.weapon].ADSTime)
		{
			//Fade this out awesomely as we go into ADS
			ecolor[3] = 1.0 - CG_IronSightsPhase(&cg.predictedPlayerState);
			if(ecolor[3] <= 0.99)
				goto endPoint;
		}
		...
		else if (chEntValid && cg_entities[cg.crosshairClientNum].currentState.eFlags2 & EF2_SWAPPABLE_WEAPON)
		{
			crossEnt = &cg_entities[cg.crosshairClientNum];
			strcpy(cg.weaponSwap, va("Press USE to swap for %s", weaponData[crossEnt->currentState.weapon].weaponName));
			cg.weaponSwapWeapon = crossEnt->currentState.weapon;
			CG_WeaponSwap(SCREEN_HEIGHT * 0.85);
		}
...
endPoint:
			ecolor[3] = 1.0;
			//if(cmd.buttons & BUTTON_SIGHTS && !cg_thirdPerson.integer && !(cmd.buttons & BUTTON_SPRINT))
			if(weaponData[cg.predictedPlayerState.weapon].ADSTime)
			{
				//Fade this out awesomely as we go into ADS
				ecolor[3] = 1.0 - CG_IronSightsPhase(&cg.predictedPlayerState);
	...
	if(cg_crosshairStyle.integer == 0)
	{ //Old style
		if(weaponData[cg.predictedPlayerState.weapon].ADSTime)
		{
			//Fade this out awesomely as we go into ADS
		...
		{//I'm in a vehicle
			...
			//bigger by default
			w = cg_crosshairSize.value*2.0f + (cg.snap->ps.stats[STAT_ACCURACY] * weaponData[cg.snap->ps.weapon].accuracyThreshold);
			h = w;
		}
		else
		{
			w = h = cg_crosshairSize.value + ((float)cg.snap->ps.stats[STAT_ACCURACY]/5*16.0f  + (weaponData[cg.snap->ps.weapon].accuracyDrain[cg.snap->ps.stats[STAT_FPOS]]/5*16.0f));
		}
		...
		if(cg.poofTime < cg.time)
			trap_R_DrawStretchPic( chX, chY, w, h, 0, 0, 1, 1, hShader );

		//we poofed.
		else if(cg.poofTime > cg.time)
			trap_R_DrawStretchPic( chX, chY, w, h, 0, 0, 1, 1, cgs.media.crosshairShader[ (cg_drawCrosshair.integer + 1) % NUM_CROSSHAIRS ] );
		...
		else if (crossEnt && crossEnt->currentState.number < MAX_CLIENTS && crossEnt->currentState.shouldtarget)
		{
			if (cgs.gametype == GT_SIEGE)
			{
...
{
	if(power == FP_SPEED || power == FP_SEE || power == FP_ABSORB || power == FP_PROTECT || power == FP_RAGE)
		return qtrue;
...
*/
//[CO-OP: New]
extern qboolean GSA_CG_IsFriendlyNPC(int clientNum);
extern char *GSA_CG_RetrieveNPCName(int clientNum);
//[/CO-OP: New]
static void CG_DrawCrosshairNames( void ) {
	...
	if(cg_entities[cg.crosshairClientNum].currentState.shouldtarget)
		return; //eez - ignore smoked targets
	//rww - still do the trace, our dynamic crosshair depends on it

	//[CO-OP: New]
	if(cgs.gametype == GT_SINGLE_PLAYER && GSA_CG_IsFriendlyNPC(cg.crosshairClientNum))
	{ 
		...
		name = GSA_CG_RetrieveNPCName(cg.crosshairClientNum);
		...
		CG_Text_Paint(320 + (CG_Text_Width(sanitized, 0.5f, FONT_SMALL)/2), 310, 0.5f, tcolor, sanitized, 0, 0, 0, FONT_SMALL);
		trap_R_SetColor( NULL );
		return;
	}
	//[/CO-OP: New]
	...
	{
		UI_DrawProportionalString(320, 170, sanitized, UI_CENTER, tcolor);
		if(cgs.vipName == cg.crosshairClientNum && cgs.gametype == GT_VIP_ESCORT)
			UI_DrawProportionalString(320, 190, "(VIP)", UI_CENTER, tcolor);
		else if((cgs.blueCName == cg.crosshairClientNum || cgs.redCName == cg.crosshairClientNum) && cgs.gametype == GT_COMMANDER)
			UI_DrawProportionalString(320, 190, "(Commander)", UI_CENTER, tcolor);
	}
...
#define MAX_TIME_DRAW	10
static qboolean CG_DrawLobby(){
	int x, y, w, h, i, time, minutes, seconds;
	char timeString[MAX_TIME_DRAW];
	char g_pregameLobbyMessage[1024];
	...
	x = ( SCREEN_WIDTH ) / 2;
	y = SCREEN_HEIGHT - 40;
	time = (cg.pregameLobbyEnd - cg.time)/1000;
	minutes = time / 60;
	seconds = time % 60;

	timeString[0] = 0;
	g_pregameLobbyMessage[0] = 0;
	hostString[0] = 0;

	if(seconds < 10)
		Q_strncpyz(timeString, va("%i:0%i", minutes, seconds), MAX_TIME_DRAW);
	else
		Q_strncpyz(timeString, va("%i:%i", minutes, seconds), MAX_TIME_DRAW);

	CG_Text_Paint ( x - CG_Text_Width ("Time until next round:", 0.7f, FONT_SMALL) / 2,
		y - 20, 0.7f, colorWhite, "Time until next round:", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL );

	CG_Text_Paint ( x - CG_Text_Width ( timeString, 0.6f, FONT_SMALL ) / 2,
		y, 0.6f, colorWhite, timeString, 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL );
	...
	color[3] = 0.4f;
	trap_R_SetColor(color);
	CG_DrawPic((float)x, (float)y, (float)w, (float)h, cgs.media.pregameLobbyBackground);
	...
	y = SCREEN_HEIGHT - 398;
	trap_R_SetColor(colorWhite);
	for(i = 0; i < cg.pregameLobbyNum; i++){
		lobby = &cg.lobby[i];
		if(lobby->client < 0)
			continue;
		ci = &cgs.clientinfo[ lobby->client ];

		if(lobby->ping <= 100){
			CG_DrawPic (x, y+2, 16 * 0.3f, 16 * 0.3f, cgs.media.pingIcon1);
		}
		else if(lobby->ping > 100 && lobby->ping <= 270){
			CG_DrawPic (x, y+2, 16 * 0.3f, 16 * 0.3f, cgs.media.pingIcon2);
		}
		else if(lobby->ping > 270 && lobby->ping <= 700){
			CG_DrawPic (x, y+2, 16 * 0.3f, 16 * 0.3f, cgs.media.pingIcon3);
		}
		else if(lobby->ping > 700){
			CG_DrawPic (x, y+2, 16 * 0.3f, 16 * 0.3f, cgs.media.pingIcon4);
		}
		else{
			CG_DrawPic (x, y+2, 16 * 0.3f, 16 * 0.3f, cgs.media.pingIcon4);
		}

		if(cg.snap->ps.stats[STAT_CLIENTS_READY] & ( 1 << lobby->client )){
			CG_Text_Paint((float)x + 8, (float)y, 0.4f, colorWhite, "READY", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL);
			CG_Text_Paint((float)x + (CG_Text_Width("READY", 0.4f, FONT_SMALL)+10) + 8, (float)y, 0.4f, colorWhite, ci->name, 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL);
		}else{
			CG_Text_Paint((float)x + 8, (float)y, 0.4f, colorWhite, "NOT READY", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL);
			CG_Text_Paint((float)x + (CG_Text_Width("NOT READY", 0.4f, FONT_SMALL)+10) + 8, (float)y, 0.4f, colorWhite, ci->name, 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL);
		}
	...
	x= 234;
	y=SCREEN_HEIGHT - 398;
	strcpy(g_pregameLobbyMessage, CG_ConfigString(CS_LOBBYMESSAGE));
	CG_Text_Paint(x, y, 0.4f, colorWhite, (const char *)g_pregameLobbyMessage, 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM);

	info = CG_ConfigString(CS_SERVERINFO);
	v = Info_ValueForKey(info, "sv_hostname");
	u = Info_ValueForKey(info, "sv_maxclients");

	Q_strncpyz(hostString, va("%s [%i/%i]", v, cg.pregameLobbyNum, atoi(u)), 1024);

	x = ((SCREEN_WIDTH / 2) - CG_Text_Width(hostString, 0.8f, FONT_MEDIUM) / 2) ;
	y = 20;
	CG_Text_Paint(x, y, 0.8f, colorWhite, (const char *)hostString, 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM);

	x = ((SCREEN_WIDTH / 2) - CG_Text_Width(CG_GameTypeString(), 0.6f, FONT_MEDIUM)/2);
	y = 35;
	CG_Text_Paint(x, y, 0.6f, colorWhite, CG_GameTypeString(), 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM);
...
=================
*/
static void CG_DrawLobbyInit( void ) {
	cg.scoreFadeTime = cg.time;
	cg.scoreBoardShowing = CG_DrawLobby();
}
...
//		s = "Waiting for players";		
	...
	if (cgs.gametype == GT_DUEL || cgs.gametype == GT_POWERDUEL)
	{
		...
		if ( cgs.gametype == GT_FFA ) {
			...
			s = CG_GetStringEdString("MENUS", "DOMINATION");//"Capture the Ysalamiri";
		//[CoOp]
		} else if( cgs.gametype == GT_HERO ) {
			s = CG_GetStringEdString("MENUS", "HERO_TFFA");//"Hero TFFA";
		} else if( cgs.gametype == GT_VIP_ESCORT ) {
			s = CG_GetStringEdString("MENUS", "VIP_ESCORT");//"VIP Escort";
...
void CG_DrawFlagStatus()
{
	...
	int startDrawPos = 2;
	//int ico_size = 32;
	int ico_size = data.hud.ctf.flagIconSize;
	...
	if (cgs.gametype == GT_CTY)
	{
		if (team == TEAM_RED)
		{
			myFlagTakenShader = trap_R_RegisterShaderNoMip( data.hud.ctf.CTFRedMyFlagTaken );
			theirFlagShader = trap_R_RegisterShaderNoMip( data.hud.ctf.CTYBlueFlagTakenEnemy );
		}
		else
		{
			myFlagTakenShader = trap_R_RegisterShaderNoMip( data.hud.ctf.CTFBlueMyFlagTaken );
			theirFlagShader = trap_R_RegisterShaderNoMip( data.hud.ctf.CTYRedFlagTakenEnemy );
		}
	}
	else
	{
		...
		{
			myFlagTakenShader = trap_R_RegisterShaderNoMip( data.hud.ctf.CTFRedMyFlagTaken );
			theirFlagShader = trap_R_RegisterShaderNoMip( data.hud.ctf.CTFBlueFlagTaken );
		}
		else
		{
			myFlagTakenShader = trap_R_RegisterShaderNoMip( data.hud.ctf.CTFBlueMyFlagTaken );
			theirFlagShader = trap_R_RegisterShaderNoMip( data.hud.ctf.CTFRedFlagTaken );
		}
	}
	if(data.hud.ctf.vertical){
		if (CG_YourTeamHasFlag())
		{
			//CG_DrawPic( startDrawPos, 330, ico_size, ico_size, theirFlagShader );
			CG_DrawPic( data.hud.ctf.yourTeamX, data.hud.ctf.yourTeamY-startDrawPos, ico_size, ico_size, theirFlagShader );
		...
		if (CG_OtherTeamHasFlag())
		{
			...
			CG_DrawPic( data.hud.ctf.enemyTeamX, data.hud.ctf.yourTeamY-startDrawPos, ico_size, ico_size, myFlagTakenShader );
		}
	} else if(data.hud.ctf.horizontal){
		if (CG_YourTeamHasFlag())
		{
			//CG_DrawPic( startDrawPos, 330, ico_size, ico_size, theirFlagShader );
			CG_DrawPic( data.hud.ctf.yourTeamX-startDrawPos, data.hud.ctf.yourTeamY, ico_size, ico_size, theirFlagShader );
			startDrawPos += ico_size+2;
		}
		...
		{
			//CG_DrawPic( startDrawPos, 330, ico_size, ico_size, myFlagTakenShader );
			CG_DrawPic( data.hud.ctf.enemyTeamX-startDrawPos, data.hud.ctf.yourTeamY, ico_size, ico_size, myFlagTakenShader );
		}
	} else{
		if (CG_YourTeamHasFlag())
		{
			//CG_DrawPic( startDrawPos, 330, ico_size, ico_size, theirFlagShader );
			CG_DrawPic( data.hud.ctf.yourTeamX, data.hud.ctf.yourTeamY, ico_size, ico_size, theirFlagShader );
			startDrawPos += ico_size+2;
		}
		...
		{
			//CG_DrawPic( startDrawPos, 330, ico_size, ico_size, myFlagTakenShader );
			CG_DrawPic( data.hud.ctf.enemyTeamX, data.hud.ctf.yourTeamY, ico_size, ico_size, myFlagTakenShader );
		}
	}
}
...
static CGAME_INLINE void CG_ChatBox_DrawStrings(void)
{
	...
	int linesToDraw = 0;
	int i = 0;
	int x = data.hud.general.chatBoxX;
	int y = cg.scoreBoardShowing ? 475 : data.hud.general.chatBoxY;
	float fontScale = data.hud.general.chatBoxFontSize;
	int fontHeight;

	if (!cg_chatBox.integer || !data.hud.general.activeChatBox)
	{
		return;
	}
	...
	if(cg.pregameLobbyEnd > cg.time){
		...
		while ( i < numToDraw ) {
			...
			if(drawThese[i]->time - cg.time < 500)
			{
				color[3] = (drawThese[i]->time - cg.time)/500;
			}
			CG_Text_Paint(234, y, fontScale, colorWhite, drawThese[i]->string, 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL );
			y += ((CHATBOX_FONT_HEIGHT*fontScale)*drawThese[i]->lines);
			i++;
		}
	} else if(!data.hud.general.customChatBoxFont){
		...
		while (i < numToDraw)
		{
			...
			CG_Text_Paint(x, y, fontScale, colorWhite, drawThese[i]->string, 0, 0, ITEM_TEXTSTYLE_OUTLINED, data.hud.general.chatBoxFont );
			y += ((CHATBOX_FONT_HEIGHT*fontScale)*drawThese[i]->lines);
			i++;
		}
	} else{
		fontHeight = trap_R_Font_HeightPixels(trap_R_RegisterFont(data.hud.general.chatBoxFontCustomLoc), fontScale);
		y -= (fontHeight * linesToDraw);
		i = 0;
		while(i < numToDraw){
			trap_R_Font_DrawString(x, y, drawThese[i]->string, colorWhite, trap_R_RegisterFont(data.hud.general.chatBoxFontCustomLoc), sizeof(drawThese[i]->string), fontScale);
			y += (fontHeight * drawThese[i]->lines);
			i++;
		}
	}
}
...
/*====================================
EVENT BOX: displays messages such as, "X took the Red Flag!"
====================================*/

//add chatbox string
void CG_EVBox_AddString(char *chatStr)
{
	evBoxItem_t *chat = &cg.evItems[cg.evItemActive];
	float chatLen;

	if (cg_chatBox.integer<=0 || !data.hud.general.activeEventBox)
	{ //don't bother then.
		return;
	}

	memset(chat, 0, sizeof(evBoxItem_t));
	...
	cg.evItemActive++;
	if (cg.evItemActive >= MAX_CHATBOX_ITEMS)
	{
		cg.evItemActive = 0;
	}
}
...
//insert item into array (rearranging the array if necessary)
void CG_EVBox_ArrayInsert(evBoxItem_t **array, int insPoint, int maxNum, evBoxItem_t *item)
{
    if (array[insPoint])
	{ //recursively call, to move everything up to the top
		if (insPoint+1 >= maxNum)
		{
			CG_Error("CG_EVBox_ArrayInsert: Exceeded array size");
		}
		CG_EVBox_ArrayInsert(array, insPoint+1, maxNum, array[insPoint]);
	}
...
//go through all the chat strings and draw them if they are not yet expired
static CGAME_INLINE void CG_EVBox_DrawStrings(void)
{
	evBoxItem_t *drawThese[MAX_CHATBOX_ITEMS];
	int numToDraw = 0;
	int linesToDraw = 0;
	int i = 0;
	int x = 200;
	int y = cg.scoreBoardShowing ? 475 : 400;
	float fontScale = data.hud.general.evBoxFontSize;
	int fontSize;
	int fontRegister;

	if (!cg_chatBox.integer || !data.hud.general.activeEventBox)
	{
		return;
	}
	...
	while (i < MAX_CHATBOX_ITEMS)
	{
		if (cg.evItems[i].time >= cg.time)
		{
			...
			{
				if (drawThese[check] &&
					cg.evItems[i].time < drawThese[check]->time)
				{ //insert here
					insertionPoint = check;
				}
				check--;
			}
			CG_EVBox_ArrayInsert(drawThese, insertionPoint, MAX_CHATBOX_ITEMS, &cg.evItems[i]);
			numToDraw++;
			linesToDraw += cg.chatItems[i].lines;
		}
		i++;
	...
	if(!data.hud.general.customChatBoxFont){
		//move initial point up so we draw bottom-up (visually)
		y -= (CHATBOX_FONT_HEIGHT*fontScale)*linesToDraw;
		...
		while (i < numToDraw)
		{
			CG_Text_Paint(x, y, fontScale, colorWhite, drawThese[i]->string, 0, 0, ITEM_TEXTSTYLE_OUTLINED, data.hud.general.evBoxFont );
			y += ((CHATBOX_FONT_HEIGHT*fontScale)*drawThese[i]->lines);
			i++;
		}
	} else{
		fontRegister = trap_R_RegisterFont(data.hud.general.evBoxFontCustomLoc);
		fontSize = trap_R_Font_HeightPixels(fontRegister, data.hud.general.chatBoxFontSize);
		y -= (fontSize * linesToDraw);
		i = 0;
		while(i < numToDraw){
			trap_R_Font_DrawString(x, y, drawThese[i]->string, colorWhite, fontRegister, 256, fontScale);
			y += (fontSize * drawThese[i]->lines);
			i++;
		}
	}
}
...
static void CG_Draw2D( void ) {
	...
	CG_DrawBeta();
#endif
	//bugfix: draw2D no longer blocks this.
	CG_DrawFlashbang();
